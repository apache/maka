  ...
  test('reclaims the bytes the v1 upgrade orphaned, including a user-deleted upload', async () => {
    await withInteractiveOwner(async (owner, root, track) => {
      ...
      for (const row of rows) {
        ...
        await writeFile(join(root, 'artifacts', relativePath), `bytes of ${row.id}`);
        db.prepare('INSERT INTO artifact_records VALUES (?, ?, ?, ?, ?, ?, ?)').run(
          row.id,
          row.id,
          'session-1',
      ...
      const drained = new DatabaseSync(join(root, 'runtime.sqlite'), { readOnly: true });
      assert.deepEqual(
        drained.prepare('SELECT relative_path FROM artifact_upgrade_orphan_paths').all(),
        [],
      );
      drained.close();
    });
