...
import { RuntimeHostAgentGraphComposition } from './agent-graph-composition.js';
...
import { createRuntimeHostSandboxComposition } from './sandbox-composition.js';
...
import {
  createRuntimeHostHistoryComposition,
  type RuntimeHostHistoryComposition,
} from './history-composition.js';
...
import { createRuntimeHostToolComposition } from './tool-composition.js';
...
import { createHostWebFetchService } from './web-fetch-tool.js';
...
export async function createExecutionRuntimeHostDomains(
  ...
  let agentGraph: RuntimeHostAgentGraphComposition | undefined;
  let sessionEffects: HostSessionEffectCoordinator | undefined;
  let memoryExtraction: HostMemoryExtractionCoordinator | undefined;
  let historyComposition: RuntimeHostHistoryComposition | undefined;
  ...
  try {
    ...
    const sandbox = createRuntimeHostSandboxComposition({
      ...(process.versions.electron ? { electronVersion: process.versions.electron } : {}),
      platform: process.platform,
    });
    workspaceExecution = sandbox.workspaceExecution;
    ...
    const tools = createRuntimeHostToolComposition({
      ...
      sandbox,
      taskLedger,
      getSessionManager: () => requireSessionManager(manager),
    });
    const {
      executionArtifacts,
      builtinTools,
    ...
    const graph = new RuntimeHostAgentGraphComposition(context.owner.capability.canonicalPath);
    agentGraph = graph;
    ...
    let canonicalProjection: RuntimeHostHistoryComposition['canonicalProjection'] | undefined;
    ...
    historyComposition = createRuntimeHostHistoryComposition({
      stores,
      ...
      sessionAdmission,
      requestDrain: context.requestDrain,
      publishSessionCatalog: (sessionId) => hostChanges.publishSessionCatalog(sessionId),
      ...(context.sessionAccessAuthority
        ? { sessionAccessAuthority: context.sessionAccessAuthority }
        : {}),
    });
    canonicalProjection = historyComposition.canonicalProjection;
    continuity = historyComposition.continuity;
    const continuityCoordinator = historyComposition.continuity;
    const canonicalPermissionOutcomes = historyComposition.canonicalPermissionOutcomes;
    ...
    const interactions = new HostInteractionCoordinator({
      ...
      preflightSessionSnapshot: (sessionId, interactionProjection) =>
        requireCanonicalProjection(canonicalProjection).fitsCandidate(sessionId, {
      ...
      onSandboxBoundarySettled: (sessionId) =>
        notifySandboxBoundaryGraphWake(
          ...
          stores.sessionStore,
          {
            listGraphIds: (rootSessionId) => graph.coordinator.listGraphIds(rootSessionId),
          },
          (rootSessionId) => graph.supervisorWake.notifyPermissionResponse(rootSessionId),
        ),
    });
        ...
        ((backendContext) =>
          createHostAiSdkBackend({
            ...
            createRunComposer: createInteractiveRunComposerFactory({
              ...
              builtinTools,
              hostTools,
              resolveRootTools: (sessionId) => graph.coordinator.toolsForSession(sessionId),
    ...
    resolveAvailableToolNames = async (sessionId: string): Promise<string[]> => {
      ...
      try {
        const [graphTools, planState] = await Promise.all([
          graph.coordinator.toolsForSession(sessionId),
    ...
    manager = new SessionManager({
      ...
      inspectContinuationSafety: createLocalContinuationSafetyInspector({
        ...
        hasPendingBackgroundOperations: async (sessionId) => {
          const graphAuthority = graph.coordinator;
          const graphWake = graph.supervisorWake;
          const [resourcesLive, graphLive, descendantLive] = await Promise.all([
            runtimeResources!.hasLiveSessionResources(sessionId),
            graphAuthority.hasLiveSessionState(sessionId),
            hasLiveLinkedDescendantState(
              ...
              async (descendantSessionId) =>
                ...
                graphAuthority.hasLiveSessionState(descendantSessionId) ||
      ...
      hostedAgentGraphExecution: {
        readAgentGraphIntentClaim: (graphId, intentId) =>
          graph.controlStore.readAgentGraphIntentClaim(graphId, intentId),
    ...
    graph.bindCoordinator(
      new AgentGraphCoordinator({
        sessionStore: stores.sessionStore,
        runStore: stores.agentRunStore,
        runtimeEventStore: stores.runtimeEventStore,
        controlStore: graph.controlStore,
        epochStore: graph.controlStore,
        runtime: manager,
        newId: randomUUID,
        acquireResidency: () => context.acquireResidency('agent-graph'),
        onReconciliation: (rootSessionId, result) => {
          void graph.supervisorWake.notify(rootSessionId, result);
        },
        onCheckpoint: (rootSessionId) => {
          void graph.supervisorWake.notify(rootSessionId);
        },
      }),
    );
    graph.bindClient(
      new HostAgentGraphCoordinator({
        authority: graph.coordinator,
        continuity: continuityCoordinator,
        stopExecution: (rootSessionId, expectedGraphId) =>
          graph.coordinator.stopExecution(rootSessionId, {
            expectedGraphId,
            stopSupervisor: () =>
            ...
            withSupervisorWakesSuppressed: (operation) =>
              graph.supervisorWake.runWithSessionWakesSuppressed(rootSessionId, operation),
          }),
      }),
    );
    ...
    rootCoordinator = new RootTurnCoordinator(
      ...
      {
        currentGraphId: (rootSessionId) => graph.coordinator.currentGraphId(rootSessionId),
        beginNextGraphEpoch: async (rootSessionId) =>
          (
            await graph.coordinator.beginNextGraphEpoch(rootSessionId, (operation) =>
              graph.supervisorWake.runWithSessionWakesSuppressed(
                rootSessionId,
                operation,
    ...
    graph.bindSupervisorWake(
      new AgentGraphSupervisorWakeCoordinator({
        activityRegistry: graph.activityRegistry,
        wakeStore: graph.controlStore,
        readSnapshot: (rootSessionId) => graph.coordinator.getSnapshot(rootSessionId),
    ...
    const sessionRetirement = new HostSessionRetirementCoordinator({
      ...
      sessionEffects: sessionEffectCoordinator,
      graph: graph.coordinator,
      graphWake: graph.supervisorWake,
      manager,
      ...
      purgeAgentGraphState: async (sessionId) => {
        for (const graphId of await graph.coordinator.listGraphIds(sessionId)) {
          await graph.controlStore.purgeAgentGraphControlState(graphId);
        }
        await graph.controlStore.purgeAgentGraphEpochs(sessionId);
      },
...
function requireCanonicalProjection(
  projection: RuntimeHostHistoryComposition['canonicalProjection'] | undefined,
): RuntimeHostHistoryComposition['canonicalProjection'] {
