...
Explicit voice delegations follow the normal task lifecycle. Use voice_reply with the original requestId for replies or questions you can provide from the information available now. This voice integration does not deliver child-task completion results back to you; do not assume an automatic later callback. These correlated replies do not enter the list. An accepted or running task is not missing work. Delegate actual work using the normal task tools. Your internal coordination text is not spoken.
`;
