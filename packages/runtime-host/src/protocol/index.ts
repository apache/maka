...
// Epoch-152 peers reject these strict input fields and previously oversized results.
// 152: Computer History adds bounded summaries, accounting, and request-bound cancellation.
...
/** Connection-local cancellation of an already sent, explicitly cancellable request. */
...
export function decodeClientFrame(value: unknown): ClientFrame {
  ...
  if (frame.kind === 'request.cancel') {
    requireExactRecord(frame, 'request cancellation', ['kind', 'requestId']);
    return { kind: 'request.cancel', requestId: requireId(frame.requestId, 'requestId') };
  }
  if (frame.kind === 'hello') {
