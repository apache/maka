...
const source = { sessionId: 'session-1', turnId: 'turn-1', terminalEventId: 'terminal-1',
  header: { model: 'model', llmConnectionSlug: 'provider' }, messages: [],
...
test('Chinese without whitespace is valid; empty/meta/multiline/oversized suggestions are discarded', () => {
  ...
  for (const raw of ['', 'none', 'no suggestion', '第一行\n第二行', 'a'.repeat(81), '<tool>run</tool>'])
...
test('prompt only contains bounded user/assistant text, never tool payloads or thinking', () => {
  const prompt = buildPromptSuggestionPrompt([
    ...
    ...Array.from({ length: 100 }, (_, i) => ({ type: 'assistant' as const, text: 'x'.repeat(5000), id: `a${i}`, ts: i + 2, turnId: 't', modelId: 'm' })),
...
test('protocol rejects arbitrary context, missing identity and multiline output', () => {
  ...
  assert.throws(() => spec.decodeOutput({ kind: 'generated', turnId: 't', terminalEventId: 'e', text: 'a\nb' }));
});
...
test('concurrent clients and repeat requests share one metered generation', async () => {
  ...
  const coordinator = new HostPromptSuggestionCoordinator({
    ...
    generate: async () => { calls++; return new Promise<string>((resolve) => { complete = resolve; }); },
  });
  const acquire = () => ({ release: () => { releases++; } }) as OperationResidency;
...
test('missing source discards a late result; failed effects are not retried', async () => {
  ...
  let complete!: (text: string) => void;
  let calls = 0;
  const coordinator = new HostPromptSuggestionCoordinator({ readSource: async () => current,
    generate: async () => { calls++; return new Promise<string>((resolve) => { complete = resolve; }); },
  });
  const first = coordinator.generate('session-1', lease);
  await new Promise((resolve) => setImmediate(resolve));
  current = undefined; complete('补上测试');
...
test('Host drain aborts the physical request and releases residency', async () => {
  let signal!: AbortSignal; let releases = 0;
  const coordinator = new HostPromptSuggestionCoordinator({ readSource: async () => source,
    generate: async (_, abortSignal) => { signal = abortSignal; return new Promise<string>((_, reject) => {
      abortSignal.addEventListener('abort', () => reject(abortSignal.reason), { once: true });
    }); },
  });
  const pending = coordinator.generate('session-1', () => ({ release: () => { releases++; } }) as OperationResidency);
  await new Promise((resolve) => setImmediate(resolve));
  await coordinator.close();
  assert.equal(signal.aborted, true); assert.equal(releases, 1);
