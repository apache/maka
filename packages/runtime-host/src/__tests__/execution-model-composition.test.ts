...
test('injects Auto tool guidance only into an eligible main-session prompt', async () => {
  const composition = createInteractiveRunComposer({
    runtimePolicy: { revision: 0, policy: createDefaultRuntimePolicy() },
    permissionMode: 'ask',
    skills: {
  ...
  assert.equal(composition.composerRevision, '2');
  assert.equal(prompt?.match(/## Auto-mode tool guidance/g)?.length, 1);
  assert.match(prompt ?? '', /Prefer Bash and composable CLI workflows/u);
  assert.match(prompt ?? '', /Prefer Read for simple structured inspection/u);
...
test('does not inject Auto guidance into child, restricted, or unavailable-shell prompts', async () => {
  ...
  const child = createInteractiveRunComposer({
    ...promptDependencies,
    childInstruction: 'Child role instructions',
  });
  const childPrompt = (
    await child.resolveSystemPrompt({
      sessionId: 'child-session',
      turnId: 'child-turn',
      cwd: '/workspace',
      workspaceRoot: '/workspace',
  ...
  assert.doesNotMatch(childPrompt ?? '', /Auto-mode tool guidance/u);

  const restricted = createInteractiveRunComposer({
    ...promptDependencies,
    boundTools: [composerTool('Bash'), composerTool('Read')],
  });
  const restrictedPrompt = (
    await restricted.resolveSystemPrompt({
  ...
  assert.doesNotMatch(restrictedPrompt ?? '', /Auto-mode tool guidance/u);

  const unavailableShell = createInteractiveRunComposer({
    ...promptDependencies,
    shell: {
      plan: { kind: 'posix', displayName: '/bin/sh' },
      setupError: new ShellPreferenceError('executable_missing', 'Bash is unavailable'),
    },
  });
  const unavailablePrompt = (
    await unavailableShell.resolveSystemPrompt({
...
test('does not append Auto guidance after the side-conversation boundary', async () => {
  ...
  assert.doesNotMatch(prompt ?? '', /Auto-mode tool guidance/u);
  assert.match(prompt ?? '', /Side conversation boundary:/u);
});

test('uses the Host permission snapshot instead of plan permission state', async () => {
  const composition = createInteractiveRunComposer({
    runtimePolicy: { revision: 0, policy: createDefaultRuntimePolicy() },
    permissionMode: 'ask',
    skills: {
  ...
  assert.match(prompt ?? '', /Auto-mode tool guidance/u);
});

test('factory forwards the Host header permission mode to the composer', async () => {
  const fixture = backendCreationFixture({
    abortSignal: new AbortController().signal,
    resolveExecutionConnection: async () => readyExecutionConnection(),
    readPricing: async () => ({ revision: 0, overrides: [] }),
  ...
  const composer = await factory({
    backendContext: {
      ...fixture.context,
      header: {
        ...fixture.context.header,
        permissionMode: 'ask',
        collaborationMode: 'plan',
      },
    },
    connection: readyExecutionConnection()
      .connection as unknown as import('@maka/core/llm-connections').RuntimeExecutionConnection,
    modelId: MODEL_ID,
    runtimePolicy: { revision: 0, policy: createDefaultRuntimePolicy() },
    contextWindow: null,
  ...
  const prompt = (
    await composer.resolveSystemPrompt({
...
test('uses the final routed tool surface rather than a filtered Bash candidate', async () => {
  const composition = createInteractiveRunComposer({
    runtimePolicy: { revision: 0, policy: createDefaultRuntimePolicy() },
    permissionMode: 'ask',
    skills: {
    ...
    taskLedger: {} as TaskLedgerStore,
    deepResearch: {
      tools: [composerTool('Bash'), composerTool('Read')],
    },
  });

  assert.equal(
    composition.tools.some(({ name }) => name === 'Bash'),
    false,
  );
  const prompt = (
    await composition.resolveSystemPrompt({
...
function composerTool(name: string): MakaTool {
  return {
    name,
