...
async function waitForCanonicalAttempts(
  ...
  let attempts: readonly ModelCallAttempt[] | undefined;
  try {
    await waitFor(
      async () => {
        ...
        if (page.attempts.length >= expectedRequests) {
...
async function waitForCaptureArtifacts(
  ...
  let captures: Awaited<ReturnType<typeof artifacts.listPage>>['records'] | undefined;
  await waitFor(
    async () => {
      const page = await artifacts.listPage(sessionId, { offset: 0, limit: 100 });
      const found = page.records.filter(
        (artifact) => artifact.source === 'provider_request_capture',
      );
      if (found.length >= expectedRequests) {
        captures = found;
        return true;
      }
      return false;
