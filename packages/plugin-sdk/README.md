...
The Host admits up to four concurrent page VMs, each with a 128 MiB JavaScript heap limit and a 200 ms synchronous execution slice. These are execution budgets, not total process memory limits or a hostile-code sandbox. Hidden drafts do not reserve VMs; accepted writes retain their owner until settlement. A failed page stops observation and preserves its last valid view and loaded transcript for local reading. Rebinding is explicit and never replays an uncertain write. Page failure does not restart the package's business activation or sibling page VMs.
