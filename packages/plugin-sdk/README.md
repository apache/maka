...
`tui.split(key, ratio, left, right)` creates a locally resizable pair. Drag the divider or focus it with Tab and use Left/Right; Shift takes larger steps, and Home/End reach the supported bounds. Resizing emits no Host request, including while a read is pending. Narrow layouts stack the panes and retain the local ratio and keyed editors when widened again.

`tui.boundary(key, body, { bottom?, padding?, emphasis?, activity? })` frames any body, including ordinary inputs. The optional bottom contains text, buttons or nested rows of those nodes; it is clipped to one right-aligned border row without taking body space. Body and bottom keys must differ. Padding has `horizontal` and `vertical` cells, each 0–4. Defaults are no bottom, zero padding, `emphasis: 'normal'` and `activity: 'idle'`. A visible `'accent'` / `'busy'` border uses the kernel's shared gentle breath, respecting reduced motion, focus and overlays; terminal-owned colors remain static. All nodes still share the View's depth, node and byte budgets and declared fields/actions. The Rust equivalent is `terminal_ui::view::build::boundary(key, body).bottom(node).padding(1, 0).emphasis(Emphasis::Accent).activity(Activity::Busy).into()`.
...
For live updates, `const changed = await ctx.tui.changes('changed')` registers a stream. Set `changes: 'changed'` on the descriptor and call `changed()` after committing data. Invalidation is coalesced; the shell rereads clean views and preserves edited drafts. The app declaration captures this exact stream registration. Each document owns its subscription; closing it leaves the shared business producer available to other pages.
...
// Declare resources: [log.resource] on tui.app; include log.resource in its Read model.
// The separate UI entry renders tui.transcript('log', model.resource).
...
The helper owns snapshots, cursors and cancellation per caller, document and reader mount. Open and Read both require the same mount UUID; multiple nodes can independently read the same source within one document. Removing a reader closes its stream; retiring a page closes its document and readers. It retains at most 4,096 records / 32 MiB of source; pages retain up to 256 records / 4 MiB, with one larger record allowed up to 16 MiB. Large records use ordered UTF-8 JSON fragments within Remote's message budget. Broadcast updates share immutable events and fragments across readers. Current records and distinct historical snapshot records share the 32 MiB encoded retention budget; pressure invalidates the oldest snapshots while preserving current data. An exhausted update queue invalidates only that reader. Idle stream waits retain their exact ownership without occupying ordinary execution slots. These encoded budgets do not measure total JavaScript heap or process RSS. Rust providers can serve the same [public records and resource protocol](../../crates/plugins/src/terminal_ui/transcript.rs) through ordinary Remote method/stream handlers; the app’s `resources` declaration captures exact source endpoints from the same activation, and unrelated same-package endpoints are unavailable to that page. A slot's context should contain stable entity identities, and its node key must distinguish independently editable entities.
