...
export interface MakaCuBackendOptions {
  ...
  binaryPath: string;
  /** Optional native host arguments; supervision remains in MakaCuService. */
...
> & {
  ...
  /**
   * Return the executor's fresh window inventory for an out-of-band harness.
   * The model-facing backend intentionally exposes only `listApps`; this seam
   * is for fixture ownership checks that must join a live PID to fresh HWNDs
   * without matching a display title as an application identity.
   */
  listWindows(signal: AbortSignal): Promise<MakaCuWindow[]>;
  observeApp(
    input: {
      app?: string;
      windowId?: number;
...
export function createMakaCuBackend(opts: MakaCuBackendOptions): MakaCuBackend {
  ...
  const service = new MakaCuService({
    ...
    ...(opts.childArgs === undefined ? {} : { childArgs: opts.childArgs }),
    imageDir,
  ...
  return {
    ...
    async listWindows(signal) {
      const sessionId = `maka-cu-inventory-${randomUUID()}`;
      return withOperationQueue(
        signal,
        async () => {
          await ensureSession(sessionId, signal);
          return listWindows(sessionId, signal);
        },
        sessionId,
      ).finally(() => {
