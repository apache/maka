...
test('Windows tools expose the same semantic-only schema as the shared backend', () => {
  const selected = selectComputerUseBackend({
    platform: 'win32',
    binaryPath: 'helper.exe',
    expectedBinarySha256: '0'.repeat(64),
  ...
  assert.ok(parameters.shape?.action?.options?.includes('click_element'));
  assert.ok(parameters.shape?.action?.options?.includes('scroll_element'));
  assert.ok(parameters.shape?.action?.options?.includes('press_key'));
  assert.equal(parameters.safeParse?.({ action: 'left_click' }).success, false);
  assert.equal(parameters.safeParse?.({ action: 'mouse_move' }).success, false);
  assert.equal(parameters.safeParse?.({ action: 'zoom' }).success, false);
  assert.match(selected.tools[0]?.description ?? '', /Coordinate mutation is not part/);
});
