...
export type ClientCapabilityGrantCapability =
  | 'browser'
  | 'computer_use'
  | 'computer_history'
  | 'desktop_mcp';
...
export function decodeClientCapabilityGrantTarget(value: unknown): ClientCapabilityGrantTarget {
  ...
  const capability = oneOf(
    record.capability,
    ['browser', 'computer_use', 'computer_history', 'desktop_mcp'] as const,
    'capability',
  );
  const scope = decodeClientCapabilityGrantScope(record.scope);
  if (
    (capability === 'browser' && scope.kind !== 'browser_origin') ||
    ((capability === 'computer_use' || capability === 'computer_history') &&
