...
import { resolveModelPdfInputContract } from '../model-runtime.js';
...
describe('native PDF input contract', () => {
  test('authorizes only PDF-capable models on verified first-party provider wires', () => {
    assert.deepEqual(
      resolveModelPdfInputContract(connection('anthropic', 'claude-opus-4-8'), 'claude-opus-4-8'),
      { providerType: 'anthropic', wire: 'anthropic-messages' },
    );
    assert.deepEqual(resolveModelPdfInputContract(connection('openai', 'gpt-4o'), 'gpt-4o'), {
      providerType: 'openai',
      wire: 'openai-chat',
    });
    assert.deepEqual(resolveModelPdfInputContract(connection('openai', 'gpt-5.4'), 'gpt-5.4'), {
      providerType: 'openai',
  ...
  test('does not infer PDF support for relays, subscriptions, or unknown models', () => {
    ...
    for (const providerType of [
      ...
      assert.equal(resolveModelPdfInputContract(relay, declaredPdfModel.id), null, providerType);
    }
    assert.equal(
      resolveModelPdfInputContract(connection('openai', 'unknown-model'), 'unknown-model'),
      null,
    );
    assert.equal(
      resolveModelPdfInputContract(
        {
