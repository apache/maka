...
describe('SessionManager permission mode updates', () => {
  ...
  test('conversation copies materialize requestless hosted permission history', async () => {
    ...
    assert.strictEqual(
      sourcePermissionEvents.some((event) => event.actions?.permissionRequest),
      false,
    );
    assert.strictEqual(
      sourcePermissionEvents.some((event) => event.content?.kind === 'function_call'),
      false,
    );
    await seedRuntimeRun(
      runStore,
    ...
    assert.strictEqual(
      childSourceEvents.some((event) => event.actions?.permissionAnswerAccepted),
      false,
    );
  ...
  test('branch child next turn receives cloned RuntimeEvent context', async () => {
    ...
    assert.deepStrictEqual(
      childInput.runtimeContext?.map((event) => event.turnId),
      ['source', 'source', 'source'],
    );
    assert.strictEqual(childInput.runtimeContext?.[0]?.sessionId, child.id);
  });
  ...
  test('required capture and later attempt still write after an attempt append fails', async () => {
    ...
    assert.deepStrictEqual(captureOutcomes, ['fulfilled']);
    const [run] = await runStore.listSessionRuns(session.id);
    const events = await runStore.readEvents(session.id, run!.runId);
    assert.strictEqual(
      events.some((event) => event.type === 'provider_request_captured'),
      true,
    );
    assert.strictEqual(
      events.some((event) => event.id === 'attempt-2'),
      true,
    );
  });
  ...
  test('carries a provider attempt failure latch into the terminal run header', async () => {
    ...
    assert.strictEqual(run?.status, 'completed');
    assert.match(
      String(run?.traceWriteError),
      /append provider request attempt: provider attempt append failed/,
    );
    const events = await runStore.readEvents(session.id, run!.runId);
    assert.strictEqual(
      events.some((event) => event.type === 'trace_write_failed'),
      false,
    );
  });
  ...
  test('finalizes the run when a required provider capture append fails', async () => {
    ...
    const [run] = await runStore.listSessionRuns(session.id);
    assert.strictEqual(run?.status, 'failed');
    assert.notStrictEqual(run?.completedAt, undefined);
  });
  ...
  test('branchFromTurn marks side conversations without changing ordinary branches', async () => {
    ...
    assert.deepStrictEqual(ordinary.labels, ['kept']);
    assert.deepStrictEqual(side.labels, ['kept', SIDE_CONVERSATION_SESSION_LABEL]);
  });
  ...
  test('conversation copies inherit the authoritative execution boundary', async () => {
    ...
    for (const [index, boundary] of boundaries.entries()) {
      ...
      for (const copy of [branch, revision]) {
        assert.deepStrictEqual(await store.readExecutionBoundary(copy.id), {
          ...boundary,
          revision: 0,
        });
      }
  ...
  test('fails before creating branch or revision sessions for child continuation authority', async () => {
    ...
    assert.deepStrictEqual(
      (await store.list()).map((candidate) => candidate.id),
      [session.id],
    );
  });
  ...
  test('hydrates an inherited running ShellRun with its source-session owner', async () => {
    ...
    assert.strictEqual(updates[0]?.sessionId, child.id);
    assert.deepStrictEqual(updates[0]?.ownership, {
      kind: 'source_owned',
      sourceSessionId: parent.id,
      ownerSessionId: parent.id,
    });
    assert.strictEqual(updates[0]?.sourceToolCallId, 'bash-1');
    assert.deepStrictEqual(updates[0]?.result, sourceSnapshot);
    assert.deepStrictEqual(
      await manager.getShellRunUpdate(child.id, sourceSnapshot.ref),
      updates[0],
    );
    assert.strictEqual(
    ...
    assert.strictEqual(revisionUpdates.length, 1);
    assert.deepStrictEqual(revisionUpdates[0]?.ownership, {
      kind: 'source_owned',
      sourceSessionId: parent.id,
      ownerSessionId: parent.id,
    });
    assert.deepStrictEqual(revisionUpdates[0]?.result, sourceSnapshot);

    ownerAvailable = false;
    await store.remove(parent.id);
    const danglingUpdates = await manager.listShellRunUpdates(child.id);
    assert.strictEqual(danglingUpdates.length, 1);
    assert.strictEqual(danglingUpdates[0]?.sessionId, child.id);
    assert.deepStrictEqual(danglingUpdates[0]?.ownership, {
      kind: 'source_unavailable',
      sourceSessionId: parent.id,
    });
    assert.strictEqual(danglingUpdates[0]?.result.status, 'running');
    assert.strictEqual(danglingUpdates[0]?.result.output, undefined);
    assert.deepStrictEqual(
      await manager.getShellRunUpdate(child.id, sourceSnapshot.ref),
      danglingUpdates[0],
    );
  ...
  test('reviseBeforeTurn creates an in-conversation version without ordinary branch lineage', async () => {
    ...
    assert.strictEqual(version2.name, 'Conversation');
    assert.strictEqual(version2.projectId, 'project-1');
    assert.strictEqual(version2.isFlagged, true);
    assert.strictEqual(version2.collaborationMode, 'plan');
    assert.strictEqual(version2.orchestrationMode, 'swarm');
    assert.strictEqual(version2.parentSessionId, undefined);
    assert.strictEqual(version2.branchOfTurnId, undefined);
    assert.strictEqual(version2.revisionRootSessionId, session.id);
    assert.strictEqual(version2.revisionParentSessionId, session.id);
    assert.strictEqual(version2.revisionOfTurnId, 'second');
    assert.strictEqual(version2.revisionIndex, 2);
    assert.strictEqual(version2.revisionState, 'preparing');
    assert.strictEqual((await store.readHeader(version2.id)).collaborationMode, 'plan');
    assert.strictEqual((await store.readHeader(version2.id)).orchestrationMode, 'swarm');
    await drain(
      manager.sendMessage(
        version2.id,
    ...
    assert.strictEqual((await store.readHeader(version2.id)).revisionState, 'committed');
    const messages = await store.readMessages(version2.id);
    assert.strictEqual(
      messages.some((message) => (message as { turnId?: string }).turnId === 'first'),
      true,
    );
    assert.strictEqual(
      messages.some((message) => (message as { turnId?: string }).turnId === 'second'),
      false,
    );

    const version3 = await manager.reviseBeforeTurn(version2.id, { sourceTurnId: 'first' });
    assert.strictEqual(version3.projectId, 'project-1');
    assert.strictEqual(version3.revisionRootSessionId, session.id);
    assert.strictEqual(version3.revisionParentSessionId, version2.id);
    assert.strictEqual(version3.revisionIndex, 3);
    assert.strictEqual(version3.revisionState, 'preparing');
    assert.strictEqual(version3.parentSessionId, undefined);
  });
  ...
  test('startup recovery removes empty preparing revisions and commits admitted edits', async () => {
    ...
    assert.strictEqual(empty.revisionState, 'preparing');
    await manager.recoverInterruptedSessions();
    ...
    assert.strictEqual((await store.readHeader(admitted.id)).revisionState, 'preparing');
    await manager.recoverInterruptedSessions();
    assert.strictEqual((await store.readHeader(admitted.id)).revisionState, 'committed');
  });
