...
export interface AiSdkBackendInput extends AiSdkCompactionCapabilities {
  ...
  /** True only when Runtime Host verified the provider identity and resolved wire for native PDFs. */
...
export class AiSdkBackend implements AgentBackend {
  ...
  private async materializeRuntimeReplayItem(
    ...
    if (item.role === 'user') {
      if (item.steering) {
        // The plan has already wrapped the envelope. Re-materialize its durable
        // attachments with the same key as live steering, then restore the
        // structured identity so injection dedupe recognizes the replayed message.
   ...
   * file parts. The PDF gate is a verified provider-identity/wire decision,
  ...
  private async appendAttachmentParts(
    ...
    for (const { attachment, index } of eligibleAttachments) {
      ...
      const mediaType = read.mimeType ?? attachment.mimeType;
      if (attachment.kind === 'pdf' && mediaType !== 'application/pdf') {
        parts.push({
          type: 'text',
          text: `PDF attachment "${attachment.name}" was omitted because its loaded bytes are ${mediaType}, not application/pdf.`,
        });
        continue;
      }
