...
export class AiSdkBackend implements AgentBackend {
    ...
    const pumpDone: Promise<void> = (async () => {
      ...
      try {
        ...
        const currentUserContent = input.continuation
          ? undefined
          : await this.messageProjection.buildCurrentUserContent(
              scope.imageBudget,
              input.text,
              input.attachments,
        ...
        const loadDurableTurnProjection = async (): Promise<ModelMessage[]> => {
          ...
          if (
            ...
            (replayPlan.hasProviderNativeSemantics &&
              !this.messageProjection.canReplayProviderNative(replayPlan))
          ) {
            throw new Error('durable current-run projection is not replayable');
          }
          const currentTurnMessages = await this.messageProjection.materializeRuntimeReplayPlan(
            replayPlan,
            scope.imageBudget,
            projectionCheckpoint,
            compatibleProviderReasoningReplayEventIds(
        ...
        agentLoop: for (;;) {
          ...
          for (;;) {
            ...
            scope.memorySourceEventMessagePositions =
              this.messageProjection.memoryEventMessagePositions(attemptMessages);
  ...
  private async buildPriorMessages(
    ...
    const materializeReplayFallback = (): Promise<ModelMessage[]> =>
      this.messageProjection.materializeRuntimeReplayTextOnly(
        scope.imageBudget,
        plan,
    ...
    if (!plan.hasProviderNativeSemantics) {
      return {
        status: 'ready',
        messages: await this.messageProjection.materializeRuntimeReplayPlan(
          plan,
          scope.imageBudget,
          projectedHistoryCompactCheckpoint,
          providerReasoningReplayEventIds,
    ...
    if (!this.messageProjection.canReplayProviderNative(plan)) {
      ...
      const degradedPlan = this.messageProjection.dropUnsupportedReplayItems(plan);
      return {
        status: 'ready',
        messages:
          degradedPlan.items.length > 0 || hasProviderHistoryCompactCheckpoint
            ? await this.messageProjection.materializeRuntimeReplayPlan(
                degradedPlan,
                scope.imageBudget,
                projectedHistoryCompactCheckpoint,
                providerReasoningReplayEventIds,
  ...
  private async drainSteeringInto(
    ...
    try {
      for (const lease of leases) {
        ...
        const providerContent = await this.messageProjection.appendImageParts(
          scope.imageBudget,
