...
export class AiSdkTurn {
  ...
  private async *runWithinScope(input: BackendSendInput): AsyncIterable<SessionEvent> {
    ...
    const snapshotStepTools = () => {
      ...
      const nestedTools = buildNestableToolSnapshot(basePlan.providerTools, basePlan.activeTools);
      const {
        availability: plan,
        providerTools,
        modelTools,
      } = buildToolSchemaPlan({
        availability: basePlan,
        toolMode,
        codeModeExecTool,
        nestedTools: toolRuntime.hasSandboxBoundaryDenial()
          ? new Map(
      ...
      return { plan, providerTools, modelTools, nestedTools };
    };
