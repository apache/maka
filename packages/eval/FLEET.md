...
`maka eval run` continues to use the existing single-host runner. There is no network daemon or fleet CLI deployment command yet. `createFleetGroupExecution` now connects the existing SubjectAdapter/ExperimentExecutor lifecycle to the fleet worker through the same `executeExperimentCell` entry point used by `runExperiment`. The library exports `FleetCoordinator`, `FileFleetPersistence`, `FleetWorker`, their protocols, and the pure `transitionFleet` function for the next transport integration.
...
## Shared execution bridge

Call `createFleetGroupExecution({ spec, executor, subjects, preflight, collectArtifacts })` before constructing/registering a worker. It runs the supplied deployment preflight and the same validation/subject preparation as the local runner once for the full spec. This avoids re-preparing shared adapters while other groups execute. Pass the existing Harbor/Pier executor and subject adapters; no second implementation of cell execution is introduced.

The returned group executor validates assigned cells against the prepared experiment, starts pending cells concurrently, and emits each result after the shared execution/finalization path and artifact collection settle. `collectArtifacts` explicitly translates legacy metadata into uploadable bytes; metadata is never assumed to be a filesystem path. Collection failure retains execution and usage evidence but blocks the run until repaired. It does not trigger verification-only recovery.

The integration tests use the existing external subject adapter through its command/exit-code path and mock only the executor's process/verifier boundary. They cover successful and timed-out subjects, preparation/transport/verifier/finalization failures, local/fleet result equivalence, independent cell emission, and a complete worker-to-coordinator commit with durable artifacts. These prove the shared code seam, not real VM admission or Docker/Harbor deployment. Environment verification, real artifact collection, and authenticated remote transport still require deployment wiring.
