  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <div
        ...
        data-conversation-expanded="false"
        style={{ position: 'relative', width: 340, height: props.height ?? 96, border: '1px solid var(--border)', borderRadius: 20, background: 'var(--background)', overflow: 'hidden' }}
      >
        <div className="workHubComposerSurface" style={{ position: 'absolute', inset: 'auto 0 0 0', padding: '0 8px 8px' }}>
          <div className="maka-model-selection-controls">{props.children}</div>
        </div>
      </div>
      <span style={{ font: 'var(--maka-text-supporting)', color: 'var(--muted-foreground)' }}>{props.caption}</span>
    </div>
  );
}
...
const collapsedSession = {
  ...
  status: 'active',
  backend: 'ai-sdk',
  llmConnectionId: COLLAPSED_ACTIVE.connectionId,
  llmConnectionSlug: COLLAPSED_ACTIVE.connectionSlug,
  connectionLocked: false,
  model: COLLAPSED_ACTIVE.model,
  permissionMode: 'ask',
} satisfies SessionSummary;
  ...
  render: () => (
    <CollapsedWindowFrame height={140} caption="收起态：模型仍是滚轮，思考档位用 bottom-sheet">
      <ChatModelSwitcher
        ...
        activeModelLabel={COLLAPSED_ACTIVE.label}
        currentProviderType={COLLAPSED_ACTIVE.providerType}
        choices={MANY_CHOICES}
        openNonce={1}
        renderProviderMark={providerMark}
        onChange={() => undefined}
