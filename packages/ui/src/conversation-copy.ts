...
export interface ConversationCopy {
  ...
  messages: {
    ...
    mailboxIncoming: string;
    mailboxOutgoing: string;
    mailboxDelivered: string;
    mailboxQueued: string;
    mailboxPending: string;
    mailboxIncomingAriaLabel: (sessionName: string) => string;
    mailboxOutgoingAriaLabel: (sessionName: string) => string;
...
const CONVERSATION_COPY = {
  zh: {
    ...
    messages: {
      ...
      mailboxIncoming: '来自任务', mailboxOutgoing: '发送到任务', mailboxDelivered: '已送达 · 已开始处理', mailboxQueued: '已排队 · 将在下一轮处理', mailboxPending: '送达待确认 · 正在恢复', mailboxIncomingAriaLabel: (sessionName) => `来自任务“${sessionName}”的消息`, mailboxOutgoingAriaLabel: (sessionName) => `发送到任务“${sessionName}”的消息`,
