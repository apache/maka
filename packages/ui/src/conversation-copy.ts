...
/** Wall-clock units for the goal chip's elapsed label, customized per locale. */
...
/** Humanize a retry delay (seconds) — 1s granularity, locale-aware. */
...
/** One shared elapsed ladder so the locale-specific goalElapsed entries cannot drift. */
...
const CONVERSATION_COPY = {
  ...
  ko: {
    ...
    questions: { other: 'Other', otherDescription: 'Enter a different answer.', otherAriaLabel: 'Other answer', otherPlaceholder: 'Enter your answer', stop: 'Stop', stopping: 'Stopping…', previous: 'Previous', submitting: 'Submitting…', submit: 'Submit answers', next: 'Next' },
    forms: {
      requester: (name) => `${name}님이 요청함`,
      requesterWithSource: (name, source) => `${name}님이 요청함 · ${source}`,
      required: '필수',
      optional: '선택 사항',
      include: (label) => `${label} 제공`,
      enabled: (label) => `${label} 활성화`,
      enterValue: '값을 입력하세요',
      enterNumber: '숫자를 입력하세요',
      constraintSeparator: ' · ',
      lengthConstraint: (minimum, maximum) => minimum === undefined ? `최대 ${maximum}자` : maximum === undefined ? `최소 ${minimum}자` : `${minimum}~${maximum}자`,
      numberConstraint: (minimum, maximum) => minimum === undefined ? `최대 ${maximum}` : maximum === undefined ? `최소 ${minimum}` : `${minimum}~${maximum}`,
      itemConstraint: (minimum, maximum) => minimum === undefined ? `최대 ${maximum}개 선택` : maximum === undefined ? `최소 ${minimum}개 선택` : `${minimum}~${maximum}개 선택`,
      formatConstraint: { email: '형식: email', uri: '형식: URI', date: '형식: date (YYYY-MM-DD)', 'date-time': '형식: date-time (RFC 3339)' },
      invalid: '요구 사항에 맞는 값을 입력하세요.',
      cancel: '취소',
      decline: '거부',
      accept: '제출',
      submitting: '제출 중…',
    ...
    messages: {
      ...
      systemNotes: {
        contextCompacted: 'Context compacted to keep this session within the model window.',
        contextCompactionFailedOpen: 'Context summary failed; the session continued without a new summary.',
        contextProviderDropping: (used, prior) => `The provider reported ${used} input tokens after adding content, no increase from ${prior}. Declare the model context window in connection settings so Maka can compact first.`,
        contextWindowSuggestion: (tokens, declared) => declared === undefined ? `The provider rejected this request and the model has no declared context window. The last successful usage was about ${tokens} tokens; set the window to that value so Maka can compact first.` : `The provider rejected this request even though usage (${tokens} tokens) was below your declared window (${declared}). Consider lowering the declared window.`,
