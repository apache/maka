...
export interface ConversationCopy {
  ...
  messages: {
    ...
    modelChanged(from: string, to: string): string;
...
const CONVERSATION_COPY = {
  zh: {
    ...
    messages: {
      ...
      userAriaLabel: '你发送的消息', systemAriaLabel: '系统消息', modelChanged: (from, to) => `模型已从 ${from} 更改为 ${to}。`, modelSwitchWarning: '在对话中途切换模型会降低性能表现。', assistantAriaLabel: 'Maka 的回答', answerActionsAriaLabel: '本轮回答操作', sourceAriaLabel: '本轮回答的来源', derivativesAriaLabel: '本轮回答的衍生', scheduledTaskTriggered: '定时任务触发', scheduledTaskTitle: (id) => `由定时任务触发 · ${id}`, goalContinued: 'Goal 自动继续', goalTitle: (id) => `由 Goal 继续执行 · ${id}`, agentGraphTriggered: 'Agent Graph 自动继续', agentGraphTitle: (graphId) => `由 Agent Graph 调度器触发 · ${graphId}`,
  ...
  en: {
    ...
    messages: {
      ...
      userAriaLabel: 'Your message', systemAriaLabel: 'System message', modelChanged: (from, to) => `Model changed from ${from} to ${to}.`, modelSwitchWarning: 'Switching models mid-conversation may reduce performance.', assistantAriaLabel: "Maka's response", answerActionsAriaLabel: 'Response actions', sourceAriaLabel: 'Source of this response', derivativesAriaLabel: 'Responses derived from this one', scheduledTaskTriggered: 'Triggered by scheduled task', scheduledTaskTitle: (id) => `Triggered by scheduled task · ${id}`, goalContinued: 'Continued by Goal', goalTitle: (id) => `Continued by Goal · ${id}`, agentGraphTriggered: 'Continued by Agent Graph', agentGraphTitle: (graphId) => `Triggered by the Agent Graph scheduler · ${graphId}`,
