...
const PLAN_REMINDER_COPY = {
  zh: {
    ...
    page: {
      title: '定时任务', subtitle: '创建和管理周期性任务，让 Maka 按计划执行提醒、复盘和投递。', actionsAriaLabel: '计划提醒操作', refreshing: '正在刷新定时任务', refresh: '刷新定时任务', create: '新建定时任务', keepAwakeHint: '电脑休眠会错过启用中的定时任务；可在页面设置中保持系统唤醒。', keepAwake: '保持系统唤醒', keepAwakeOn: '保持唤醒已开启', pageSettings: '定时任务页面设置', keepAwakeErrorTitle: '无法更新保持系统唤醒', keepAwakeErrorFallback: '更新保持系统唤醒设置失败，请稍后重试。', viewsAriaLabel: '计划提醒视图', tasks: '我的定时任务', runs: '执行记录', filtersAriaLabel: '计划提醒筛选', sort: '排序', sortAriaLabel: '定时任务排序', sortOptions: [['created-desc', '按创建时间倒序'], ['next-run-asc', '按下次触发升序'], ['updated-desc', '按更新时间倒序']], searchLabel: '搜索计划提醒', searchPlaceholder: '搜索标题、备注、投递或执行记录…', state: '状态', filterAriaLabel: '计划提醒筛选', filterOption: (label, count) => `${label} ${count}`, active: '进行中', all: '全部', runsFilterAriaLabel: '执行记录筛选', range: '范围', rangeAriaLabel: '执行记录范围', rangeOptions: [['day', '今天'], ['week', '近 7 天'], ['month', '近 30 天'], ['all', '全部记录']], searchMatches: (count) => `找到 ${count} 个匹配提醒`, clearSearch: '清除搜索', templatesAriaLabel: '定时任务示例模板', noSearchTitle: '没有匹配的提醒', noFilterTitle: '当前筛选没有提醒', noSearchBody: '调整搜索词，或切换状态筛选查看其他提醒。', noFilterBody: '切换筛选查看其他状态，或创建新的计划提醒。', emptyTitle: '还没有定时任务', emptyBody: '创建一个提醒，让 Maka 在指定时间继续这项工作。', listAriaLabel: '计划提醒列表', completed: '已完成', pause: '暂停提醒', enable: '启用提醒', reminderActions: '提醒操作', edit: '编辑', duplicate: '复制', triggering: '触发中…', triggerNow: '立即触发', snoozing: '延后中…', snooze: '延后 10 分钟', clearing: '清空中…', clearRuns: '清空记录', deleting: '删除中…', delete: '删除', lastRun: (status, message) => `${status}：${message}`, nextRun: (time) => `下次触发：${time}`, recentRun: (time) => `最近 ${time}`, unscheduled: '未安排', neverRun: '尚未执行', noRunsTitle: '暂无执行记录', noRunsBody: '提醒触发、手动执行或投递失败后，会在这里保留最近记录。', runsAriaLabel: '计划提醒执行记录',
    },
  },
  en: {
    templates: [
