...
export const TUI_COPY_RESOURCES = {
  ...
  'mcp-status': {
    ...
    ko: {
      ...
      footer: {
        ...
        manage:
          'a 추가 · Enter 편집 · Space 활성화/비활성화 · t 테스트 · r 재연결 · d 삭제 · Esc 닫기',
      },
      unavailableTitle: '이 TUI는 로컬 MCP 제어 평면에 연결되어 있지 않습니다.',
      unavailableDetail:
        '원격 Runtime Host의 클라이언트 MCP 도구 연결은 향후 릴리스에서 제공될 예정입니다.',
  ...
  pickers: {
    ...
    ko: {
      ...
      modelSwitchCacheWarning:
        '⚠ 모델을 전환하면 프롬프트 캐시가 재구축될 수 있어 다음 요청이 더 느리거나 비용이 더 들 수 있습니다.',
      ...
      apiKeyLabel: 'API 키',
      onboardingUnavailable:
        '온보딩을 사용할 수 없습니다. 이 환경에서는 설정 화면을 제공하지 않습니다.',
      ...
      accountSavedRefreshFailed:
        '계정은 저장되었지만 모델 목록이 새로고침되지 않았습니다. Maka를 다시 시작하면 다시 불러옵니다.',
      ...
      reuseBaseUrlHint:
        '비워두면 저장된 Base URL을 재사용하거나, 새 주소를 입력해 교체하세요 · Esc로 프로바이더 선택으로 돌아가기',
      ...
      keyHints: {
        reuse: {
          baseUrl:
            '비워두면 저장된 키를 재사용하거나, 새 키를 입력해 교체하세요 · Esc로 Base URL로 돌아가기',
          provider:
            '비워두면 저장된 키를 재사용하거나, 새 키를 입력해 교체하세요 · Esc로 프로바이더 선택으로 돌아가기',
        },
        enter: {
