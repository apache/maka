...
export class AcpSessionInteractions {
  ...
  readonly #cancelledInteractions = new Set<string>();
  ...
  pending(snapshot: InteractionPendingSnapshot): Promise<void> {
    ...
    if (
      this.#cancelledTurns.has(snapshot.turnId) ||
  ...
  cancelTurn(turnId: string): void {
    ...
    for (const entry of this.#pending.values()) {
      if (entry.snapshot.turnId !== turnId) continue;
      this.#cancelledInteractions.add(entry.snapshot.interactionId);
      this.#retire(entry.snapshot.interactionId);
    }
  }

  /** Retires local presentation work after a Turn settles without cancelling another Turn. */
  ...
  #retireCancelledInteraction(pending: InteractionPendingSnapshot): void {
