...
export interface MakaPiTranscriptState {
  ...
  followup: string[];
  /**
   * Messages whose enqueue hit the no-session `fallback` outcome during the
   * first Session's admission window (the turn is running but `session.create`
   * has not yet assigned a session id). CLI-owned, NOT a runtime mirror: the
   * runner holds them durably and re-enqueues at the turn boundary, so the
   * text is never dropped. Rendered in the pending bar alongside the mirror.
   */
  pendingAdmission: Array<{ text: string; enqueue: 'steer' | 'queue' }>;
...
export function renderMakaPiPendingQueue(state: MakaPiTranscriptState, width: number): string[] {
  if (
    ...
    state.pendingAdmission.length === 0
  ) {
    return [];
  }
  const safeWidth = Math.max(1, width);
