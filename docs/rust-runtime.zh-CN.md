...
模型配置中的 `contextWindow` 是**输入与输出共享的总容量**。原生 AI 执行要求
提供商目录或模型配置明确给出它，单独的 `inputLimit` 不能替代。缺失时仍可通过
目录和设置修复；ACP executor 与确定性的 Code 执行不受此要求限制。

```text
自动阈值 = floor(85% × min(contextWindow, inputLimit（若已知）))
最终阈值 = 显式 compactionThreshold，否则使用自动阈值
```

例如，**总窗口 1,000,000 token**，默认在 **850,000 token** 触发，与最大输出
设置为 128,000 还是 64,000 无关。若独立输入上限为 800,000，默认阈值为 680,000。
唯一可选覆盖项 `compactionThreshold` 仍是绝对 token 数。

主请求前检查上一匹配主请求的实际用量：正数输入用量本身达到阈值即可触发；否则
只有输出用量也已知时才比较输入与输出之和。未知计数不按零处理。用量须匹配模型、
连接、route、checkpoint 和实际历史投影。

每次请求都有有限输出预算：优先使用解析后的请求预算，其次使用已知的提供商输出
上限，两者都未知才使用 8,000 token。预算受提供商上限约束。存在完整匹配用量 `R`
和总窗口 `C` 时，进一步按 `C − R − 8000 − 固定 thinking 预算` 收紧输出，同时
保留 `min(已选预算, 8000)` 的可用下限。Anthropic 文本与 thinking 预算只转换一次。
没有完整用量依据时（包括刚压缩后）保留已选预算。摘要文本预算取已选预算与
8,000 的较小值。这些限制只作用于请求，不改写模型容量配置。

压缩捕获已结算前缀，并冻结模型、adapter 和源，供有限次数的摘要修复复用。
模型 executor 有空闲额度时，摘要与主请求并行；否则同步执行同一个压缩器。
候选只在新主请求前采用，保留精确前缀以及之后追加的全部消息和工具结果。
候选存在时延后 pruning。取消、完成和 handoff 都先取消并等待摘要收尾，再封口
invocation；只有新的已接受主请求进展才能更新自动尝试预算。

用量观测和增长余量都是启发式保护，**不能证明下一次完整提示词一定放得下**。
新文本、工具或媒体仍可能溢出。只有提供商在可观察输出前明确拒绝上下文溢出，才
触发有界自动压缩和重试；摘要自身溢出直接有界失败，不递归压缩。展示估算不参与准入。

有效且完整闭合的长度截断响应会保留内容，但 Turn 以 `model_incomplete` 结束；
其中本地工具调用会持久拒绝，绝不派发。残缺调用和未结算的 provider 效果仍严格
失败。不自动续写；显式 resume 保留从稳定效果边界重新生成的既有语义，新用户消息
可以引用已经保存的部分文本。

内部请求证据区分 `ModelRequestContext.context_window`（有效输入上限）、
`model_context_window`（总容量）和 `declared_window`（最终触发阈值）；历史缺失
字段仍视为未知。实现见 [Host 解析](../crates/runtime-host/src/execution/provider/context.rs)、
[用量和输出检查](../crates/agent/src/auto_context.rs)与
[压缩生命周期](../crates/agent/src/steps.rs)。
