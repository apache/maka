...
这份记录仅描述 59565b1a 的历史验收。后续架构复盘确认：可选 routing adapter 未默认启用是 #5152 的既定设计，不是 upstream 遗漏。目标选择应复用 Host 交互生命周期并与路由实验解耦；不能通过直接开启实验来补齐此测试。新的实现契约见 WorkHub domain language。
