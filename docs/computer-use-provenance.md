...
The pinned `objc2` license describes its generated Apple framework bindings as SDK-derived and raises a question about redistributing those crates as source. That statement does not identify Apple SDK files in this binary distribution. [Apple's Xcode agreement](https://www.apple.com/legal/sla/docs/xcode.pdf) permits distribution of compliant macOS applications and libraries while prohibiting redistribution of the SDK itself; [ASF policy](https://www.apache.org/legal/resolved.html#does-it-matter-what-platform-an-apache-product-is-created-to-work-with) likewise permits platform dependencies without redistributing platform code. The packaged Cua subtree contains the executable and third-party notices, and `otool -L` shows its Apple framework dependencies at system paths. The checked-in `objc2` upstream license text and the full MIT permission text are present in the packaged notices. This review found no concrete Apple SDK redistribution or separate ASF licensing blocker from `objc2`; it is not a source-to-binary attestation.
