...
This audit does not authorize release yet. The published binary is pinned by upstream archive and executable SHA-256 and signed by Cua AI Inc, but there is no reproducible-build proof or upstream SBOM establishing the exact linked set. Thirty-five external crates in the conservative graph have no root-level license file in their published crate directory: 24 from `madsmtm/objc2`, eight from `mozilla/uniffi-rs`, two from `Nugine/simd`, and one from `Stranger6667/jsonschema`. Their declared metadata and upstream license/copyright locations still need a per-crate attribution review. Before setting `distributionReady` to `true`, finish and package the remaining applicable license and copyright notices, resolve the exact build provenance with upstream or an equivalent verifiable build, and review the resulting binary package against ASF policy. The release gate remains false until then.
