...
The pinned `objc2` license describes its generated Apple framework bindings as SDK-derived and raises a question about distributing those crates as source. Maka distributes the compiled Cua CLI, not the binding source or Apple SDK files. [Apple's Xcode agreement](https://www.apple.com/legal/sla/docs/xcode.pdf) §2.4 permits distribution of compliant macOS applications and libraries, while §2.7 prohibits redistribution of Apple Software. The [ASF platform FAQ](https://www.apache.org/legal/resolved.html#platform) allows projects to target a platform unless its terms change the Apache product's licensing, but does not permit redistribution of platform code. The packaged Cua files contain the executable and third-party notices; inspection found no Apple SDK headers or libraries in that payload. `otool -L` shows only that its Apple framework dependencies resolve to system paths, not what code was statically compiled into the CLI. The upstream `objc2` license text and MIT permission text are present in the notices; the newer upstream license file has no copyright line to reproduce. This review found no concrete Apple SDK redistribution or separate ASF licensing blocker from `objc2`, but it is not a source-to-binary attestation.

The published binary is pinned by upstream archive and executable SHA-256 and signed by Cua AI Inc, but no upstream SBOM or reproducible-build proof establishes its exact linked set. This remains an evidence limit, not an ASF requirement by itself. The signed and notarized app still needs real-device qualification, so `distributionReady` remains false.
