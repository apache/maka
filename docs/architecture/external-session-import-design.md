...
Codex keeps no server-side catalog snapshot, SQLite transaction, TTL, or LRU across requests. Cursors are bound to the current query. State DB pages order by `(normalized sort_ts DESC, id DESC)`: the first page selects the newest readable `state_N.sqlite`, and continuation names that generation and uses a SQL keyset condition. Connections close after each request. A missing original generation invalidates the cursor; a transient read failure remains a persistence failure. Second and millisecond timestamps are normalized before ordering and display.
...
- Adapter tests cover source filtering, full-or-reject behavior, resource ceilings, cursor position, and both Codex catalog paths. Host and Storage tests cover wire truncation, typed errors, import counts, in-flight coalescing, staging, publication, and recovery.
