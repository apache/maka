...
## Returning delegated results to Coordination

New assignments opt in with `returnResults: true`. A Host coordinator reconciles
those active links against the target Message's owning or shared Turn and its
latest physical Run. Terminal outcomes and pending user interactions can wake the
Coordination model without another user message. A shared Turn is explicitly
identified: its answer is evidence for the delegated request, not proof that only
that request was executed.

Delivery is an ordinary `workhub_coordination` root admission with a
`workhub_result` origin. Its deterministic Turn identity binds the delegation,
physical Run and terminal event or pending interaction set. That admission is the
durable delivery receipt; there is no separate inbox database. Busy Coordination
waits for an idle admission. Host startup reconciles after domain recovery, and
periodic reconciliation also discovers provider reconnection. Delivery requires
the authenticated capability binding from Coordination's preceding admission.
An unavailable Desktop provider delays delivery until it reconnects.

Immediately before admission, the Host rechecks both Sessions under their
admission leases. Archived/deleted targets, retired links, pending stop or
replacement intent, and interactions that have already changed cannot produce a
late wake. A failed target read is isolated so other results can still return.
Legacy assignments without the opt-in flag do not replay historical results.

After successful delegation, the model acknowledges admission and yields instead
of polling tools. An early result read returns `pending`; the Host notification
starts the next turn when evidence is available. Tool results use lossless JSON
values, including explicit nulls for absent outcome details.

The model receives the original request, delegation identity, terminal status and
a bounded answer from the target Turn. The result is task data, not fresh user
permission. Action admission retains the original user request and attachments.
`WorkHubResult.read` reads further Unicode-safe result pages. The model can
summarize, investigate, continue authorized work or await other results. A
terminal execution alone does not establish that the user's objective is met.

`WorkHubResult.ask_question` presents the target's exact pending question through
WorkHub's ordinary human interaction. Only the answer actually collected there
is forwarded to the original interaction authority; the model cannot supply an
answer argument. Delegation and target identity are rechecked after the human
responds. Stop/replacement during that wait makes the answer obsolete. Permissions
and non-question interactions remain in the target's original interface. Existing
Host recovery closes orphaned human continuations; this does not add replayable
in-memory question promises.

The initial delivery surface covers terminal results and requests for input,
not streaming progress. The UI labels these turns as task result updates rather
than user messages, while preserving the existing target result projection.
