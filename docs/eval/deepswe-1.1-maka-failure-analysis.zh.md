...
| P2-1 | 重读短路 + cwd 规范化 | 潜在 10–20% 步数 | M | 中 | 工作区执行器 |
...
1. **按 `(路径, 行范围)` 重算重复读** —— 用于区分真重复与 `sed` 窗口滚动，决定「重读短路」改进的真实收益。已写脚本但 `trajectory.json` 的 schema 与假设不符（工具调用字段名未命中），提取到 0 条记录，需先摸清结构。当前 3,793 次重读的数字来自另一路独立提取，口径为「仅按路径去重」，**已知会高估**。
