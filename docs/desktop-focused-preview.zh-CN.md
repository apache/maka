...
日期：2026-09-20（评审迭代）。关联 issue：[apache/maka#5469](https://github.com/apache/maka/issues/5469)；工作区 Markdown 引用入口另见 [#2664](https://github.com/apache/maka/issues/2664)。
...
6. 聚焦时输入区上方常驻共享进度卡片：运行中显示处理状态/可用时的分秒时长，展开后只读地跟随最近一次 turn 的文字与工具活动；结束后显示“本轮已结束”，展开显示该 turn 最后已落盘的助手回复。收起保留入口与草稿，展开内容独立滚动；用户向上阅读时不强制追尾。文件和网页共用这一层。
7. 四态必须保持截图中的同一锚点与宽度：运行中收起只保留输入卡片上沿的紧凑状态行和时长；运行中展开为盖在文件内容之上的宽幅、有高度上限的对话流；完成后收起只显示“本轮已结束”；完成后展开显示多段最终回复。上层与真实输入卡片边缘贴合，带附件时附件抽屉也是同一连续表面，不做间隔卡片；聚焦时仍使用真实 Composer 的附件、模型、权限与停止/发送控件。
...
- 完整原生体验与逐步截图见 [交互验收图解](images/pr/desktop-focused-preview/live-review/README.md)。拖动聚焦后回到原编辑器，临界位置显示松手提示；输入框的 Escape 在局部弹层处理后退出聚焦，停止保留明确按钮；Browser 的 Escape 先撤销未提交地址，在工具栏上则还原分栏。
- 中断回合没有最终落盘助手消息时，保留已有的有界 live projection 并标记“已暂停”。只读观察器跟随当前 Files/Browser 面板生命周期，分栏时不渲染回复 DOM，切回聚焦仍可看到本次收到的部分内容；不另建持久化消息缓存。
...
| 按还原，或在输入框/文件预览/浏览器工具栏上按 Esc | 聚焦 | 分栏 | 当前文件、源码/预览切换状态、会话滚动 |
...
## 2026-09-20 评审修正与真实验收

- 通过本地 Codex 连接，在真实 Electron 应用中选择 `gpt-5.6-terra` / `medium`。HTML 文件由模型通过 Write 实际生成，长回复与工具调用均来自真实提供方。截图、过程及构建边界见 [本轮验收](images/pr/desktop-focused-preview/live-review/README.md)。
- 已完成回复首次从头打开；只有读者位于末尾时追随实时更新。回复折叠、输入区最小化与恢复保持阅读位置。每次高度测量不再清除聚焦布局属性，避免在恢复时短暂按分栏宽度重排。
- 16,000 字符或最近 12 项的显示上限被触发时显示说明。展开区始终提供“查看完整对话”，调用现有会话导航并传入当前 turn ID。
- 卡片使用 `min(960px, 容器可用宽度)`，移除 990px 视口分支；Git 分支名允许缩短，给发送/停止按钮留出空间。真实窗口已检查 990、991、720 与 1240px。
- 进度状态词汇由共享 UI 提供：处理中、已暂停、需要查看、已结束。WorkHub 的隐藏动作与预览输入区的最小化动作继续由各自宿主管理。
- 消融：删掉不再使用的摘要/状态副本及额外的初始化标志；尝试保留隐藏 Markdown 树并长期显示终止后的 live 内容，定位到真正的布局属性清理问题后移除了这两项，真实折叠/最小化恢复仍保持同一段落。
