...
[ASF Category X policy](https://www.apache.org/legal/resolved.html#prohibited) allows external build tools when they do not affect the product's licensing, but prohibits distributing their source or binaries. The source verifier permits only these reviewed sharp/libvips license expressions, requires external development lock entries, and checks that their consumers lead only to the website's Astro development dependency. Another workspace consumer, bundled dependency, generated distribution notice, or different license closes the exception. This is not a general exemption for development dependencies; Desktop renderer dependencies can be development dependencies and still ship. Existing archive payload checks and convenience-artifact attribution checks remain applicable.
