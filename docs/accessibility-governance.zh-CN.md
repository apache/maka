...
还有五个可访问名称在汇报元素自身的状态。它们在 `packages/ui/src/conversation-copy.ts`
里各带一条 `a11y-allow` 而不是被静默放过，因为那些元素上确实没有别处暴露这个值 ——
直接从名称里删掉是丢信息，不是搬信息。正解是先把值暴露出来，再去掉插值和那条 allow。

| 标签 | 元素 | 应该变成 |
|---|---|---|
| `chooseAriaLabel` | 输入区的项目选择器 | 名称 `选择项目`，项目与分支作为控件的值 |
| `branchAriaLabel` | 分支切换器 | 名称 `切换分支`，分支作为值 |
| `currentAriaLabel` | 输入区旁的模型 chip | 它是状态展示不是控件 —— 给它一个带值的 role |
| `newChatAriaLabel` | 新对话的模型选择器 | 照输入区那个切换器，它已经是对的 |
| `configureAriaLabel` | 模型连接入口 | 同上 |
