...
## WorkHub acceptance (updated 2026-09-23)

The initial negative result exposed the coordination-role exclusion. After rebasing onto `c7d205a42`, the permanent WorkHub identity is explicitly eligible. `workhub-check.mjs` now verifies a generated suggestion through the real native surface and Host request, Tab without send, native Undo, Enter in the durable coordination transcript, Esc dismissal and delayed-result preservation. See `workhub-check.json` and the [Storybook evidence](../prompt-suggestion-storybook/README.md). Background child sessions remain excluded.
