...
try {
  ...
  await input.waitFor();
  console.log('session', sessionId, 'initial', (await page.locator('body').innerText()).slice(-2500));
  await page.locator('.maka-composer-plus-menu button').first().click();
  await page.getByRole('menuitemcheckbox', { name: /下一步输入建议|Next prompt suggestions/ }).click();
  ...
  async function textMetrics(selector) {
    return page.locator(selector).first().evaluate((el) => {
      ...
      return {
        rects: [...range.getClientRects()].map((r) => ({ x: r.x, y: r.y, width: r.width, height: r.height })),
        font: style.font, letterSpacing: style.letterSpacing, whiteSpace: style.whiteSpace,
  ...
  if (multiline) assert.ok(suggestionMetrics.rects.length > 1, 'long suggestion must actually wrap');
  const acceptedMetrics = await textMetrics('.maka-composer-editor [contenteditable="true"]');
  assert.deepEqual(suggestionMetrics, acceptedMetrics, 'suggested and accepted glyph positions and typography must match');
  await writeFile(join(output, 'text-alignment.json'), JSON.stringify({ suggestionMetrics, acceptedMetrics }, null, 2));
  ...
  const usage = await page.evaluate(async (id) => await window.maka.sessions.generatePromptSuggestion(id), sessionId);
  assert.equal(usage.kind, 'generated');
  assert.equal(requests.filter((r) => r.suggestion).length, 2, 'repeat IPC request must reuse result');
  // Accept and submit through the original send path, then confirm durable transcript.
  await input.fill('第三轮：确认接口'); await input.press('Enter');
  await offer.waitFor({ timeout: 20000 }); await input.press('Tab'); await input.press('Enter');
  await offer.waitFor({ timeout: 20000 });
  ...
  for (const [width, height, zoom] of [[900, 650, 1.25], [780, 600, 0.9], [1180, 800, 1]]) {
    await app.evaluate(({ BrowserWindow }, size) => {
      const w = BrowserWindow.getAllWindows()[0]; w.setSize(size[0], size[1]); w.webContents.setZoomFactor(size[2]);
    }, [width, height, zoom]);
    const bounds = await offer.boundingBox(); assert.ok(bounds && bounds.width > 100);
    assert.ok(await offer.isVisible());
  }
  await page.screenshot({ path: join(output, '03-after-send.png') });
  await input.fill('自行输入会隐藏建议'); assert.equal(await offer.count(), 0);
  ...
  await input.fill('第五轮：生成一个延迟建议'); await input.press('Enter');
  ...
  await input.fill('关闭建议之后的正常对话'); await input.press('Enter');
  ...
  assert.equal(await offer.count(), 0);
  assert.equal(errors.length, 0, JSON.stringify(errors));
  await writeFile(join(output, 'result.json'), JSON.stringify({ ok: true, profile, sessionId, requests, errors, checks: ['empty ghost text', 'Tab accepts without send', 'native undo', 'Esc dismiss', 'deduplicated IPC', 'accepted Enter reaches durable transcript', 'three viewport/zoom combinations', 'typing hides suggestion without resurrection', 'late response preserves new draft', 'disabled makes no prediction request'] }, null, 2));
