        ...
        for (const variant of [0, 1, 2, 3]) {
          const text =
            variant === 0
              ? 'Implement '
              : variant === 1
                ? '这是一个中文输入框边界复现实验。'.repeat(12)
                : variant === 2
                  ? 'Line number test content\n'.repeat(9) + 'And next '
                  : 'Code '.repeat(70);
          const suffix =
            variant === 0
              ? 'the next step.'
              : variant === 1
                ? '接着验证补全是否会导致界面反复更新。'
                : variant === 2
                  ? 'continue with more implementation and tests.'
                  : ' more words to wrap across the bottom edge of this capped field.';
          configs.push({
            font,
            width,
            lineHeight,
