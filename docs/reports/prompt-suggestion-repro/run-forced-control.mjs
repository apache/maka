...
await p.evaluate(() => {
  ...
  Element.prototype.getBoundingClientRect = function () {
    ...
    const result = new DOMRect(
      rect.x,
      visible ? field.top + 1 : field.bottom + 20,
      rect.width,
      visible ? 1 : rect.height,
    );
    window.forcedTrace.push({
      n,
      visible,
      realBottom: rect.bottom,
      forcedBottom: result.bottom,
...
await p
  .waitForFunction(() => window.forcedCalls >= 300 || window.errors.length > 0, undefined, {
    timeout: 5000,
  })
  .catch(() => {});
const result = await p.evaluate(() => ({
  synthetic: true,
  calls: window.forcedCalls,
  trace: window.forcedTrace,
  ...window.metrics(),
}));
