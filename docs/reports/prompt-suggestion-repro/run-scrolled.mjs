...
const configs = Array.from({ length: 240 }, (_, i) => ({
  width: 200 + Math.floor(rand() * 600) + [0, 0.25, 0.5, 0.75][i % 4],
  fontSize: [12, 13, 14, 15, 16][i % 5],
  lineHeight: 17 + Math.round(rand() * 28) / 4,
  zoom: [0.75, 0.8, 0.9, 1, 1.1, 1.25, 1.5][i % 7],
  rows: [2, 3, 10][i % 3],
  font: ['Arial', 'monospace', '"PingFang SC"'][i % 3],
  text: (i % 2 ? '请检查这个边界场景。' : 'Check the boundary case. ').repeat(
...
const dir =
  '/Users/a404/.codex/worktrees/maka-prompt-suggestions/maka-agent/docs/reports/prompt-suggestion-repro';
await fs.writeFile(`${dir}/scrolled-${batch}.json`, JSON.stringify(results, null, 2));
console.log({
  batch,
  cases: results.length,
  errors: results.filter((x) => x.errors.length),
  shown: results.filter((x) => x.offerText).length,
  maxMutations: Math.max(...results.map((x) => x.mutations)),
});
