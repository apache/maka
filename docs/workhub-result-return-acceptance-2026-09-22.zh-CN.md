...
[观看 WorkHub 真实桌面回包录屏](images/pr/workhub-result-return/return-demo.mp4)。视频时长 19.88 秒，由 Playwright 直接录制实际 Electron WorkHub 窗口；未用静态组件渲染代替运行界面。测试使用独立 profile、现有 DeepSeek V4.1 Flash 连接和真实模型响应，`MAKA_E2E` 未启用。此轮已使用按目标 Turn 读取结果的修复代码。
...
在独立 profile 中创建两个已有任务 A、B，分别委派读取各自 `input.json`，等待各自工作目录中的 `GO` 文件后写 `result.json`。两次 WorkHub 派发确认后，同时写入两个 `GO` 文件；放行前尚无结果回包。真实 DeepSeek V4.1 Flash 任务写出 A 的 `{"sum":6}` 和 B 的 `{"sum":9}`，独立读取文件确认结果。

WorkHub 最终生成两条独立的“任务结果更新”，分别对应 A、B 的持久结果 Turn `whf_4fd01767f51434a1c9546567a49224ba7a08d4f6dc676c25` 和 `whf_e21c0572d0ddc742eb6638c295fd1f0f0d33f077d2727402`，都已完成。第一条汇报 A 并说明 B 尚在运行；第二条汇报 B，同时由模型结合上下文汇总 A、B。放行后 A 的文件约 2.1 秒写出、结果通知约 3.4 秒开始；B 的文件约 49.0 秒写出、结果通知约 50.4 秒开始。故此轮真实模型测试证明多任务回包不丢失，但并非两个结果同时就绪。协调器另有测试将两个结果同时设为就绪、WorkHub 一次只能接纳一个，验证两个独立通知依次送达且不重复；系统层并不把它们合成一个 Turn。
