...
The npm schedule starts at 18:17 UTC. Before changing the npm tag, the workflow requires its run number to exceed the current `nightly` version. Desktop assembles and verifies a draft before one publish mutation; a packaging, attestation, tag, upload, or digest failure leaves no partially published GitHub Release. Never rerun a failed workflow attempt in place; dispatch a fresh npm Nightly with a newer version. A `GITHUB_TOKEN` push is also rejected whenever `.github/workflows/` changed on `main` after the npm Nightly source commit, because that token can never hold `workflows` write and so cannot create a ref whose workflow files differ from the default branch; the run then stops at the tag step and that version has no tag and no prerelease. Dispatch a fresh npm Nightly as above instead of issuing a credential that may edit CI, and do not backfill the skipped version.
