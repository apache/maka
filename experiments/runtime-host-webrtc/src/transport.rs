...
pub struct WebRtcTransport {
    outbound: Arc<Mutex<HashMap<PeerId, WebRtcConnection>>>,
    incoming: mpsc::UnboundedReceiver<TransportOutput>,
...
pub struct WebRtcTransportControl {
    outbound: Arc<Mutex<HashMap<PeerId, WebRtcConnection>>>,
    incoming: mpsc::UnboundedSender<TransportOutput>,
}

impl WebRtcTransport {
    pub fn new() -> (Self, WebRtcTransportControl) {
        let outbound = Arc::new(Mutex::new(HashMap::new()));
        let (incoming_sender, incoming) = mpsc::unbounded();
        (
            Self {
...
impl WebRtcTransportControl {
    pub fn register_outbound(
        ...
        let mut outbound = self
            .outbound
            .lock()
            .map_err(|_| io::Error::other("WebRTC transport state was poisoned"))?;
        if outbound.insert(peer, connection).is_some() {
            return Err(io::Error::new(
                io::ErrorKind::AlreadyExists,
                "a WebRTC connection is already ready for this peer",
            ));
        }
    ...
    pub fn inject_inbound(
        ...
        self.incoming
            .unbounded_send((peer, connection))
            .map_err(|_| io::Error::new(io::ErrorKind::BrokenPipe, "WebRTC transport stopped"))
    }
}
...
impl Transport for WebRtcTransport {
    ...
    fn dial(
        ...
        let Some(peer) = parse_webrtc_peer_address(&address) else {
            return Err(TransportError::MultiaddrNotSupported(address));
        };
        let connection = self
            .outbound
            .lock()
            .map_err(|_| {
                TransportError::Other(io::Error::other("WebRTC transport state was poisoned"))
            })?
            .remove(&peer)
            .ok_or_else(|| {
                TransportError::Other(io::Error::new(
                    io::ErrorKind::NotConnected,
                    "WebRTC connection is not ready",
                ))
            })?;
...
fn parse_webrtc_peer_address(address: &Multiaddr) -> Option<PeerId> {
    let mut protocols = address.iter();
    if !matches!(protocols.next(), Some(Protocol::WebRTC)) {
        return None;
    }
    let Some(Protocol::P2p(peer)) = protocols.next() else {
        return None;
    };
    protocols.next().is_none().then_some(peer)
}
...
mod tests {
    ...
    fn private_webrtc_address_is_closed_and_unambiguous() {
        ...
        assert_eq!(
            parse_webrtc_peer_address(&webrtc_peer_address(peer)),
            Some(peer)
        );
        assert!(parse_webrtc_peer_address(&Multiaddr::empty()).is_none());
    }
}
