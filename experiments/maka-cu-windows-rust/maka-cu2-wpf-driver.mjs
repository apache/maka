...
function rpc(exe, args) {
  ...
  child.once('exit', (code, signal) => {
    closed = true;
    for (const waiter of pending.values()) waiter.reject(new Error(`child exited code=${code} signal=${signal ?? 'none'}`));
    pending.clear();
  });
  function call(method, params = {}, timeoutMs = 20_000) {
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => { if (pending.delete(id)) reject(new Error(`timeout waiting for ${method}`)); }, timeoutMs);
      pending.set(id, {
...
function fixtureProcess(exe) {
  ...
  return { child, ready, shutdown: () => { child.stdin.write('shutdown\n'); child.stdin.end(); } };
}
...
try {
  ...
  const hello = await helper.call('host.hello', {
    protocol: 'maka.cu/2', host: { name: 'maka-cu2-wpf-driver', version: 'test' },
  ...
  const target = listed.find((window) => window.pid === identity.pid && window.windowId === identity.hwnd);
  ...
  async function dispatch(snapshot, element, action, name) {
    if (!element) { record(name, 'blocked', 'semantic element not exposed by UIA'); return null; }
    const response = await helper.call('dispatch.element', {
      session, snapshotId: snapshot.snapshotId, toolCallId: `wpf-${name}`,
      elementToken: element.token, expectElementDigest: element.digest,
      strictness: 'element', occlusionPolicy: 'same_app', action,
      observeAfter: { includeImage: false, settle: 'quiesce' },
    });
    const result = response.result;
    if (result?.outcome === 'ok' && result.effect === 'confirmed') record(name, 'pass');
    else if (result?.outcome === 'unknown' || result?.error?.code === 'outcome_unknown') record(name, 'unknown', result?.error?.message ?? 'helper outcome unknown');
    else if (result?.error?.code === 'element_not_actionable' || result?.error?.code === 'unsupported_action') record(name, 'blocked', result.error.code);
  ...
  snapshot = await dispatch(snapshot, find(snapshot, 'wpf-input'), { kind: 'set_value', value: 'maka-wpf-value' }, 'semantic set_value');
  snapshot = await observe();
  const inputAfter = find(snapshot, 'wpf-input');
  record('set_value readback is visible', inputAfter?.value === 'maka-wpf-value' ? 'pass' : 'fail', inputAfter?.value ?? 'missing');
  snapshot = await dispatch(snapshot, inputAfter, { kind: 'select_text', text: 'wpf' }, 'semantic select_text');

  snapshot = await dispatch(snapshot, find(snapshot, 'wpf-button'), { kind: 'click', button: 'left', count: 1 }, 'semantic button click');
  snapshot = await observe();
  record('button oracle reports clicked', find(snapshot, 'wpf-status')?.title === 'clicked' ? 'pass' : 'fail', find(snapshot, 'wpf-status')?.title ?? 'missing');

  const alpha = (snapshot?.elements ?? []).find((element) => element.title === 'Alpha');
  snapshot = await dispatch(snapshot, alpha, { kind: 'click', button: 'left', count: 1 }, 'semantic list selection');
  snapshot = await observe();
  record('list selection oracle reports Alpha', find(snapshot, 'wpf-status')?.title === 'selected:Alpha' ? 'pass' : 'fail', find(snapshot, 'wpf-status')?.title ?? 'missing');

  snapshot = await dispatch(snapshot, find(snapshot, 'wpf-button'), { kind: 'secondary_action', action: 'press' }, 'secondary_action press');
  snapshot = await observe();
  record('secondary_action oracle reports clicked', find(snapshot, 'wpf-status')?.title === 'clicked' ? 'pass' : 'fail', find(snapshot, 'wpf-status')?.title ?? 'missing');

  snapshot = await dispatch(snapshot, find(snapshot, 'wpf-toggle'), { kind: 'click', button: 'left', count: 1 }, 'semantic toggle click');
  snapshot = await observe();
  record('toggle oracle reports on', find(snapshot, 'wpf-status')?.title === 'toggled:on' ? 'pass' : 'fail', find(snapshot, 'wpf-status')?.title ?? 'missing');

  const scroll = find(snapshot, 'wpf-scroll');
  await dispatch(snapshot, scroll, { kind: 'scroll', direction: 'down', pages: 1 }, 'semantic scroll down');

  snapshot = await observe();
  const enter = await helper.call('dispatch.key', {
    session, snapshotId: snapshot.snapshotId, toolCallId: 'wpf-enter',
    focusToken: snapshot.focusedElementToken ?? find(snapshot, 'wpf-input')?.token,
    expectElementDigest: (snapshot.elements ?? []).find((element) => element.token === snapshot.focusedElementToken)?.digest ?? find(snapshot, 'wpf-input')?.digest,
  ...
  record('Enter helper remains honest unknown', enter.result?.outcome === 'unknown' ? 'unknown' : 'fail', 'page oracle is not rewritten as helper verified');
...
const result = {
  ...
  executor: 'rust-native-windows', protocol: 'maka.cu/2',
  target: { pid: identity?.pid ?? null, hwnd: identity?.hwnd ?? null, fixture: fixtureExe },
  summary: { pass: tests.filter((test) => test.status === 'pass').length, fail: failures, blocked, unknown },
  tests,
};
