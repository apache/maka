...
if (!Number.isInteger(foregroundWaitMs) || foregroundWaitMs < 0 || foregroundWaitMs > 30000)
  throw new Error('invalid foreground-wait-ms (0..30000)');
...
const wakeProbePath = resolve(
  wakeProbeIndex >= 0
    ? argv[wakeProbeIndex + 1]
    : 'experiments/maka-cu-windows/out/handoff-wake/UiaWakeProbe.exe',
);
if (wakeProbeIndex >= 0) argv.splice(wakeProbeIndex, 2);
const settleIndex = argv.indexOf('--settle-ms');
const settleMs =
  settleIndex >= 0 ? Math.min(5000, Math.max(1, Number(argv[settleIndex + 1]) || 0)) : 0;
...
if (
  ...
  throw new Error('invalid pre-uia-ms or repetitions');
const [firstHelper, secondHelper] = argv;
if (!firstHelper || (!oneHelper && !secondHelper)) {
  console.error(
    'usage: node browser-task-harness.mjs <helper-1.exe> [helper-2.exe] [--out result.json] [--one-helper] [--observe-only] [--modes name] [--tasks name,name]',
  );
  process.exit(2);
}
...
const ALL_MODES = [
  ...
  {
    name: 'force-renderer-accessibility-complete',
...
const ACTION_TASK_NAMES = [
  'chromium_set_text_and_readback',
  'chromium_scroll_capability',
  'chromium_semantic_click_and_status_readback',
  'chromium_compat_type_text_authorized',
  'chromium_compat_press_enter_authorized_navigation',
];
const KNOWN_TASK_NAMES = ['chromium_observe_page_controls', ...ACTION_TASK_NAMES];
const TASK_NAMES =
  selectedTaskNames ?? (observeOnly ? ['chromium_observe_page_controls'] : KNOWN_TASK_NAMES);
...
const dependencyClosure = (absolute) => {
  const files = readdirSync(resolve(absolute, '..'), { withFileTypes: true })
    .filter(
      (entry) =>
        entry.isFile() &&
  ...
  const details = files.map((file) => {
    ...
    return { path: file, sizeBytes: stats.size, sha256: hash(file) };
  });
  const closureSha256 = createHash('sha256')
    .update(details.map((item) => `${item.path}\0${item.sizeBytes}\0${item.sha256}\n`).join(''))
    .digest('hex')
    .toUpperCase();
  return {
    files: details,
    totalSizeBytes: details.reduce((total, item) => total + item.sizeBytes, 0),
    closureSha256,
  };
};
const artifact = (path) => {
  ...
  return {
    ...
    dependencyClosure: dependencyClosure(absolute),
  };
};
const inputFingerprints = [
  'browser-task-harness.mjs',
  'oracle-state.mjs',
  'fixture/web-task-fixture.html',
].map((file) => {
...
function processTree(rootPid, profile) {
  try {
    ...
    const ids = new Set(list.some((row) => Number(row.ProcessId) === root) ? [root] : []);
    if (profile) {
    ...
    return { ids: [], rows: [], queryFailed: true };
  }
}
...
function runWakeProbe(hwnd, waitMs) {
  if (!existsSync(wakeProbePath))
    return { ok: false, error: 'wake_probe_missing', path: wakeProbePath };
  try {
    const raw = execFileSync(wakeProbePath, [String(hwnd), String(waitMs)], {
      encoding: 'utf8',
      windowsHide: true,
      timeout: waitMs + 4000,
    });
...
function chromeRows() {
  try {
    const script =
      '$rows=@(Get-CimInstance Win32_Process -Filter "Name=\'chrome.exe\'" | Select-Object ProcessId,CommandLine); $rows | ConvertTo-Json -Compress';
    const raw = execFileSync('powershell.exe', ['-NoProfile', '-Command', script], {
      encoding: 'utf8',
      windowsHide: true,
      timeout: 3000,
    ...
    throw new Error('Chrome process query failed; refusing unverified cleanup');
  }
}

function pidsMatching(predicate) {
  return chromeRows()
    .filter((row) => predicate(String(row.CommandLine ?? '')))
...
function leftoverTestChromePids() {
  return pidsMatching(
    (command) => command.includes('\\Temp\\maka-cu-') || command.includes('/Temp/maka-cu-'),
  );
}
...
async function ensureSingleTestChrome() {
  ...
  if (desktop.locked || desktop.unknown)
    throw Object.assign(new Error('interactive desktop unavailable; no UI actions attempted'), {
      blocked: true,
      reason: desktop.locked ? 'environment_desktop_locked' : 'environment_desktop_unknown',
      desktop,
    });
  // Never kill another test run merely because its profile uses our prefix.
  if (leftoverTestChromePids().length)
    throw Object.assign(new Error('another test Chrome still running; not owned by this run'), {
      blocked: true,
      reason: 'environment_blocked_chrome_not_released',
    });
}

function windowsForTree(listWindows, tree, baseline) {
  return (listWindows.result?.windows ?? []).map((item) => ({
    ...item,
    pidInTree: tree.ids.includes(Number(item.pid)),
    newHwnd: !baseline.has(Number(item.hwnd)),
  }));
}

function helperFor(path) {
  const resolved = resolve(path);
  const command = resolved.toLowerCase().endsWith('.dll') ? 'dotnet' : resolved;
  ...
  lines.on('line', (line) => {
    ...
    rpcTrace.push({ id: message.id, response: message });
  ...
  const call = (method, params = {}, timeout = ACTION_TIMEOUT_MS) =>
    new Promise((resolveCall, rejectCall) => {
      const id = nextId++;
      const request = { jsonrpc: '2.0', id, method, params };
      const budget = Math.min(
        timeout,
        Math.max(1, activeDeadline - Date.now()),
      ...
      rpcTrace.push({ id, sentAt: Date.now(), request });
...
async function stopHelper(helper) {
  ...
  try {
    await helper.call('shutdown', {}, 3000);
  } catch {
    try {
      helper.child.kill();
    } catch {}
...
function startOracle() {
  ...
  return {
    ...
    ready: new Promise((resolveReady, reject) => {
      ...
      server.listen(0, '127.0.0.1', () => resolveReady(server.address().port));
    }),
  };
}

async function oracleState(oracle, run) {
  return oracle.store.get(run);
}
async function waitOracle(oracle, run, predicate, timeout = 20_000) {
  const end = Math.min(Date.now() + timeout, deadline);
  while (Date.now() < end) {
    const state = await oracleState(oracle, run);
    if (predicate(state)) return { pass: true, state };
    await sleep(100);
  }
...
function candidateEvidence(observation) {
  return nodesOf(observation)
    .map((item) => ({
      token: item.token,
      name: item.name,
...
function findNode(observation, name, action) {
  return nodesOf(observation).find(
    (item) =>
      item.name === name &&
      (!action || item.actions?.includes(action) || item.patterns?.includes(action)),
  );
}
function pageControlEvidence(observation) {
  ...
  const found = required.map((item) => ({
    ...item,
    node:
      nodesOf(observation).find(
        (node) =>
          node.name === item.name &&
          node.automationId === item.automationId &&
          node.actions?.includes(item.action),
      ) ?? null,
  }));
  return {
    found,
    missing: found.filter((item) => !item.node).map((item) => item.name),
    present: found.filter((item) => item.node).map((item) => item.name),
  };
}

function notAvailable(name, observation, action) {
  const nodes = nodesOf(observation);
  const sameName = nodes.filter((node) => node.name === name);
  const tree = observation?.result?.tree ?? {};
  const reason =
    sameName.length > 0
      ? `unsupported_${action}_pattern`
      : tree.truncated
        ? 'environment_blocked_observation_truncated'
        : 'unsupported_provider_element';
  return {
    ...
    dispatched: false,
    observationEvidence: {
      target: observation?.result?.target ?? null,
      tree,
...
async function runTask(name, fn, tasks) {
  ...
  try {
    ...
    tasks.push({ name, durationMs: Date.now() - at, ...detail });
  } catch (error) {
    tasks.push({
      name,
      durationMs: Date.now() - at,
      executionState: 'fail',
      contractConformance: 'fail',
      dispatched: false,
      error: error.message,
    });
  }
...
async function runMode(helperPath, label, mode, oracle, port) {
  const timing = {
    ...
    settleMs,
    limitation:
      'No target UIA calls from this harness before firstTargetDiscoveryAt; external accessibility clients are not controlled. list_windows may itself touch UIA.',
  };
  const started = Date.now();
  const runDeadline = Math.min(deadline, started + 120_000);
  const run = `${label}-${mode.name}-${Math.random().toString(16).slice(2)}-${Date.now().toString(16)}`;
  const tasks = [];
  let helper;
  let browser;
  let profile;
  ...
  if (remaining() <= 0)
    return {
      ...
      mode: mode.name,
      run,
      chromeArgs: mode.args,
      tasks: padTasks(tasks),
      executionState: 'blocked',
      contractConformance: 'not_tested',
      reason: 'harness_deadline_exceeded',
      artifact: artifact(helperPath),
      durationMs: 0,
    };
  const args = [
  ...
  const watchdog = setTimeout(
    () => {
      ...
      if (browser?.pid) {
        try {
          execFileSync('taskkill.exe', ['/PID', String(browser.pid), '/T', '/F'], {
            windowsHide: true,
            timeout: 5000,
          });
        } catch {}
  ...
  try {
    if (!chromePath)
      throw Object.assign(new Error('Chrome executable unavailable'), {
        blocked: true,
        reason: 'environment_chrome_unavailable',
      });
    profile = mkdtempSync(join(tmpdir(), `maka-cu-${run}-`));
    args[0] = `--user-data-dir=${profile}`;
    helper = helperFor(helperPath);
    helper.setDeadline(runDeadline);
    const init = await helper.call('initialize');
    const before = await helper.call('list_windows');
    ...
    const pageReady = await waitOracle(
      oracle,
      run,
      (state) => state.events.includes('ready'),
      BROWSER_WAIT_MS,
    );
    if (!pageReady.pass)
      throw Object.assign(new Error('fixture not ready before UIA discovery'), { blocked: true });
    timing.pageReadyReceivedAt = Date.now();
    if (preUiaMs > 0) await sleep(preUiaMs);
    timing.firstTargetDiscoveryAt = Date.now();
    const spawnPid = browser.pid;
    let lastWindows = [];
    const waitEnd = Math.min(Date.now() + BROWSER_WAIT_MS, deadline);
    let oracleReady = false;
    while (Date.now() < waitEnd && remaining() > 0) {
      const listed = await helper.call('list_windows');
      const tree = processTree(spawnPid, profile);
      lastWindows = windowsForTree(listed, tree, baseline);
      target = selectTarget(lastWindows, run);
      if (target) break;
      const ready = await waitOracle(oracle, run, (state) => state.events.includes('ready'), 50);
      oracleReady ||= ready.pass;
      await sleep(250);
    }
    if (!target)
      throw Object.assign(
        new Error(`Chromium fixture target not uniquely observable; run=${run}`),
        {
          blocked: true,
          reason: 'environment_blocked_target_identity',
          targetEvidence: { spawnPid, lastWindows, baselineHwnds: [...baseline] },
        },
      );
    const ready = await waitOracle(oracle, run, (state) => state.events.includes('ready'), 5_000);
    if (!ready.pass)
      throw Object.assign(new Error('fixture page did not report ready'), {
        blocked: true,
        reason: 'environment_blocked_fixture_not_ready',
      });
    const observe = async () => {
      const sentAt = Date.now();
      const value = await helper.call('observe', { hwnd: target.hwnd });
      observations.push({
        sentAt,
        receivedAt: Date.now(),
        target: value.result?.target ?? null,
        tree: value.result?.tree ?? null,
        candidates: candidateEvidence(value),
        error: value.error,
      });
      return value;
    };
    const act = (observation, node, op, extra = {}) =>
      helper.call('act', {
    ...
    const compatAct = async (observation, node, op, value) => {
      ...
      return act(observation, node, op, {
        authorizationToken: auth.result.authorizationToken,
        ...(op === 'compat_type_text' ? { value } : {}),
      });
    };
    const desktopBeforeActivation = desktopState();
    if (desktopBeforeActivation.locked || desktopBeforeActivation.unknown)
      throw Object.assign(new Error('desktop unavailable before activation'), {
        blocked: true,
        reason: 'environment_desktop_unavailable',
    ...
    if (broughtForward?.targetIsForeground !== true && foregroundWaitMs > 0) {
      ...
      console.error(
        `WAIT_FOR_FOREGROUND run=${run} hwnd=${target.hwnd}: click the test Chrome title bar; no input is dispatched until the exact HWND is foreground.`,
      );
      while (
        broughtForward?.targetIsForeground !== true &&
        Date.now() + 3250 < foregroundDeadline
      ) {
        await sleep(250);
      ...
      if (Date.now() >= foregroundDeadline)
        broughtForward = { ...broughtForward, targetIsForeground: false, waitExpired: true };
      timing.foregroundWait = { maxMillis: foregroundWaitMs, attempts };
    }
    if (broughtForward?.targetIsForeground !== true)
      throw Object.assign(new Error('test window did not become foreground'), {
        blocked: true,
        reason: 'environment_foreground_not_acquired',
        desktop: broughtForward,
      });
    if (wantTask('chromium_observe_page_controls'))
      await runTask(
        ...
        async () => {
          ...
          if (wakeMs > 0) {
            ...
            wake = {
              ...
              subscribed: false,
              waitMs: settleMs,
              kind: 'settle_only_no_subscription',
            };
          ...
          const added = after
            ? after.pageControls.present.filter(
                (name) => !before.pageControls.present.includes(name),
              )
            : [];
          return {
            ...
            observationEvidence: {
              target:
                (after ? null : beforeObs)?.result?.target ?? beforeObs?.result?.target ?? null,
              tree: {
                truncated: judged.truncated,
                nodeCount: judged.nodeCount,
                elapsedMs: judged.elapsedMs,
    ...
    if (wantTask('chromium_set_text_and_readback'))
      await runTask(
        'chromium_set_text_and_readback',
        async () => {
          const beforeObs = await observe();
          const node = findNode(beforeObs, 'Web text input', 'set_value');
          if (!node) return notAvailable('Web text input', beforeObs, 'set_value');
          const response = await act(beforeObs, node, 'set_value', {
            value: 'chromium-matrix-text',
          });
          const oracleResult = await waitOracle(
            oracle,
            run,
            (state) => pageAHasValue(state, 'chromium-matrix-text'),
            3_000,
          );
          const readbacks = [];
          ...
          for (const delayMs of [0, 250, 750]) {
            ...
            const targetMatches = ['hwnd', 'pid', 'processStartTimeUtc', 'windowGeneration'].every(
              (key) => readback.result?.target?.[key] === beforeObs.result?.target?.[key],
            );
            const matchingNodes = nodesOf(readback).filter(
              (item) => item.automationId === 'web-input',
            );
            const sameElement = matchingNodes.find(
              (item) =>
                node.runtimeId?.length &&
                JSON.stringify(item.runtimeId) === JSON.stringify(node.runtimeId) &&
                item.actions?.includes('set_value'),
            );
            readbacks.push({
              delayMs,
              target: readback.result?.target,
              targetMatches,
              sameElementIdentity: !!sameElement,
              valueMatches:
                targetMatches && !!sameElement && sameElement.value === 'chromium-matrix-text',
              nodes: matchingNodes,
          ...
          return {
            ...classifyDispatchedTask({
              ...
              applicationCompleted: oracleResult.pass,
            }),
            readbacks,
            oracle: { ...oracleResult, application: applicationEvidence(oracleResult.state) },
          };
        },
        tasks,
      );
    if (wantTask('chromium_scroll_capability'))
      await runTask(
        'chromium_scroll_capability',
        async () => {
          const beforeObs = await observe();
          const node = nodesOf(beforeObs).find(
            (item) =>
              item.name === 'Web scroll region' &&
              item.automationId === 'scroll' &&
              item.actions?.includes('scroll') &&
              item.patterns?.includes('Scroll') &&
              item.runtimeId?.length,
          );
          ...
          const requestEvidence = {
            target: beforeObs.result.target,
            snapshotId: beforeObs.result.snapshotId,
            element: node,
            op: 'scroll',
            direction: 'vertical',
            amount: 'large_increment',
          };
          const response = await act(beforeObs, node, 'scroll', {
            direction: 'vertical',
            amount: 'large_increment',
          });
          const oracleResult = await waitOracle(
            oracle,
            run,
            (state) => pageAScrolled(state, beforeScrollTop),
            3_000,
          );
          const usedScrollItemFallback =
            response.result?.outcome?.verification === 'scroll_item_dispatched';
          const readbacks = [];
          for (const delayMs of [0, 250, 750]) {
            if (delayMs) await sleep(delayMs);
            ...
            const targetMatches = ['hwnd', 'pid', 'processStartTimeUtc', 'windowGeneration'].every(
              (key) => observation.result?.target?.[key] === beforeObs.result?.target?.[key],
            );
            const sameElement = nodesOf(observation).find(
              (item) =>
                node.runtimeId?.length &&
                JSON.stringify(item.runtimeId) === JSON.stringify(node.runtimeId) &&
                item.patterns?.includes('Scroll'),
            ...
            readbacks.push({
              ...
              targetMatches,
              sameElementIdentity: !!sameElement,
              scrollState: sameElement?.scrollState,
          ...
          return {
            ...classifyDispatchedTask({
              helperResponse: response,
              applicationCompleted: oracleResult.pass && !usedScrollItemFallback,
            }),
            requestEvidence,
            beforeScrollTop,
            usedScrollItemFallback,
            readbacks,
            oracle: { ...oracleResult, application: applicationEvidence(oracleResult.state) },
          };
        },
    ...
    if (wantTask('chromium_semantic_click_and_status_readback'))
      await runTask(
        'chromium_semantic_click_and_status_readback',
        async () => {
          const beforeObs = await observe();
          const node = nodesOf(beforeObs).find(
            (item) =>
              item.name === 'Apply semantic click' &&
              item.automationId === 'web-button' &&
              item.actions?.includes('click_element') &&
              item.patterns?.includes('Invoke') &&
              item.runtimeId?.length,
          );
          ...
          const oracleResult = await waitOracle(
            oracle,
            run,
            (state) => pageAClicked(state, beforeClickCount),
            3_000,
          );
          ...
          return {
            ...
            requestEvidence,
            beforeClickCount,
            duplicateCheckMs: 250,
            oracle: {
              ...oracleResult,
              pass: completed,
              state: settledState,
              application: applicationEvidence(settledState),
            },
          };
        },
        tasks,
      );
    if (wantTask('chromium_compat_type_text_authorized'))
      await runTask(
        'chromium_compat_type_text_authorized',
        async () => {
          const beforeObs = await observe();
          const node = nodesOf(beforeObs).find(
            (n) =>
              n.name === 'Compat text input' &&
              n.automationId === 'compat-input' &&
              n.runtimeId?.length &&
              n.patterns?.includes('Value'),
          );
          if (!node) return notAvailable('Compat text input', beforeObs, 'compat_type_text');
          const requestEvidence = {
            target: beforeObs.result.target,
            snapshotId: beforeObs.result.snapshotId,
            element: node,
            op: 'compat_type_text',
            value: 'compat-browser-text',
          };
          const response = await compatAct(
            beforeObs,
            node,
            'compat_type_text',
            'compat-browser-text',
          ...
          const oracleResult = await waitOracle(
            oracle,
            run,
            (state) => pageAHasCompatValue(state, 'compat-browser-text'),
            3_000,
          );
          const readbacks = [];
          for (const delayMs of [0, 250, 750]) {
            ...
            const targetMatches = ['hwnd', 'pid', 'processStartTimeUtc', 'windowGeneration'].every(
              (key) => observed.result?.target?.[key] === beforeObs.result?.target?.[key],
            );
            const sameElement = nodesOf(observed).find(
              (n) =>
                JSON.stringify(n.runtimeId) === JSON.stringify(node.runtimeId) &&
                n.patterns?.includes('Value'),
            );
            readbacks.push({
              delayMs,
    ...
    if (wantTask('chromium_compat_press_enter_authorized_navigation'))
      await runTask(
        'chromium_compat_press_enter_authorized_navigation',
        async () => {
          const beforeObs = await observe();
          ...
          if (!node) return notAvailable('Compat text input', beforeObs, 'compat_press_enter');
          const beforeState = await oracleState(oracle, run);
          if (
            !pageAHasCompatValue(beforeState, 'compat-browser-text') ||
            node.value !== 'compat-browser-text'
          )
            return {
              executionState: 'blocked',
              contractConformance: 'not_tested',
              dispatched: false,
              reason: 'navigation_input_prerequisite_not_met',
              observationEvidence: { target: beforeObs.result.target, element: node },
              oracle: beforeState,
            };
          const requestEvidence = {
            target: beforeObs.result.target,
            snapshotId: beforeObs.result.snapshotId,
            element: node,
            op: 'compat_press_enter',
          };
          const binding = {
            run,
            value: 'compat-browser-text',
            sourceUrl: `http://127.0.0.1:${port}/page-a?run=${encodeURIComponent(run)}`,
            destinationUrl: `http://127.0.0.1:${port}/page-b?run=${encodeURIComponent(run)}`,
          };
          const response = await compatAct(beforeObs, node, 'compat_press_enter');
          const oracleResult = await waitOracle(
            oracle,
            run,
            (state) => boundNavigationCompleted(state, binding),
            8_000,
          );
          await sleep(250);
          ...
          return {
            ...
            oracle: {
              ...
              pass: completed,
              state: settled,
              application: applicationEvidence(settled),
            },
          };
        },
        tasks,
    ...
    const executionState = tasks.some((task) => task.executionState === 'fail')
      ...
      : tasks.some((task) => task.executionState === 'unknown')
        ...
        : tasks.some((task) => task.executionState === 'blocked')
          ? 'blocked'
          : 'pass';
    const contractConformance = tasks.some((task) => task.contractConformance === 'fail')
      ? 'fail'
    ...
    return {
      ...
      target,
      spawnPid,
      init: init.result ?? null,
      observations,
      rpcTrace: helper.rpcTrace,
      tasks,
      executionState,
      contractConformance,
      artifact: artifact(helperPath),
    ...
    padTasks(
      tasks,
      error.reason ?? (timedOut ? 'harness_watchdog_timeout' : 'harness_setup_failed'),
    );
    return {
      label,
      mode: mode.name,
      ...
      run,
      target,
      spawnPid: browser?.pid ?? null,
      observations,
      rpcTrace: helper?.rpcTrace ?? [],
      tasks,
      executionState: error.blocked || timedOut ? 'blocked' : 'fail',
      contractConformance: error.blocked || timedOut ? 'not_tested' : 'fail',
      error: timedOut ? 'harness_watchdog_timeout' : error.message,
      reason: error.reason,
      desktop: error.desktop,
      targetEvidence: error.targetEvidence,
      artifact: artifact(helperPath),
      durationMs: Date.now() - started,
      oracle: await oracleState(oracle, run),
    };
...
async function main() {
  ...
  runningOracle = oracle;
  const port = await oracle.ready;
  const subjects = completedSubjects;
  ...
  for (let repetition = 1; repetition <= repetitions; repetition++) {
    for (const [index, helper] of helpers.entries()) {
      for (const mode of modes) {
        await ensureSingleTestChrome();
        subjects.push(
          await runMode(helper, `subject-${index + 1}-repeat-${repetition}`, mode, oracle, port),
        );
        await ensureSingleTestChrome();
      }
    }
  ...
  const count = (items, field) => ({
    ...
    not_tested: items.filter((item) => item[field] === 'not_tested').length,
    total: items.length,
  });
  const result = {
    schema: 'maka.cu.windows/browser-task-results/5',
    startedAt,
    finishedAt: new Date().toISOString(),
    host: { platform: process.platform, arch: process.arch, node: process.version },
    fixture: artifact(pagePath),
    oracle: { bind: '127.0.0.1', port, independent: true, perPageEvidence: true },
    observeOnly,
    modes: modes.map((mode) => ({ name: mode.name, args: mode.args })),
    subjects,
    summary: {
      subjects: {
        execution: count(subjects, 'executionState'),
        contractConformance: count(subjects, 'contractConformance'),
      },
      tasks: {
        execution: count(allTasks, 'executionState'),
        contractConformance: count(allTasks, 'contractConformance'),
      },
    },
  ...
  for (const subject of subjects)
    console.log(
      `${subject.executionState.toUpperCase()} ${subject.label}/${subject.mode} tasks=${(subject.tasks ?? []).filter((task) => task.executionState === 'pass').length}/${(subject.tasks ?? []).length}`,
    );
...
main().catch(async (error) => {
  ...
  writeFileSync(
    outputPath,
    `${JSON.stringify({ schema: 'maka.cu.windows/browser-task-results/5', startedAt, finishedAt: new Date().toISOString(), inputFingerprints, harnessError: error.message, reason: error.reason, desktop: error.desktop, subjects: completedSubjects, executionState: error.blocked ? 'blocked' : 'fail', distributionReady: false }, null, 2)}\n`,
  );
  process.exitCode = 1;
});
