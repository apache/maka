...
if (!firstHelper || !secondHelper || !fixture) {
  console.error(
    'usage: node observe-tree-harness.mjs <helper-1.exe> <helper-2.exe> <fixture.exe> [--out result.json]',
  );
  process.exit(2);
}
...
function actionableNamed(observation, name, action) {
  return named(observation, name).filter(
    (node) =>
      !action ||
      node.actions?.includes(action) ||
      node.patterns?.includes(action) ||
      node.patterns?.includes('Value') ||
  ...
  try {
    ...
    await record('multiple_same_type_edits', async () => {
      ...
      for (let index = 1; index <= 5; index++) {
        ...
        found.push({
          name,
          count: named(observation, name).length,
          actionable: actionableNamed(observation, name, 'set_value').length,
        });
      }
    ...
    await record('multiple_same_type_buttons', async () => {
      ...
      for (let index = 1; index <= 5; index++) {
        ...
        found.push({
          name,
          count: named(observation, name).length,
          actionable: actionableNamed(observation, name, 'click_element').length,
        });
      }
    ...
    await record('deep_nested_input_beyond_render_depth', async () => {
      ...
      const hasDepthEvidence =
        Number.isInteger(renderDepth) && deep.some((node) => Number.isInteger(node.rawDepth));
      const actuallyDeep =
        hasDepthEvidence &&
        deep.some((node) => node.rawDepth > renderDepth && node.actions?.includes('set_value'));
      return {
        ...
        reason:
          deep.length === 0
            ? 'target_not_observed'
            : actuallyDeep
              ? 'deep_actionable_target_observed'
              : hasDepthEvidence
                ? 'fixture_target_not_beyond_render_depth'
                : 'raw_depth_evidence_unavailable',
        deepCount: deep.length,
        midCount: mid.length,
        depthEvidence: {
          maxRawDepthVisited: observation.result?.tree?.maxRawDepthVisited ?? null,
          deep: deep.map(({ runtimeId, rawDepth, observationSource }) => ({
            runtimeId,
            rawDepth,
            observationSource,
          })),
          mid: mid.map(({ runtimeId, rawDepth, observationSource }) => ({
            runtimeId,
            rawDepth,
            observationSource,
          })),
    ...
    await record('budget_truncation_shared_node_limit', async () => {
      ...
      for (let index = 1; index <= 5; index++)
        siblingEdits.push(
          named(observation, `Sibling edit ${String(index).padStart(2, '0')}`).length,
        );
    ...
    await record('actionable_depth_boundary', async () => {
      ...
      const observation = await observe({
        debugLimits: { maxActionableDepth: 2, maxRenderDepth: 1 },
      });
      const tree = observation.result?.tree ?? {};
      ...
      return {
        state:
          tree.truncated === true &&
          tree.truncatedReasons?.includes('actionable_depth') &&
          Number.isInteger(tree.maxRawDepthVisited) &&
          tree.maxRawDepthVisited <= 2 &&
          nodesOf(observation).every((node) =>
            node.observationSource === 'live-patterns'
              ? node.rawDepth == null || node.rawDepth <= 2
              : node.observationSource === 'cached-properties'
                ? node.rawDepth <= 1
                : true,
          )
            ? 'pass'
        ...
        depthEvidence: nodesOf(observation).map(
          ({ name, automationId, runtimeId, rawDepth, observationSource }) => ({
            name,
            automationId,
            runtimeId,
            rawDepth,
        ...
        siblingEditCounts: [1, 2, 3, 4, 5].map(
          (index) => named(observation, `Sibling edit ${String(index).padStart(2, '0')}`).length,
        ),
      };
    });

    const executionState = checks.some((check) => check.state === 'fail')
      ? 'fail'
      : checks.every((check) => check.state === 'not_tested')
        ? 'not_tested'
        : 'pass';
    return {
      label: path,
      app: { kind: 'WPF', title: FIXTURE_TITLE },
...
const subjects = [];
try {
  for (const helperPath of [firstHelper, secondHelper])
    subjects.push(await runSubject(helperPath, fixtureProcess));
} finally {
...
const result = {
  ...
  summary: {
    subjects: {
      pass: subjects.filter((item) => item.executionState === 'pass').length,
      fail: subjects.filter((item) => item.executionState === 'fail').length,
    ...
    checks: {
      ...
      not_tested: subjects
        ...
        .filter((item) => item.state === 'not_tested').length,
...
for (const subject of subjects)
  console.log(
    `${subject.executionState.toUpperCase()} ${subject.artifact.path} checks=${subject.checks.filter((item) => item.state === 'pass').length}/${subject.checks.length}`,
  );
