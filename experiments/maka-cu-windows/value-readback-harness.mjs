...
if (!outputPath)
  throw new Error(
    'usage: node value-readback-harness.mjs <helper1> <helper2> <fixture> <output.json>',
  );
...
const cases = [
  ...
  {
    name: 'password-after-write',
    mode: 'protect-after-write',
    status: 'unknown',
    verification: 'readback_password_field_refused',
  },
  {
    name: 'read-error',
    mode: 'read-error',
    status: 'unknown',
    verification: 'readback_unavailable',
  },
  ...
  {
    name: 'window-name-invalidates-generation',
    mode: 'name-change',
    status: 'unknown',
...
for (const helperPath of [first, second]) {
  ...
  for (const test of cases.filter(
    (t) => !process.env.MAKA_READBACK_CASE || t.name === process.env.MAKA_READBACK_CASE,
  )) {
    ...
    try {
      ...
      const target = nodes.find(
        (n) => n.automationId === 'readback-target' && n.actions?.includes('set_value'),
      );
      ...
      const afterObservation = !test.close
        ? await helper.call('observe', { hwnd: ready.hwnd }).promise
        : null;
      await pause(50);
...
const result = {
  schema: 'maka/value-readback-tests/1',
