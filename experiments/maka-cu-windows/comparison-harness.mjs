...
if (!firstHelper || !secondHelper || !fixture) {
  console.error('usage: node comparison-harness.mjs <helper-1.exe> <helper-2.exe> <fixture.exe> [--out results.json]');
  process.exit(2);
}
...
function artifact(path) {
  ...
  const dependencyFiles = readdirSync(dirname(absolutePath), { withFileTypes: true }).filter((entry) => entry.isFile() && (/\.(?:exe|dll)$/i.test(entry.name) || /(?:runtimeconfig|deps)\.json$/i.test(entry.name))).map((entry) => resolve(dirname(absolutePath), entry.name)).sort((a, b) => a.localeCompare(b));
  const dependencyClosureFiles = dependencyFiles.map((file) => ({ path: file, sizeBytes: statSync(file).size, sha256: sha256File(file) }));
  const dependencyClosureSha256 = createHash('sha256').update(dependencyClosureFiles.map((item) => `${item.path}\0${item.sizeBytes}\0${item.sha256}\n`).join('')).digest('hex').toUpperCase();
  return {
    path: absolutePath,
    exists: true,
    ...
    dependencyClosure: { files: dependencyClosureFiles, totalSizeBytes: dependencyClosureFiles.reduce((total, item) => total + item.sizeBytes, 0), closureSha256: dependencyClosureSha256 },
  };
}
...
function parseChecks(stdout, driverName) {
  ...
  for (const line of stdout.split(/\r?\n/)) {
    ...
    if (lifecycle) {
      checks.push({ status: lifecycle[1].toLowerCase(), name: lifecycle[2], note: lifecycle[3] ?? '' });
      continue;
    }
    const protocol = line.match(/^(PASS|FAIL)\s+(.+?)(?:\s+(\{.*\}))?$/);
    if (protocol) checks.push({ status: protocol[1].toLowerCase(), name: protocol[2], raw: protocol[3] ?? '' });
  }
  return checks;
}
  ...
  return new Promise((resolveResult) => {
    ...
    if (!existsSync(helper) || !existsSync(fixtureArg ?? helper)) {
      ...
      resolveResult({
        ...
        checks: [{ status: 'blocked', name: 'harness_execution', note: `missing executable: ${missing}` }],
        raw: { stdout: '', stderr: '' },
    ...
    const finish = (state, code = null, signal = null, error = null) => {
      ...
      if (checks.length !== spec.expectedChecks) {
        validation.push({ status: 'fail', name: 'harness_check_count', note: `expected=${spec.expectedChecks} actual=${checks.length}` });
      }
      if (!stdout.includes(spec.sentinel)) {
        validation.push({ status: 'fail', name: 'harness_summary_sentinel', note: `missing ${spec.sentinel}` });
      }
      if (checks.some((check) => check.status === 'fail')) {
        validation.push({ status: 'fail', name: 'harness_reported_failure', note: 'driver emitted FAIL' });
      }
    ...
    child.once('error', (error) => finish(error.code === 'ENOENT' ? 'blocked' : 'fail', null, null, error.code ?? error.message));
...
const driverSummary = Object.fromEntries(driverSpecs.map((spec) => {
...
const result = {
  ...
  contract: { path: contractPath, sha256: sha256File(contractPath), version: contract.contractVersion },
  harness: { path: harnessPath, sha256: sha256File(harnessPath) },
  startedAt,
  finishedAt,
  host: { platform: platform(), arch: arch(), osRelease: release(), windowsBuild: version(), node: process.version },
...
for (const subject of subjects) {
  for (const run of subject.runs) {
    ...
    console.log(`${run.state.toUpperCase()} ${subject.label} ${run.driver.name} checks=${checks}/${run.expectedChecks}`);
  }
}
console.log(`RESULTS ${outputPath}`);
process.exitCode = result.summary.subjects.fail === 0 && result.summary.subjects.blocked === 0 ? 0 : 1;
