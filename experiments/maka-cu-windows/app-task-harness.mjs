...
if (!firstHelper || !secondHelper || !fixture) {
  console.error('usage: node app-task-harness.mjs <helper-1.exe> <helper-2.exe> <fixture.exe> [--out result.json]');
  process.exit(2);
}
...
async function stopFixture(fixtureProcess) {
  ...
  await new Promise((resolveDone) => {
    const timer = setTimeout(() => { fixtureProcess.child.kill(); resolveDone(); }, 2_000);
...
async function stopHelper(helper) {
  ...
  try { await helper.call('shutdown'); } catch { helper.child.kill(); }
  await new Promise((resolveDone) => {
    const timer = setTimeout(() => { helper.child.kill(); resolveDone(); }, 2_000);
    helper.child.once('exit', () => { clearTimeout(timer); resolveDone(); });
  });
}
...
const findNode = (observation, automationId, action) => allNodes(observation)
  .find((node) => node.automationId === automationId && (!action || node.actions?.includes(action) || node.patterns?.includes(action)));
const findAnyNode = (observation, automationId) => allNodes(observation)
  .filter((node) => node.automationId === automationId)
  .sort((left, right) => Number(Boolean(right.actions?.length || right.patterns?.length || right.value !== undefined))
    - Number(Boolean(left.actions?.length || left.patterns?.length || left.value !== undefined)))[0];
...
async function runSubject(path, label) {
  ...
  try {
    ...
    if (!setup) throw new Error(`fixture window not selected by explicit HWND/title (hwnd=${fixtureProcess.hwnd})`);
    const observe = async () => helper.call('observe', { hwnd: fixtureProcess.hwnd });
    const action = async (observation, node, params) => helper.call('act', {
    ...
    const record = async (name, fn) => {
      ...
      try {
        ...
        tasks.push({ name, executionState, contractConformance, durationMs: Date.now() - taskStarted, detail: detail?.detail ?? detail });
      } catch (error) {
        tasks.push({ name, executionState: error.blocked ? 'blocked' : 'fail', contractConformance: error.blocked ? 'pass' : 'fail', durationMs: Date.now() - taskStarted, error: error.message });
      }
    };
    if (!initialized.result || initialized.result.protocol !== 'maka.cu.windows/0') throw new Error('initialize protocol mismatch');
    await record('set_text_and_readback', async () => {
      ...
      if (response.result?.outcome?.status !== 'verified') throw new Error(`set_value ${JSON.stringify(response)}`);
      const after = await observe();
      const readback = findAnyNode(after, 'wpf-input');
      if (readback?.value !== 'matrix-text') throw new Error(`set_value readback=${JSON.stringify(readback?.value)} nodes=${JSON.stringify(allNodes(after).filter((candidate) => candidate.automationId === 'wpf-input'))}`);
      return { status: response.result.outcome.status, verification: response.result.outcome.verification };
    });
    await record('semantic_click_and_status_readback', async () => {
      const before = await observe();
      const node = findNode(before, 'wpf-button', 'click_element');
      if (!node) throw new Error(`wpf-button Invoke token unavailable nodes=${JSON.stringify(allNodes(before).filter((candidate) => candidate.automationId === 'wpf-button'))}`);
      ...
      if (status?.name !== 'clicked') throw new Error(`click readback=${JSON.stringify(status?.name)}`);
      return { status: response.result.outcome.status, verification: response.result.outcome.verification };
    });
    await record('selection_and_status_readback', async () => {
      const before = await observe();
      const node = allNodes(before).find((candidate) => candidate.name === 'Alpha'
        && (candidate.actions?.includes('select') || candidate.patterns?.includes('SelectionItem')));
      if (!node) throw new Error(`Alpha SelectionItem token unavailable candidates=${JSON.stringify(allNodes(before).filter((candidate) => candidate.name?.includes('Alpha') || candidate.actions?.includes('select') || candidate.patterns?.includes('SelectionItem')))}`);
      ...
      const status = findAnyNode(await observe(), 'wpf-status');
      if (status?.name !== 'selected:Alpha') throw new Error(`select readback=${JSON.stringify(status?.name)}`);
      return { status: response.result.outcome.status, verification: response.result.outcome.verification };
    });
    await record('toggle_and_status_readback', async () => {
      ...
      const status = findAnyNode(await observe(), 'wpf-status');
      if (status?.name !== 'toggled:on') throw new Error(`toggle readback=${JSON.stringify(status?.name)}`);
      return { status: response.result.outcome.status, verification: response.result.outcome.verification };
    });
    await record('scroll_and_position_readback', async () => {
      ...
      if (!['verified', 'unknown'].includes(response.result?.outcome?.status)) throw new Error(`scroll ${JSON.stringify(response)}`);
      const after = await observe();
      const readback = findAnyNode(after, 'wpf-scroll');
      return { executionState: response.result.outcome.status === 'verified' ? 'pass' : 'unknown', contractConformance: 'pass', detail: { status: response.result.outcome.status, verification: response.result.outcome.verification, observed: !!readback } };
    });
    await record('enter_is_typed_unsupported', async () => {
      ...
      if (outcome?.status !== 'refused' || outcome.reason !== 'unsupported_enter' || outcome.effect !== 'none')
    ...
    await record('compat_type_text_authorized', async () => {
      ...
      const response = await compatAction(compatBefore, compatNode, 'compat_type_text', 'compat-text');
      if (response.error) {
        if (response.error.message?.includes('compat_focus_refused')) return { executionState: 'blocked', contractConformance: 'pass', detail: response.error };
      ...
      if (readback?.value !== 'compat-text') throw new Error(`compat readback=${JSON.stringify(readback?.value)}`);
    ...
    await record('compat_press_enter_authorized', async () => {
      ...
      const outcome = response.result?.outcome;
      if (outcome?.status === 'unknown') return { executionState: 'unknown', contractConformance: 'pass', detail: outcome };
      if (outcome?.status === 'refused') return { executionState: 'blocked', contractConformance: 'pass', detail: outcome };
      throw new Error(`compat_press_enter unexpectedly verified ${JSON.stringify(response)}`);
    });
    const security = async (name, fn) => {
      try { securityChecks.push({ name, state: await fn() }); }
      catch (error) { securityChecks.push({ name, state: 'fail', error: error.message }); }
    };
    ...
    await security('authorization_payload_and_reuse_refused', async () => {
      const before = await observe();
      const node = findAnyNode(before, 'wpf-input');
      const auth = await helper.call('authorize_compat', { snapshotId: before.result.snapshotId, elementToken: node.token, op: 'compat_type_text', value: 'negative' });
      const mismatch = await action(before, node, { op: 'compat_type_text', value: 'tampered', authorizationToken: auth.result.authorizationToken });
      if (mismatch.error?.message !== 'compat_authorization_mismatch') return 'fail';
      const used = await action(before, node, { op: 'compat_type_text', value: 'negative', authorizationToken: auth.result.authorizationToken });
      if (used.error || !['verified', 'unknown', 'refused'].includes(used.result?.outcome?.status)) return 'fail';
      const reused = await action(before, node, { op: 'compat_type_text', value: 'negative', authorizationToken: auth.result.authorizationToken });
    ...
    await security('authorization_cross_snapshot_refused', async () => {
      const before = await observe();
      const node = findAnyNode(before, 'wpf-input');
      const auth = await helper.call('authorize_compat', { snapshotId: before.result.snapshotId, elementToken: node.token, op: 'compat_type_text', value: 'negative-cross' });
      const other = await observe();
      const otherNode = findAnyNode(other, 'wpf-input');
      const response = await action(other, otherNode, { op: 'compat_type_text', value: 'negative-cross', authorizationToken: auth.result.authorizationToken });
      return response.error?.message === 'compat_authorization_mismatch' ? 'pass' : 'fail';
    });
    const executionState = tasks.some((task) => task.executionState === 'fail') ? 'fail' : tasks.some((task) => task.executionState === 'unknown') ? 'unknown' : tasks.some((task) => task.executionState === 'blocked') ? 'blocked' : 'pass';
    return { label, app: { kind: 'WPF', title: FIXTURE_TITLE, version: 'fixture-v1' }, artifact: artifact(path), fixture: artifact(fixturePath), executionState, contractConformance: tasks.every((task) => task.contractConformance === 'pass') && securityChecks.every((check) => check.state === 'pass') ? 'pass' : 'fail', tasks, securityChecks, durationMs: Date.now() - started };
  } catch (error) {
    return { label, app: { kind: 'WPF', title: FIXTURE_TITLE, version: 'fixture-v1' }, artifact: artifact(path), fixture: artifact(fixturePath), executionState: 'blocked', contractConformance: 'pass', tasks, securityChecks, durationMs: Date.now() - started, error: error.message };
  } finally {
...
const subjects = [await runSubject(firstHelper, 'subject-1'), await runSubject(secondHelper, 'subject-2')];
const counts = (items, field = 'executionState') => ({ pass: items.filter((item) => item[field] === 'pass').length, fail: items.filter((item) => item[field] === 'fail').length, blocked: items.filter((item) => item[field] === 'blocked').length, unknown: items.filter((item) => item[field] === 'unknown').length, total: items.length });
...
for (const subject of subjects) console.log(`${subject.executionState.toUpperCase()} ${subject.label} tasks=${subject.tasks.filter((task) => task.executionState === 'pass').length}/${subject.tasks.length}`);
