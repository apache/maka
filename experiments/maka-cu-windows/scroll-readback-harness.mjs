...
if (!outputPath) throw new Error('usage: node scroll-readback-harness.mjs <helper1> <helper2> <fixture> <output.json>');
const pause = ms => new Promise(r => setTimeout(r, ms));
const fingerprint = path => ({ path, sha256: createHash('sha256').update(readFileSync(path)).digest('hex') });
const artifacts = path => [path, join(dirname(path), basename(path).replace(/\.exe$/, '.dll'))].filter(existsSync).map(fingerprint);
function child(path, args = []) {
  const process = spawn(path, args, { stdio: ['pipe', 'pipe', 'pipe'] });
  const events = [], transcript = [], pending = new Map();
  ...
  createInterface({ input: process.stdout }).on('line', line => {
    ...
    if (value.id != null && pending.has(value.id)) {
      const w = pending.get(value.id); pending.delete(value.id); clearTimeout(w.timer); w.resolve(value);
...
const cases = [
  ...['vertical', 'horizontal'].flatMap(direction => ['small_increment', 'large_increment', 'small_decrement', 'large_decrement'].map(amount => ({ name: `delayed-${direction}-${amount}`, mode: 'delayed', direction, amount, status: 'verified', verification: 'scroll_position_readback_changed', retry: true }))),
  { name: 'timeout', mode: 'never', status: 'unknown', verification: 'scroll_readback_timeout' },
  { name: 'cancel-after-mutation', mode: 'never', status: 'unknown', verification: 'readback_cancelled_after_dispatch', cancel: true },
  { name: 'wrong-direction', mode: 'wrong-direction', status: 'unknown', verification: 'scroll_readback_wrong_direction' },
  { name: 'invalid-after', mode: 'invalid-after', status: 'unknown', verification: 'scroll_readback_invalid_percent' },
  { name: 'read-error', mode: 'read-error', status: 'unknown', verification: 'scroll_readback_unavailable' },
  { name: 'throw-after-scroll', mode: 'throw-after-scroll', status: 'unknown', verification: 'scroll_failed_after_dispatch' },
  { name: 'axis-lost', mode: 'no-scroll-after', status: 'unknown', verification: 'scroll_readback_axis_not_scrollable' },
  ...
  { name: 'generation-changed', mode: 'name-change', status: 'unknown', verification: 'post_revalidation_failed' },
  { name: 'at-end', mode: 'at-end', status: 'refused', verification: 'scroll_at_boundary', noMutation: true },
  { name: 'at-start', mode: 'at-start', amount: 'large_decrement', status: 'refused', verification: 'scroll_at_boundary', noMutation: true },
  { name: 'no-scroll', mode: 'no-scroll', status: 'refused', verification: 'scroll_axis_not_scrollable', noMutation: true },
  { name: 'invalid-before', mode: 'invalid-before', status: 'refused', verification: 'scroll_invalid_percent', noMutation: true },
  { name: 'no-amount', mode: 'delayed', amount: 'no_amount', status: 'refused', verification: 'scroll_no_amount', noMutation: true },
  { name: 'scroll-item-not-directional', mode: 'scrollitem-only', status: 'refused', verification: 'scroll_pattern_unavailable', noMutation: true },
];
const subjects = [];
for (const helperPath of [first, second]) {
  const checks = [];
  for (const test of cases.filter(t => !process.env.MAKA_SCROLL_CASE || t.name === process.env.MAKA_SCROLL_CASE)) {
    const fx = child(fixturePath, [test.mode]), helper = child(helperPath);
    let result;
    try {
      ...
      const target = nodes.find(n => n.automationId === 'scroll-readback-target' && n.patterns?.includes(test.mode === 'scrollitem-only' ? 'ScrollItem' : 'Scroll'));
      if (!target || !target.runtimeId?.length) throw new Error('fixture scroll target not observed');
      const direction = test.direction ?? 'vertical', amount = test.amount ?? 'large_increment';
      const params = { snapshotId: observation.result.snapshotId, elementToken: target.token, op: 'scroll', direction, amount };
      ...
      const expectedPosition = 40 + (increment ? 20 : -20) * (test.mode === 'wrong-direction' ? -1 : 1);
      const expectedAmount = amount.split('_').map(s => s[0].toUpperCase() + s.slice(1)).join('');
      const checksOk = outcome?.status === test.status
        && (!test.verification || outcome?.verification === test.verification)
        && (!outcome?.readback || (outcome.readback.direction === direction && outcome.readback.amount === amount
          && outcome.readback.beforePercent === 40 && outcome.readback.source === `ScrollPattern.Current${direction === 'horizontal' ? 'Horizontal' : 'Vertical'}ScrollPercent`))
        && (test.status !== 'verified' || outcome?.readback?.samples?.at(-1)?.percent === expectedPosition)
        && (!test.retry || (outcome?.readback?.attempts > 1 && outcome.readback.samples[0].percent === 40))
        && mutations.length === (test.noMutation ? 0 : 1)
        && mutations.every(e => e.count === 1 && e.position === expectedPosition
        ...
        && durationMs < 3000;
      result = { name: test.name, state: checksOk ? 'pass' : 'fail', durationMs, response, cancelResponse, replay, mutations, observation };
      ...
      result.fixtureEvents = fx.events; result.fixtureTranscript = fx.transcript; result.transcript = helper.transcript;
      checks.push(result);
...
const result = { schema: 'maka/scroll-readback-tests/1', finishedAt: new Date().toISOString(),
