...
export function McpPage(props: { hubHeader?: ModuleHubHeader }) {
  ...
  async function importJson(event: React.FormEvent) {
    ...
    let claimed: string[] = [];
    try {
      // Same claim rule as saveDraft — main's in-lane gate is the real
      // authority, but claiming here means a veto (a login owning one of
      // these ids) surfaces as the localized serverBusy copy instead of
      // main's raw English error.
      for (const serverId of Object.keys(imported.mcpServers)) {
        if (!serverOpsRef.current.claim(serverId, 'save')) {
          if (mounted.current) toast.error(copy.errors.import, copy.errors.serverBusy);
          return;
        }
      ...
      for (const serverId of claimed) serverOpsRef.current.release(serverId);
      if (mounted.current) setBusy(null);
    }
  }
