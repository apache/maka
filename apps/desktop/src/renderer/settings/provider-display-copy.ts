...
type ProviderLocaleCatalog = Record<'zh-CN' | 'zh-TW' | 'en', ProviderCopy>;
...
} satisfies Record<ProviderType, ProviderLocaleCatalog>;

export function providerDisplay(type: ProviderType, locale: UiLocale): ProviderCopy {
  const copy = PROVIDER_DISPLAY_COPY[type]?.[locale === 'ko' ? 'en' : locale];
