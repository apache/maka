...
const MCP_COPY = {
  ...
  ko: {
  errors: {
    load: "MCP를 로드하지 못했습니다.",
    install: name => `${name}을(를) 설치하지 못했습니다.`,
    cancelInstall: name => `${name} 설치를 취소하지 못했습니다.`,
    save: "MCP를 저장하지 못했습니다.",
    import: "MCP를 가져오지 못했습니다.",
    update: "MCP를 업데이트하지 못했습니다.",
    test: "MCP 테스트 실패",
    remove: "MCP를 삭제하지 못했습니다.",
    unavailableStatus: "서버가 사용 가능 상태를 반환하지 않았습니다.",
    mapLine: line => `${line} 행에서는 KEY=값을 사용해야 합니다.`,
    importJson: "MCP 구성은 유효한 JSON이어야 합니다.",
    importObject: "MCP JSON은 객체여야 합니다.",
    importVersion: version => `지원되지 않는 MCP 구성 버전 ${version}; 버전 1, 2, 3이 지원됩니다.`,
    importServersObject: "mcpServers는 객체여야 합니다.",
    importProtocolVersion: "원격 프로토콜 기본 설정에는 버전 2 또는 3이 필요합니다. stdio 프로토콜 기본 설정에는 버전 3이 필요합니다."
  },
  toast: {
    templateInstalled: name => `${name} 템플릿이 설치되었습니다.`,
    templateInstalledDetail: "연결을 활성화하기 전에 설치됨에서 자격 증명 구성을 완료하세요.",
    installed: name => `${name} 설치됨`,
    installedDetail: "발견된 도구는 다음 에이전트 턴부터 적용됩니다.",
    installCancelled: name => `${name} 설치 취소됨`,
    saved: "MCP가 저장되었습니다.",
    savedDetail: "새로운 도구는 다음 에이전트 차례부터 적용됩니다.",
    imported: "MCP를 가져왔습니다.",
    importedDetail: count => `${count} ${count === 1 ? '서버' : '서버'}를 가져왔습니다.`,
    connectionOk: "MCP 연결이 정상입니다.",
    toolLatency: (count, latencyMs) => `${count}개 도구 · ${latencyMs}ms`,
    connectionFailed: "MCP 연결 실패",
    removed: "MCP가 삭제되었습니다."
  },
  remove: {
    title: id => `MCP “${id}”을 삭제하시겠습니까?`,
    description: "해당 도구는 다음 에이전트 차례부터 제거되며 구성은 자동으로 복원될 수 없습니다.",
    confirm: "삭제",
    cancel: "취소"
  },
  page: {
    ...
    refresh: "새로 고치다",
    add: "MCP 추가",
    metaInstalled: count => `${count} 설치됨`,
    metaErrors: count => `${count} ${count === 1 ? "연결 오류" : "연결 오류"}`,
    searchMatches: count => `${count}개 일치`,
    workspaceAria: "MCP 마켓플레이스 및 설치된 서버",
    toolbarAria: "MCP 브라우저 컨트롤",
    setupTitle: "Maka를 작업 환경에 연결하세요",
    setupDescription: "선별된 템플릿으로 시작하거나 stdio, Streamable HTTP 또는 SSE 서버를 추가하세요.",
    localStdio: "로컬 스튜디오",
    categoriesAria: "MCP 카테고리",
    market: "시장",
    installed: "설치됨",
    searchPlaceholder: "MCP 검색…",
    searchAria: "MCP 검색",
    noMarket: "일치하는 MCP 서버가 없습니다.",
    noMarketDetail: query => `다른 키워드를 시도하거나 "${query}"을 지워 모든 템플릿을 보십시오.`,
    clearSearch: "검색 지우기",
    loading: "MCP 구성을 읽는 중…",
    noInstalled: "MCP 서버가 설치되지 않았습니다.",
    noInstalledDetail: "마켓플레이스에서 템플릿을 선택하거나 자체 서버를 수동으로 추가하세요.",
    browseMarket: "마켓플레이스 찾아보기",
    noInstalledMatch: "일치하는 설치된 MCP 서버가 없습니다.",
    noInstalledMatchDetail: query => `다른 키워드를 사용하거나 "${query}"을 지워 설치된 모든 서버를 확인하세요.`
  },
  detail: {
    label: "서버 세부정보",
    enabled: "활성화됨",
    ...
    toolsLabel: "도구",
    statusLabel: "상태",
    protocolLabel: "MCP 프로토콜",
    negotiatedProtocol: (era, revision) => `${era === 'modern' ? "현대의" : "유산"} · ${revision}`,
    inspectorOpened: id => `${id} 세부정보 열림`
  },
  card: {
    macOnly: "macOS만 해당",
    manage: "관리하다",
    cancellingAria: name => `${name} 설치 취소 중`,
    cancelAria: name => `${name} 설치 취소`,
    installAria: name => `${name} 설치`,
    cancelling: "취소 중…",
    cancel: "설치 취소",
    install: "설치하다"
  },
  row: {
    ...
    edit: "편집하다",
    delete: "삭제",
    tools: count => `${count}개 도구`,
    disabled: "장애가 있는",
    disconnected: "연결이 끊김",
    connecting: "연결 중",
    connected: count => `${count}개 도구`,
    failed: "연결 실패"
  },
  editor: {
    importTitle: "JSON에서 가져오기",
    editTitle: id => `${id} 편집`,
    addTitle: "MCP 추가",
    importSubtitle: "mcpServers 구성을 붙여넣습니다. 이름이 일치하는 서버가 업데이트됩니다.",
    manualSubtitle: "구성은 현재 작업 영역에 대한 mcp.json에 저장됩니다.",
    modeAria: "MCP 추가 방법",
    manual: "수동 구성",
    pasteJson: "JSON 붙여넣기",
    jsonConfig: "JSON 구성",
    jsonHelp: "완전한 mcpServers 구성 또는 서버 맵을 지원합니다. 이 가져오기에서 생략된 기존 MCP 서버는 보존됩니다.",
    cancel: "취소",
    importConnect: "가져오기 및 연결",
    transportAria: "연결 방법",
    localStdio: "로컬 스튜디오",
    remoteUrl: "원격 URL",
    serverId: "서버ID",
    command: "명령",
    commandPlaceholder: "npx -y @modelcontextprotocol/서버 파일 시스템 /경로/to/폴더",
    commandHelp: "전체 명령줄; 공백이 포함된 인수를 인용합니다. 쉘에서는 해석되지 않습니다.",
    workingDirectory: "작업 디렉토리",
    workingDirectoryPlaceholder: "선택사항(예: /path/to/project)",
    environment: "환경",
    environmentHelp: "한 줄에 하나의 KEY=값 항목; 이 MCP에 필요한 변수를 완성합니다.",
    url: 'MCP URL',
    headers: "HTTP 헤더",
    headersHelp: "한 줄에 하나의 헤더=값 항목이 있습니다.",
    saveConnect: "저장하고 연결하세요",
    required: "이 필드는 필수입니다.",
    invalidUrl: "유효한 HTTP 또는 HTTPS URL을 입력하세요.",
    unbalancedQuote: "닫히지 않은 견적.",
    transportLabel: "수송",
    transportAuto: "자동 대체",
    transportStreamableHttp: "스트리밍 가능한 HTTP",
    transportLegacySse: "레거시 SSE",
    protocolLabel: "프로토콜 선호도",
    protocolLegacy: "유산",
    protocolAuto: "자동 협상",
    protocolModern: "2026년 7월 28일에만",
    protocolHelp: "기존 구성은 기본적으로 레거시로 설정됩니다. 자동 협상은 서버 응답에서 시대를 선택합니다.",
    sseProtocolHelp: "레거시 SSE는 레거시 프로토콜 시대만 지원합니다.",
    expandAdvanced: "고급 설정 표시",
    collapseAdvanced: "고급 설정 숨기기",
    stdioProtocolHelp: "자동 협상 및 \"2026-07-28만\"은 먼저 동일한 명령, 인수, 작업 디렉터리 및 환경을 사용하여 단기 프로브를 시작합니다. 세션 프로세스는 프로브가 종료된 후에만 시작됩니다. 기존 구성은 기본적으로 레거시로 설정되고 하나의 프로세스를 시작합니다."
  }
}
