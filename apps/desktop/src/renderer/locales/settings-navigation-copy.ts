...
const SETTINGS_NAVIGATION_COPY_BY_LOCALE = {
  'zh-CN': {
    ...
    sections: {
      ...
      'web-access': { label: '网页访问', description: '浏览器客户端的通行口令与验证器。桌面应用不会索要这些凭据。' },
  ...
  'zh-TW': {
    ...
    sections: {
      ...
      'web-access': { label: '網頁存取', description: '瀏覽器用戶端的通行口令與驗證器。桌面應用不會索要這些憑據。' },
  ...
  en: {
    ...
    sections: {
      ...
      'web-access': { label: 'Web access', description: 'Passphrase and authenticator for the browser client. The desktop app does not ask for these.' },
