...
const SETTINGS_MEMORY_COPY = {
  ...
  'zh-TW': {
    ...
    countArchived: (count, draft) => draft ? `草稿 ${count} 則已封存` : `${count} 則已封存`,
    saveSummary: (active, archived) => archived > 0
      ? `目前 ${active} 則生效 / ${archived} 則已封存；已保留上一版備份。`
      : `目前 ${active} 則生效；已保留上一版備份。`,
    backupSummary: (active, archived) => archived > 0
      ? `${active} 則生效 / ${archived} 則已封存`
      : `${active} 則生效`,
    ...
    results: {
      ...
      invalid_content: 'MEMORY.md 內容無效，請檢查格式後重試。', invalid_scope: '目前記憶操作的作用範圍無效。',
      not_found: '找不到對應的記憶項目。', not_pending: '對應的記憶項目不在待確認狀態。',
      upload_not_found: '記憶上傳工作階段不存在或已過期。', upload_incomplete: '記憶內容尚未上傳完整。',
      upload_conflict: '另一個記憶上傳正在進行，請重試。',
    },
    origins: { manual: '手動記錄', imported: '匯入記錄', extracted: '確認提取', unknown: '手寫項目' }, entryStatuses: { draft: '草稿', review_required: '待確認', active: '生效', archived: '已封存', rejected: '已拒絕', unknown: '未識別' }, backupKinds: { reset: '重置前備份', restore: '恢復前備份', save: '儲存前備份' }, memoryStatuses: { ok: '本地檔案已就緒', disabled: '已關閉', safe_mode: '安全模式', incognito_blocked: '隱身停用', error: '讀取失敗' }, promptBlocked: { disabled: '本地記憶已關閉。', incognito: '隱身模式下不會提供本地記憶。', safeMode: 'MEMORY.md 過大，目前不會提供。', agentRead: '模型上下文讀取未開啟。' }, backupOversize: '備份過大，無法預覽項目', previewOversize: '草稿過大，項目預覽已暫停；儲存前請先刪減 MEMORY.md 內容。', previewTruncationMarker: '[本地記憶已按長度截斷]',
  },
  en: {
    intlLocale: 'en-US',
    text: enText,
