...
const zhCopy = {
  detail: {
    ...
    saveFailed: '保存模型连接失败', saveModelsFailed: '保存启用模型失败', saveThinkingOptionsFailed: '保存思考级别声明失败',
    ...
    thinkingOptions: '思考级别', thinkingOptionsNone: '未声明',
    thinkingOptionsDeclared: (count: number) => `${count} 个模型已声明`,
    thinkingOptionsHint: '声明该模型支持的思考档位后，聊天输入框会出现思考级别选择器。档位值会原样作为 reasoning_effort 发送给中转站，请确认中转站确实透传后再声明（常见取值为 low / high / max）。',
    thinkingOptionsModel: '模型', thinkingOptionsEfforts: '档位',
  ...
  panel: {
    ...
    emptyHelp: '从下方选择一种连接方式开始。', default: '默认', addHelp: '选择账号登录、模型计划、API、聚合服务或本地运行时。',
...
const enCopy: ProviderSettingsCopy = {
  detail: {
    ...
    saveFailed: 'Failed to save model connection', saveModelsFailed: 'Failed to save enabled models', saveThinkingOptionsFailed: 'Failed to save thinking-level declaration',
    ...
    thinkingOptions: 'Thinking level', thinkingOptionsNone: 'Not declared',
    thinkingOptionsDeclared: (count: number) => `${count} model${count === 1 ? '' : 's'} declared`,
    thinkingOptionsHint: 'Declaring the effort levels a model supports brings back the thinking-level selector in the chat composer. The values are sent verbatim as reasoning_effort to your relay — confirm the relay actually passes them through before declaring (common values: low / high / max).',
    thinkingOptionsModel: 'Model', thinkingOptionsEfforts: 'Levels',
  ...
  panel: {
    ...
    emptyHelp: 'Choose a connection method below to begin.', default: 'Default', addHelp: 'Choose account sign-in, a model plan, API, aggregator, or local runtime.',
