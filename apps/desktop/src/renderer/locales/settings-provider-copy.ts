...
const zhCopy = {
  ...
  add: {
    ...
    probeCatalog: '读取模型目录',
    probeCatalogHelp: '从上方服务地址读取该端点提供的模型，选中后填入默认模型；端点没有模型目录时照旧手填。',
    probeCatalogReading: '读取中…',
    probeCatalogLoaded: (count: number) => `已读取 ${count} 个模型`,
    probeCatalogEmpty: '该端点没有返回模型。',
    probeCatalogNeedsKey: '该端点需要 API Key，请先填写再读取。',
    probeCatalogNeedsEndpoint: '该端点未配置服务地址，请先填写再读取。',
    probeCatalogFailed: '读取失败，请确认服务地址与密钥后重试。',
    probeCatalogUnsupported: '该服务商不提供模型目录。',
    probeCatalogPickPlaceholder: '从目录选择默认模型',
    ...zhCapabilitiesCopy,
  },
  oauthFlow: {
...
const enCopy: ProviderSettingsCopy = {
  ...
  add: {
    ...
    probeCatalog: 'Read model catalog',
    probeCatalogHelp: 'Reads the models this endpoint serves and fills the default model when you pick one. If the endpoint has no catalog, type a model ID as before.',
    probeCatalogReading: 'Reading…',
    probeCatalogLoaded: (count: number) =>
      count === 1 ? 'Read 1 model' : `Read ${count} models`,
    probeCatalogEmpty: 'This endpoint returned no models.',
    probeCatalogNeedsKey: 'This endpoint requires an API key. Enter one first.',
    probeCatalogNeedsEndpoint: 'This endpoint has no service URL configured. Enter one first.',
    probeCatalogFailed: 'Could not read the catalog. Check the service URL and key, then try again.',
    probeCatalogUnsupported: 'This provider does not expose a model catalog.',
    probeCatalogPickPlaceholder: 'Pick a default model from the catalog',
    ...enCapabilitiesCopy,
  },
  oauthFlow: {
