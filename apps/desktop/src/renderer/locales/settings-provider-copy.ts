...
const zhCopy = {
  detail: {
    ...
    accounts: '账号 / API Keys', accountsHelp: '同一个模型连接下可以配置多个 API Key 并启用负载均衡；密钥只保存在本机。',
    accountsOAuth: 'Codex 账号', accountsOAuthHelp: '可以登录多个 OpenAI Codex 账号并启用负载均衡；登录令牌只保存在本机。', addOAuthProfile: '添加 Codex 账号', profileOAuthUnconfigured: '尚未登录',
    profileReadFailed: '读取账号状态失败', addProfile: '添加 API Key', profileLabel: '名称', profileWeight: '权重',
    profileWeightHelp: '权重决定该 Key 在均衡模式下的流量比例（1–100）。',
    profilePrimary: '主账号', profileAccount: (index: number) => `账号 ${index}`, profileSecondary: (label: string) => `备用账号 · ${label}`, profileMoreActions: (label: string) => `${label}的更多操作`,
    profileConfigured: '已配置', profileUnconfigured: '未配置密钥', profileUnverified: '未验证', profileReady: '可用',
    profileCooldown: '冷却中', profileInvalid: '凭据无效', profileNeedsReauth: '需要重新验证', profileDisabledTag: '已停用',
    profileEnabled: '启用', profileDisabled: '停用', profileLastTestNone: '尚未测试',
    profileSupportedModels: (count: number) => `已验证 ${count} 个模型`,
    profileReplaceCredential: '更换密钥', profileSaveCredential: '保存密钥', profileKeyPlaceholder: '粘贴新的 API Key',
    profileTest: '测试', profileFetchModels: '同步模型目录',
    profileRemove: '移除', profileRemovePrimaryHelp: '这个账号不能单独移除；可以停用或重新登录。',
    removeProfileTitle: (label: string) => `移除账号「${label}」？`,
    removeProfileDescription: '该账号的密钥与验证记录会一并删除；删除后需要重新添加才能参与均衡。',
    removeProfileConfirm: '移除账号',
    profileTestSuccess: (label: string) => `账号「${label}」测试成功`, profileTestFailed: (label: string) => `账号「${label}」测试失败`,
    profileRoutingMode: '路由模式', profileRoutingLegacy: '主账号模式（均衡未启用）', profileRoutingBalanced: '负载均衡已启用',
    profileRoutingAutomatic: '自动模式（负载均衡）', profileRoutingAutomaticHelp: '在可用账号之间等量分配新请求。',
    profileRoutingManual: '手动模式（按顺序切换）', profileRoutingManualHelp: '优先使用排在最前的账号；遇到额度或限流不可用时切换到下一个。',
    profileRoutingInactive: '尚未启用多账号路由', profileRoutingFailed: '无法修改路由模式', profileOrderFailed: '无法保存账号顺序',
    profileMoveUp: '上移', profileMoveDown: '下移', profileRefreshUsage: '刷新额度',
    profileUsageLoading: '额度读取中…', profileUsageUnavailable: '额度暂不可用',
    profileUsageUnsupported: '当前账号不支持额度查询',
    profileUsageCredentialUnavailable: '登录状态不可用，请重新登录',
    profileUsageProviderRejected: '额度服务拒绝了查询请求',
    profileUsageProviderUnavailable: '额度服务或网络暂不可用',
    profileUsageInvalidResponse: '额度数据格式暂不兼容',
    profileUsageFiveHour: (value: number) => `5 小时已用 ${value}%`, profileUsageSevenDay: (value: number) => `每周已用 ${value}%`, profileUsageRefreshAt: (when: string) => `预计下次刷新 ${when}`,
    profileBalancedHint: '仅使用主账号执行请求；备用账号可配置和测试，但不会参与流量。',
    profileBalancedActive: (count: number) => `已启用均衡：${count} 个模型有多个可用账号。`,
    profileBalancedPending: '启用均衡需要至少一个模型有两个已测试的账号。',
    profileEnableBalanced: '启用负载均衡', profileDisableBalanced: '关闭负载均衡',
    profileReadyCandidate: (count: number) => `${count} 个模型已有多账号候选`,
    profileNoProfiles: '还没有备用账号。添加密钥后先测试，再启用均衡。',
    profileDraftSaved: '已保存密钥，测试通过后可在「路由模式」启用均衡。',
  },
  shared: {
  ...
  oauthSection: {
    ...
    codexAddAccount: '登录 OpenAI 账号', codexAccountLabel: '账号名称', codexAccountPlaceholder: '例如：工作账号', codexDefaultAccount: '账号',
    codexAddAccountDetail: '登录后会添加到现有 OpenAI OAuth 连接；账号昵称可稍后在模型连接详情中修改。',
...
const enCopy: ProviderSettingsCopy = {
  detail: {
    ...
    // Credential Profiles (Accounts / API Keys)
    accounts: 'Accounts / API Keys', accountsHelp: 'Multiple API keys can be configured under one model connection with load balancing; keys stay on this machine.',
    accountsOAuth: 'Codex accounts', accountsOAuthHelp: 'Sign in to multiple OpenAI Codex accounts and enable load balancing. Login tokens stay on this device.', addOAuthProfile: 'Add Codex account', profileOAuthUnconfigured: 'Not signed in',
    profileReadFailed: 'Failed to read account status', addProfile: 'Add API key', profileLabel: 'Name', profileWeight: 'Weight',
    profileWeightHelp: 'The weight decides this key’s share of traffic in balanced mode (1–100).',
    profilePrimary: 'Primary account', profileAccount: (index: number) => `Account ${index}`, profileSecondary: (label: string) => `Secondary account · ${label}`, profileMoreActions: (label: string) => `More actions for ${label}`,
    profileConfigured: 'Configured', profileUnconfigured: 'No key set', profileUnverified: 'Unverified', profileReady: 'Ready',
    profileCooldown: 'Cooling down', profileInvalid: 'Credential invalid', profileNeedsReauth: 'Reauth needed', profileDisabledTag: 'Disabled',
    profileEnabled: 'Enable', profileDisabled: 'Disable', profileLastTestNone: 'Not tested yet',
    profileSupportedModels: (count: number) => `${count} ${count === 1 ? 'model' : 'models'} verified`,
    profileReplaceCredential: 'Replace key', profileSaveCredential: 'Save key', profileKeyPlaceholder: 'Paste a new API key',
    profileTest: 'Test', profileFetchModels: 'Sync model catalog',
    profileRemove: 'Remove', profileRemovePrimaryHelp: 'This account cannot be removed separately. It can be disabled or signed in again.',
    removeProfileTitle: (label: string) => `Remove account “${label}”?`,
    removeProfileDescription: 'The key and its verification records are deleted. Add it again to join balancing.',
    removeProfileConfirm: 'Remove account',
    profileTestSuccess: (label: string) => `Account “${label}” tested successfully`, profileTestFailed: (label: string) => `Account “${label}” test failed`,
    profileRoutingMode: 'Routing mode', profileRoutingLegacy: 'Primary-account mode (balancing off)', profileRoutingBalanced: 'Load balancing enabled',
    profileRoutingAutomatic: 'Automatic (load balancing)', profileRoutingAutomaticHelp: 'Distribute new requests equally across available accounts.',
    profileRoutingManual: 'Manual (ordered failover)', profileRoutingManualHelp: 'Use the first account until quota or rate limiting makes it unavailable, then use the next.',
    profileRoutingInactive: 'Multi-account routing is not enabled', profileRoutingFailed: 'Could not change routing mode', profileOrderFailed: 'Could not save account order',
    profileMoveUp: 'Move up', profileMoveDown: 'Move down', profileRefreshUsage: 'Refresh quota',
    profileUsageLoading: 'Loading quota…', profileUsageUnavailable: 'Quota unavailable',
    profileUsageUnsupported: 'Quota lookup is not supported for this account',
    profileUsageCredentialUnavailable: 'Login is unavailable; sign in again',
    profileUsageProviderRejected: 'The quota service rejected the request',
    profileUsageProviderUnavailable: 'The quota service or network is unavailable',
    profileUsageInvalidResponse: 'The quota response format is not supported yet',
    profileUsageFiveHour: (value: number) => `5-hour used ${value}%`, profileUsageSevenDay: (value: number) => `Weekly used ${value}%`, profileUsageRefreshAt: (when: string) => `Expected refresh ${when}`,
    profileBalancedHint: 'Requests run on the primary account only; secondary accounts can be configured and tested but never carry traffic.',
    profileBalancedActive: (count: number) => `Balancing enabled: ${count} ${count === 1 ? 'model has' : 'models have'} multiple ready accounts.`,
    profileBalancedPending: 'Enabling balancing requires at least one model with two tested accounts.',
    profileEnableBalanced: 'Enable load balancing', profileDisableBalanced: 'Turn off load balancing',
    profileReadyCandidate: (count: number) => `${count} ${count === 1 ? 'model has' : 'models have'} multi-account candidates`,
    profileNoProfiles: 'No secondary accounts yet. Add a key, test it, then enable balancing.',
    profileDraftSaved: 'Key saved. After it passes a test you can enable balancing under “Routing mode”.',
  },
  shared: {
  ...
  oauthSection: {
    ...
    codexAddAccount: 'Sign in with OpenAI', codexAccountLabel: 'Account name', codexAccountPlaceholder: 'For example: Work', codexDefaultAccount: 'Account',
    codexAddAccountDetail: 'After sign-in, the account is added to the existing OpenAI OAuth connection. Its nickname can be edited later in connection details.',
