...
const zhCopy = {
  detail: {
    ...
    saveEndpoint: '保存服务地址', testing: '测试中…', testConnection: '测试连接', updating: '更新中…',
    updateModels: '更新模型目录', setting: '设置中…', setDefault: '设为默认连接', endpoint: '服务地址',
    credentials: '凭据', dangerZone: '删除连接', deleteRowHelp: '删除后需要重新添加才能再次使用；此操作不可撤销。',
    credentialsHelp: '这个连接调用模型时使用的密钥，只保存在本机。',
    credentialsHelpAccount: '这个连接用账号登录授权，令牌只保存在本机。',
    modelManagementHelp: '选择新会话默认使用的模型，以及哪些模型出现在模型选择器里。',
    advancedHelp: '默认走服务商的地址；用代理或自建服务时改这里，留空恢复默认。',
    defaultConnectionHelp: '新会话默认使用这个连接；每个会话仍可单独切换。',
    defaultConnection: '默认连接', isDefaultConnection: '这个连接已经是默认连接。',
    modelManagement: '模型管理', connectionDefaultModel: '此连接的默认模型', connectionDefaultModelHelp: '新对话选择此连接时优先使用该模型。',
    defaultModelSet: (model: string) => `默认模型已更新 · ${model}`,
    defaultModelSaveFailed: '保存默认模型失败', disconnectAndDelete: '退出账号并删除',
    ...
    enabledModelsHelp: '勾选的模型会出现在模型选择器中。',
    noModels: '暂无可选模型，请先更新模型目录。', defaultModel: '默认',
...
const enCopy: ProviderSettingsCopy = {
  detail: {
    ...
    saveEndpoint: 'Save service URL', testing: 'Testing…', testConnection: 'Test connection', updating: 'Updating…',
    updateModels: 'Update model catalog', setting: 'Setting…', setDefault: 'Set as default', endpoint: 'Service URL',
    credentials: 'Credentials', dangerZone: 'Delete connection', deleteRowHelp: 'You will need to add it again to use it; this cannot be undone.',
    credentialsHelp: 'The key this connection calls models with. It stays on this machine.',
    credentialsHelpAccount: 'This connection is authorized by an account sign-in. The token stays on this machine.',
    modelManagementHelp: 'Pick the model new sessions start on, and which models appear in the model picker.',
    advancedHelp: 'Calls go to the provider by default; change this for a proxy or a self-hosted service, or leave it empty to restore the default.',
    defaultConnectionHelp: 'New sessions start on this connection; each session can still switch.',
    defaultConnection: 'Default connection', isDefaultConnection: 'This connection is already the default.',
    modelManagement: 'Model management', connectionDefaultModel: 'Default model for this connection', connectionDefaultModelHelp: 'Used first when a new chat selects this connection.',
    defaultModelSet: (model: string) => `Default model updated · ${model}`,
    defaultModelSaveFailed: 'Failed to save default model', disconnectAndDelete: 'Sign out and delete',
    ...
    enabledModelsHelp: 'Selected models appear in the model picker.',
    noModels: 'No models are available. Update the model catalog first.', defaultModel: 'Default',
