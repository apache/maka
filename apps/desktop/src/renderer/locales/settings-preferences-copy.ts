...
export type SettingsPreferencesCopy = {
  ...
  general: {
    ...
    autoReviewModelHelp: string;
    saveAutoReviewModelFailed: string;
...
const SETTINGS_PREFERENCES_COPY_BY_LOCALE = {
  'zh-CN': {
    ...
    general: {
      ...
      autoReviewModel: 'Auto review 模型', autoReviewModelHelp: '在 Auto review 模式下，独立审核工具操作的风险与授权。', saveAutoReviewModelFailed: '保存审核模型失败', followDefaultModel: '跟随默认模型', defaultModel: '默认模型', defaultModelHelp: '新任务默认使用的模型。', notSet: '未设置', saveDefaultModelFailed: '保存默认模型失败', defaultPermission: '默认权限模式', defaultPermissionHelp: '新任务的默认模式，也用于 WorkHub。普通任务可单独切换。', saveDefaultPermissionFailed: '保存默认权限模式失败', defaultThinking: '默认思考级别', defaultThinkingHelp: '新任务的思考级别；当前模型不支持所选级别时用模型默认。', followModelDefault: '跟随模型默认', saveDefaultThinkingFailed: '保存默认思考级别失败',
  ...
  'zh-TW': {
    ...
    general: {
      ...
      autoReviewModel: 'Auto review 模型', autoReviewModelHelp: '在 Auto review 模式下，獨立審核工具操作的風險與授權。', saveAutoReviewModelFailed: '儲存審核模型失敗', followDefaultModel: '跟隨預設模型', defaultModel: '預設模型', defaultModelHelp: '新任務預設使用的模型。', notSet: '未設定', saveDefaultModelFailed: '儲存預設模型失敗', defaultPermission: '預設權限模式', defaultPermissionHelp: '新任務的預設模式，也用於 WorkHub。一般任務可單獨切換。', saveDefaultPermissionFailed: '儲存預設權限模式失敗', defaultThinking: '預設思考級別', defaultThinkingHelp: '新任務的思考級別；目前模型不支援所選級別時用模型預設。', followModelDefault: '跟隨模型預設', saveDefaultThinkingFailed: '儲存預設思考級別失敗',
