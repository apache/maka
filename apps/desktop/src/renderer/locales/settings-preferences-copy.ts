...
export type SettingsPreferencesCopy = {
  ...
  general: {
    ...
    defaultToolModeHelp: string;
    toolModeAuto: string;
    toolModeDirect: string;
    toolModeCodeMode: string;
    toolModeCodeModeUnavailable: string;
    saveDefaultToolModeFailed: string;
...
const SETTINGS_PREFERENCES_COPY_BY_LOCALE = {
  zh: {
    ...
    general: {
      ...
      defaultModel: '默认模型', defaultModelHelp: '新任务默认使用的模型。', notSet: '未设置', saveDefaultModelFailed: '保存默认模型失败', defaultPermission: '默认权限模式', defaultPermissionHelp: '新任务默认使用的权限模式；可在任务内随时切换。', saveDefaultPermissionFailed: '保存默认权限模式失败', defaultThinking: '默认思考级别', defaultThinkingHelp: '新任务的思考级别；当前模型不支持所选级别时用模型默认。', followModelDefault: '跟随模型默认', saveDefaultThinkingFailed: '保存默认思考级别失败', defaultToolMode: '默认工具模式', defaultToolModeHelp: '新任务的工具调用方式；仅影响更改设置后新建的任务。', toolModeAuto: '自动（当前等同直接调用）', toolModeDirect: '直接工具调用', toolModeCodeMode: '代码模式', toolModeCodeModeUnavailable: '当前后端不可用', saveDefaultToolModeFailed: '保存默认工具模式失败',
