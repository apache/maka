...
const SETTINGS_PREFERENCES_COPY_BY_LOCALE = {
  ...
  'zh-TW': {
    ...
    general: {
      ...
      autoReviewModel: '自動審核模型', autoReviewModelHelp: '在自動審核模式下，獨立審核工具操作的風險與授權。', saveAutoReviewModelFailed: '儲存審核模型失敗', followDefaultModel: '跟隨預設模型', defaultModel: '預設模型', defaultModelHelp: '新任務預設使用的模型。', notSet: '未設定', saveDefaultModelFailed: '儲存預設模型失敗', defaultPermission: '預設權限模式', defaultPermissionHelp: '新任務的預設模式，也用於 WorkHub。一般任務可單獨切換。', saveDefaultPermissionFailed: '儲存預設權限模式失敗', defaultThinking: '預設思考級別', defaultThinkingHelp: '新任務的思考級別；目前模型不支援所選級別時用模型預設。', followModelDefault: '跟隨模型預設', saveDefaultThinkingFailed: '儲存預設思考級別失敗',
