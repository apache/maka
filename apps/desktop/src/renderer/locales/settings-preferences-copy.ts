...
export type SettingsPreferencesCopy = {
  ...
  general: {
    ...
    followUpMode: string;
    followUpModeHelp: string;
    followUpQueue: string;
    followUpSteer: string;
    saveFollowUpModeFailed: string;
    proxy: string;
    proxyHelp: string;
    enableProxy: string;
...
const SETTINGS_PREFERENCES_COPY_BY_LOCALE = {
  zh: {
    ...
    sections: {
      ...
      chatDefaults: '对话默认', chatDefaultsHelp: '新对话的起始模型、权限模式、思考级别，以及运行中的跟进行为。',
    ...
    general: {
      ...
      defaultModel: '默认模型', defaultModelHelp: '新对话默认使用的模型。', notSet: '未设置', saveDefaultModelFailed: '保存默认模型失败', defaultPermission: '默认权限模式', defaultPermissionHelp: '新对话默认使用的权限模式；可在对话内随时切换。', saveDefaultPermissionFailed: '保存默认权限模式失败', defaultThinking: '默认思考级别', defaultThinkingHelp: '新对话的思考级别；当前模型不支持所选级别时用模型默认。', followModelDefault: '跟随模型默认', saveDefaultThinkingFailed: '保存默认思考级别失败', followUpMode: '跟进行为', followUpModeHelp: 'Maka 运行时，默认把新指令排到下一轮，或立即调整当前运行。Shift+Enter 可对单条消息临时反转。', followUpQueue: '排队', followUpSteer: '引导', saveFollowUpModeFailed: '保存跟进行为失败',
  ...
  en: {
    ...
    sections: {
      ...
      chatDefaults: 'Conversation defaults', chatDefaultsHelp: 'The starting model, permission mode, and thinking level, plus follow-up behavior during a run.',
