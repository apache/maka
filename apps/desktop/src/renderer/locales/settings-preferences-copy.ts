...
export type SettingsPreferencesCopy = {
  ...
  general: {
    ...
    detectingProxy: string;
    noProxyDetected: string;
    noProxyDetectedHelp: string;
    detectAgain: string;
    systemProxy: string;
    environmentProxy: string;
    detectedProxy(source: string, endpoint: string): string;
    detectedProxyHelp: string;
    detectedProxyAuthHelp: string;
    testAndUseDetected: string;
    detectedProxyEnabled: string;
    resetProxy: string;
    resetProxyConfirmTitle: string;
    resetProxyConfirmDescription: string;
    resetProxyConfirm: string;
    resetProxyCancel: string;
    resetProxyComplete: string;
    resetProxyFailed: string;
  };
  about: {
    loadFailed: string;
    loading: string;
...
const SETTINGS_PREFERENCES_COPY_BY_LOCALE = {
  zh: {
    ...
    general: {
      ...
      proxy: '代理服务器', proxyHelp: '为 AI 模型请求配置网络代理', enableProxy: '启用代理服务器', saveNetworkFailed: '保存网络设置失败', proxyProtocol: '代理协议', serverAddress: '服务器地址', port: '端口', proxyAuth: '代理认证', proxyAuthHelp: '需要用户名和密码时开启。', enableProxyAuth: '启用代理认证', username: '用户名', password: '密码', bypassList: '代理白名单', bypassHelp: '这些域名将绕过代理直连，多个用逗号分隔。', autoBypass: (count) => `已自动添加 ${count} 个域名。代理仅作用于 AI 模型请求。`, testing: '测试中…', testCurrent: '测试当前配置', proxyReachable: '代理可达', proxyTestFailed: '代理测试失败', proxyTestError: '代理测试出错', detectingProxy: '正在检测可用代理…', noProxyDetected: '未检测到可直接采用的代理', noProxyDetectedHelp: '如果你使用代理软件，请手动填写它提供的协议、地址和端口，再测试当前配置。', detectAgain: '重新检测', systemProxy: '系统代理', environmentProxy: '环境变量代理', detectedProxy: (source, endpoint) => `检测到${source}：${endpoint}`, detectedProxyHelp: 'Maka 会先测试这个候选代理；测试成功后才会启用并保存。', detectedProxyAuthHelp: '这个代理包含认证信息。为避免暴露密码，请在下方手动启用代理认证并填写凭据。', testAndUseDetected: '测试并采用', detectedProxyEnabled: '已启用检测到的代理', resetProxy: '恢复默认设置', resetProxyConfirmTitle: '恢复代理默认设置？', resetProxyConfirmDescription: '将关闭代理，恢复默认地址、端口和白名单，并删除已保存的代理认证凭据。', resetProxyConfirm: '恢复默认', resetProxyCancel: '取消', resetProxyComplete: '已恢复代理默认设置', resetProxyFailed: '恢复代理默认设置失败',
    },
    about: {
