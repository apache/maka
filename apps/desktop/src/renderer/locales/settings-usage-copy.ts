...
export type UsageSettingsCopy = {
  ...
  incompleteValue(value: string): string; incompleteProjection(unreadable: number, pending: number): string; joinDetails(parts: string[]): string; incompleteCost(count: number): string;
  incompleteTokens(input: number, output: number, count: number): string; incompleteCache(miss: number, read: number, creation: number, count: number): string;
  ...
  tables: {
    ...
    noPricing: string; modelKind: string; toolKind: string; openSession(label: string): string; success: string; error: string; aborted: string; unknown: string; partial(value: number): string;
...
const SETTINGS_USAGE_COPY = {
  zh: {
    ...
    incompleteValue: (value) => `${value} + 未知`, incompleteProjection: (unreadable, pending) => `使用统计投影不完整（无法读取 ${unreadable} 条记录，待修复 ${pending} 个运行）`, joinDetails: (parts) => parts.join('；'),
    incompleteCost: (count) => `${count} 个请求的费用未知`, incompleteTokens: (input, output, count) => `已知输入 ${input} / 输出 ${output}；${count} 个请求的 Token 不完整`,
    incompleteCache: (miss, read, creation, count) => `已知新 ${miss} / 命中 ${read} / 创建 ${creation}；${count} 个请求的 Token 不完整`,
    ...
    tables: {
      ...
      noPricing: '暂无定价覆盖配置', modelKind: '模型', toolKind: '工具', openSession: (label) => `打开 ${label}`, success: '成功', error: '错误', aborted: '已中断', unknown: '未知', partial: (value) => `${value}（部分）`,
  ...
  en: {
    ...
    incompleteValue: (value) => `${value} + unknown`, incompleteProjection: (unreadable, pending) => `Usage projection is incomplete (${unreadable} unreadable ${unreadable === 1 ? 'record' : 'records'}, ${pending} pending ${pending === 1 ? 'repair' : 'repairs'})`, joinDetails: (parts) => parts.join('; '),
    incompleteCost: (count) => `${count} ${count === 1 ? 'request has' : 'requests have'} unknown cost`, incompleteTokens: (input, output, count) => `Known input ${input} / output ${output}; ${count} ${count === 1 ? 'request has' : 'requests have'} incomplete token usage`,
    incompleteCache: (miss, read, creation, count) => `Known new ${miss} / hit ${read} / created ${creation}; ${count} ${count === 1 ? 'request has' : 'requests have'} incomplete token usage`,
    ...
    tables: {
      ...
      noPricing: 'No pricing overrides', modelKind: 'Model', toolKind: 'Tool', openSession: (label) => `Open ${label}`, success: 'Success', error: 'Error', aborted: 'Aborted', unknown: 'Unknown', partial: (value) => `${value} (partial)`,
