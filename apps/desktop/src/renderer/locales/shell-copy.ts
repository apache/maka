...
const SHELL_COPY_BY_LOCALE = {
  ...
  'zh-TW': {
    ...
    sessionRowActions: {
      ...
      bulkDeleteTitle: (count: number) => `刪除選取的 ${count} 個任務？`,
      bulkDeleteDescription: '刪除後無法復原，任務的全部修訂版本都會一併刪除。',
      bulkArchiveTitle: (count: number) => `歸檔選取的 ${count} 個任務？`,
      bulkArchiveDescription: '歸檔後可在「設定 › 活動 › 已歸檔任務」中找回。',
      bulkArchiveLabel: '歸檔',
      bulkDeletedTitle: (count: number) => `已刪除 ${count} 個任務`,
      bulkArchivedTitle: (count: number) => `已歸檔 ${count} 個任務`,
      bulkKeptRestored: (count: number) => `另有 ${count} 個已被恢復，未刪除。`,
      bulkDeleteFailedTitle: '部分任務無法刪除',
      bulkArchiveFailedTitle: '部分任務無法歸檔',
      bulkFailedBody: (count: number) => `還有 ${count} 個未處理成功。`,
      bulkUnverified: '無法確認處理結果，請重新整理後查看。',
      bulkDeleteSubtaskNote: () => '它們的普通子任務不會被刪除，將保留並移入歸檔。',
      bulkDeleteSubtaskNoteUncertain: () =>
        '它們的普通子任務（如有）不會被刪除，將保留並移入歸檔。',
    },
    skillActions: {
