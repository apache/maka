...
const STATIC_COMMAND_KEYWORDS: Record<StaticCommandId, readonly string[]> = {
  'action:new-chat': ['new', 'chat', 'start', '新', '建', '任务', '대화', '채팅'],
  'action:side-chat': [
    'side',
    'chat',
    'btw',
  ...
  'action:new-scheduled-task': ['plan', 'task', 'schedule', 'new', 'create', '计划', '提醒', '新建', '创建', '예약', '일정'],
  'action:open-settings': ['settings', 'preferences', '设置', 'options', '환경설정'],
  'action:keyboard-help': ['shortcuts', 'keyboard', 'help', '快捷键', '帮助', '도움말'],
  'theme:light': ['light', 'theme', '浅色', '主题', '밝은'],
  'theme:dark': ['dark', 'theme', '深色', 'night', '主题', '어두운', '야간'],
  'theme:auto': ['auto', 'system', 'theme', '跟随', '系统', '主题', '자동'],
  'nav:sessions': ['sessions', 'chats', '任务', '会话', '对话', 'left', '세션', '대화'],
  'nav:automations': ['automations', 'plan', 'task', 'schedule', 'cron', '定时任务', '计划', '提醒', '정기 작업', '예약'],
  'nav:skills': ['skills', '技能', '스킬'],
  'nav:mcp': ['mcp', 'server', 'tools', '扩展', '工具', '서버', '도구'],
  'nav:daily-review': ['daily', 'review', 'today', '每日', '回顾', '今天', '오늘'],
  'diag:open-workspace': ['workspace', 'folder', 'open', 'finder', '工作区', '文件夹', '目录', '워크스페이스'],
  'diag:open-project-folder': ['project', 'folder', 'open', 'finder', '项目', '目录', '文件夹'],
  'diag:open-skills': ['skills', 'folder', 'open', 'finder', '技能', '文件夹', '스킬'],
  'diag:export-conversation': ['export', 'markdown', 'copy', 'conversation', '导出', '任务', '剪贴板', 'md', '내보내기', '마크다운'],
  'diag:save-conversation-file': [
    'save',
    'file',
    'markdown',
  ...
  'diag:copy-today-daily-review': ['daily', 'review', 'today', 'copy', 'markdown', '今日', '回顾', '复制', '剪贴板', '데일리 리뷰'],
  'diag:paste-today-daily-review': ['daily', 'review', 'paste', 'composer', '今日', '回顾', '粘贴', '输入框', '데일리 리뷰'],
  'diag:save-today-daily-review': [
    'daily',
    'review',
    'save',
  ...
  'diag:test-network-proxy': ['network', 'proxy', 'test', 'ping', '网络', '代理', '测试', '连接', '诊断', '연결'],
  'diag:open-local-memory': ['memory', 'md', 'open', '记忆', '本地', '编辑', 'edit', '메모리'],
};
...
const KO_STATIC_COMMANDS: Record<StaticCommandId, CommandCopy> = {
  'action:new-chat': {
    label: '새 작업',
    hint: '새 작업 시작',
    group: '동작',
  ...
  'theme:light': { label: '테마 · 라이트', group: '테마' },
  'theme:dark': { label: '테마 · 다크', group: '테마' },
  'theme:auto': { label: '테마 · 시스템 설정 따름', group: '테마' },
  'nav:sessions': { label: '사이드바 · 작업', group: '내비게이션' },
  'nav:automations': { label: '사이드바 · 자동화', group: '내비게이션' },
  'nav:skills': { label: '열기 · Skills', group: '내비게이션' },
  'nav:mcp': { label: '열기 · MCP', group: '내비게이션' },
  'nav:daily-review': { label: '열기 · 데일리 리뷰', group: '내비게이션' },
  'diag:open-workspace': {
    label: '작업 공간 폴더 열기',
    hint: 'Finder',
    group: '진단',
  ...
  'diag:copy-today-daily-review': {
    label: "오늘의 리뷰를 Markdown으로 복사",
    hint: '클립보드에 복사',
    group: '진단',
  },
  'diag:paste-today-daily-review': {
    label: "오늘의 리뷰를 입력창에 붙여 넣기",
    hint: '클립보드를 거치지 않음',
    group: '진단',
  },
  'diag:save-today-daily-review': {
    label: "오늘의 리뷰를 .md 파일로 저장",
    hint: '시스템 저장 대화상자 사용',
    group: '진단',
  },
  'diag:copy-diagnostics': {
    label: '진단 정보 복사',
    platformHint: {
      apple: '⇧⌘D · 민감 정보를 가린 로그 · 클립보드에만',
      other: 'Ctrl+Shift+D · 민감 정보를 가린 로그 · 클립보드에만',
    },
    group: '진단',
  },
  'diag:test-network-proxy': {
    label: '현재 네트워크 프록시 테스트',
    hint: '설정을 열지 않고 진단',
    group: '진단',
  },
  'diag:open-local-memory': {
    label: '로컬 MEMORY.md 열기',
    hint: '시스템 편집기',
    group: '진단',
  },
};
...
const KO_SETTINGS_SECTIONS: Record<SettingsSection, string> = {
  general: '일반',
  appearance: '모양',
  projects: '작업 공간',
  models: '모델',
  subagents: '하위 에이전트',
  usage: '사용량',
  'archived-tasks': '보관된 작업',
  'import-tasks': '작업 가져오기',
  memory: '메모리',
  'daily-review': '데일리 리뷰',
  'bot-chat': '원격 접속',
  search: '웹 검색',
  data: '데이터',
  permissions: '권한과 기능 상태',
  health: '상태 점검',
  about: '앱 정보',
};
...
const SHELL_COPY_BY_LOCALE = {
  ...
  ko: {
    ...
    errors: {
      messageRead: '작업 내용을 일시적으로 사용할 수 없습니다. 잠시 후 다시 시도해 주세요.',
      messageRefresh: '작업 내용을 새로고침하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      openPath: (path: string) => `${path} 열기에 실패했습니다. 잠시 후 다시 시도해 주세요.`,
      workspaceUnavailableTitle: '작업 디렉터리를 사용할 수 없음',
      workspaceUnavailableDescription:
        '작업 디렉터리가 없거나 접근할 수 없습니다. 새 작업에 사용할 올바른 폴더를 선택하세요.',
    },
    chatActions: {
      newConversation: '새 작업',
      sendFailedTitle: '메시지를 보내지 못함',
      sendFailedFallback: '메시지를 보내지 못했습니다. 잠시 후 다시 시도해 주세요.',
      skillInvocationBlockedTitle: 'Skill 호출에 실패해 메시지를 보내지 못함',
      skillInvocationBlockedDescription: (items) => `${items.join(', ')}. 선택을 조정한 뒤 다시 시도해 주세요.`,
      skillInvocationFailedTitle: '일부 Skill을 호출하지 못했습니다',
      skillInvocationFailedDescription: (items) =>
        `${items.join(', ')}. 나머지 Skill은 호출되었습니다.`,
      skillInvocationFailureReason: {
        invalid_name: '잘못된 이름',
        not_found: '찾을 수 없음',
        disabled: '꺼짐',
        host_incompatible: '필요한 도구를 사용할 수 없음',
        resolution_failed: '해석 실패',
        too_many_requests: '서로 다른 Skill 호출 요청이 50개를 넘음',
      },
      responseFailedTitle: '응답 실패',
      responseFailedFallback: '작업 동작을 수행하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      refreshFailedTitle: '작업 새로고침 실패',
      sessionStartFailedTitle: '작업 시작 실패',
      sessionStartFailedFallback: '작업을 시작하지 못했습니다. 잠시 후 다시 시도해 주세요.',
    },
    projectActions: {
      currentProject: '현재 프로젝트',
      readPathFailedTitle: '프로젝트 경로 읽기 실패',
      readPathFailedFallback: '프로젝트 경로를 일시적으로 사용할 수 없습니다. 잠시 후 다시 시도해 주세요.',
      selectDirectoryFailedTitle: '작업 디렉터리 선택 실패',
      selectedPathUnreadable: '선택한 경로가 없거나 읽을 수 없습니다.',
      directorySwitchedTitle: '작업 디렉터리 변경됨',
      projectUpdateFailedTitle: '프로젝트 업데이트 실패',
      projectUpdateFailedFallback: '프로젝트를 업데이트하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      catalogUnavailable: 'Runtime Host를 사용할 수 없음',
      retryCatalog: '불러오기 다시 시도',
      remoteDirectoryTitle: (host: string) => `${host}에 프로젝트 추가`,
      remoteDirectoryBreadcrumbs: '현재 폴더',
      remoteDirectoryHome: '홈',
      remoteDirectoryEmpty: '이 위치에 폴더 없음',
      remoteDirectorySelect: '이 폴더 추가',
      remoteDirectoryCancel: '취소',
      remoteDirectoryRetry: '다시 시도',
      remoteDirectoryLoading: '폴더 불러오는 중…',
      remoteDirectoryShowHidden: '숨김 폴더 표시',
      remoteDirectoryHideHidden: '숨김 폴더 숨기기',
      runtimeHostReadiness: {
        connecting: '연결하는 중',
        reconnecting: '다시 연결하는 중',
        unavailable: '사용할 수 없음',
      },
      openFailedTitle: (path: string) => `${path} 열기 실패`,
      openPathLabels: {
        workspace: '작업 공간 폴더',
        skills: 'Skills 폴더',
        memory: '메모리 폴더',
      ...
      openPathFailures: {
        'unknown-key': '알 수 없는 작업 공간 폴더입니다.',
        'not-allowed': '경로가 Maka에서 열 수 있는 폴더 밖에 있습니다.',
        missing: '폴더가 없습니다.',
        'not-a-directory': '대상이 폴더가 아닙니다.',
        'open-failed': '시스템에서 폴더를 열지 못했습니다.',
        unknown: '폴더를 열지 못했습니다.',
      },
    },
    commandActions: {
      connectionVerified: (name: string) => `연결 확인됨 · ${name}`,
      connectionLatency: (latency: number | string, model?: string) =>
        `지연 시간 ${latency}ms${model ? ` · ${model}` : ''}`,
      connectionTestFailed: (name: string) => `연결 테스트 실패 · ${name}`,
      testErrorTitle: '테스트 오류',
      connectionUnavailable: '연결 테스트를 일시적으로 사용할 수 없습니다. 잠시 후 다시 시도해 주세요.',
      connectionFailures: {
        rateLimit: '계정 또는 모델 서비스의 속도 제한을 초과했습니다. 잠시 후 다시 시도해 주세요.',
        timeout: '요청 시간 초과. 네트워크 또는 프록시를 확인한 뒤 다시 시도해 주세요.',
        auth: '인증 실패. 모델 키, 구독 로그인 또는 인증 정보를 확인한 뒤 다시 시도해 주세요.',
        network: '네트워크 오류. 네트워크 또는 프록시를 확인한 뒤 다시 시도해 주세요.',
        provider: '모델 서비스가 오류를 반환했습니다. 잠시 후 다시 시도해 주세요.',
        unknown: '연결 테스트에 실패했습니다. 잠시 후 다시 시도해 주세요.',
      },
      setDefaultSuccess: (name: string) => `기본값으로 설정됨 · ${name}`,
      setDefaultFailedTitle: '기본값 변경 실패',
      setDefaultFallback: '기본 모델을 변경하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      newConversation: '새 작업',
      conversationCopiedTitle: '작업이 Markdown으로 복사됨',
      lineCount: (lines: number) => `${lines}줄 · Notion / Obsidian / GitHub에 바로 붙여 넣을 수 있음`,
      copyFailedTitle: '복사 실패',
      clipboardUnavailable: '클립보드를 사용할 수 없음',
      conversationSavedTitle: '작업 저장됨',
      saveSummary: (lines: number, fileName: string) => `${lines}줄 · ${fileName} 파일로 저장됨`,
      saveFailedTitle: '저장 실패',
      invalidExport: '내보낼 내용이 올바르지 않습니다',
      writeFailed: '선택한 위치에 쓰지 못했습니다',
      exportFallback: '작업을 내보내지 못했습니다. 잠시 후 다시 시도해 주세요.',
      memoryOpenFailedTitle: 'MEMORY.md 열기 실패',
      openFailedTitle: '열기 실패',
      memoryOpenFallback: 'MEMORY.md를 열지 못했습니다. 잠시 후 다시 시도해 주세요.',
      today: '오늘',
      reviewCopiedTitle: "오늘의 리뷰가 Markdown으로 복사됨",
      reviewSummary: (sessions: number, requests: number) => `작업 ${sessions}개 · 요청 ${requests}개`,
      reviewCopyFallback: "오늘의 리뷰를 사용할 수 없거나 클립보드 접근이 거부되었습니다.",
      reviewPastedTitle: "오늘의 리뷰가 입력창에 추가됨",
      reviewCopied: (label: string) => `${label} 리뷰 복사됨`,
      reviewPasted: (label: string) => `${label} 리뷰가 입력창에 추가됨`,
      reviewSaved: (label: string) => `${label} 리뷰 저장됨`,
      reviewSaveFallback: '데일리 리뷰를 저장하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      pasteFailedTitle: '붙여 넣기 실패',
      reviewUnavailable: "오늘의 리뷰를 일시적으로 사용할 수 없습니다. 잠시 후 다시 시도해 주세요.",
      diagnosticsCopiedTitle: '진단 정보 복사됨',
      diagnosticsCopiedDescription: '내용을 확인한 뒤 이슈 보고서에 붙여 넣으세요',
      clipboardDenied: '클립보드를 사용할 수 없거나 접근이 거부되었습니다',
      networkPassedTitle: '네트워크 프록시 테스트 통과',
      networkFailedTitle: '네트워크 프록시 테스트 실패',
      genericTestFailedTitle: '테스트 실패',
      networkTestFallback: '네트워크 프록시 테스트를 일시적으로 사용할 수 없습니다. 잠시 후 다시 시도해 주세요.',
    },
    sessionRowActions: {
      actionFallback: '작업 동작을 수행하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      flagFailedTitle: '작업 깃발 표시 실패',
      unflagFailedTitle: '깃발 제거 실패',
      archiveFailedTitle: '작업 보관 실패',
      unarchiveFailedTitle: '작업 복원 실패',
      renameFailedTitle: '작업 이름 변경 실패',
      deleteFailedTitle: '작업 삭제 실패',
      currentConversation: '현재 작업',
      deleteTitle: (name: string) => `"${name}" 삭제`,
      deleteDescription:
        '작업과 모든 메시지가 디스크에서 영구적으로 제거됩니다. 이 동작은 되돌릴 수 없습니다.',
      deleteLabel: '삭제',
      cancelLabel: '취소',
      deletedTitle: (name: string) => `${name} 삭제됨`,
      deleteRestoredTitle: (name: string) => `${name}: 복원되어 유지됨`,
      deleteSubtaskNote: () => '일반 하위 작업은 유지되며 보관된 작업으로 이동합니다.',
      deleteSubtaskNoteUncertain: () =>
        '일반 하위 작업이 있으면 유지되며 보관된 작업으로 이동합니다.',
      deletedSubtaskNote: (count: number) =>
        `하위 작업 ${count}개를 보관된 작업으로 이동했습니다`,
      bulkArchiveDescription: '보관된 작업은 설정 › 활동에서 계속 볼 수 있습니다.',
      bulkArchivedTitle: (count: number) => `${count}개 작업을 보관했습니다`,
      bulkArchiveFailedTitle: '일부 작업을 보관하지 못했습니다',
      bulkFailedBody: (count: number) => `그중 ${count}개는 처리되지 않았습니다.`,
    },
    skillActions: {
      refreshSkillsFailedTitle: 'Skills 새로고침 실패',
      refreshSkillsFallback: 'Skills를 새로고침하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      refreshSourcesFailedTitle: 'Skill 소스 새로고침 실패',
      refreshSourcesFallback: 'Skill 소스를 새로고침하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      refreshBundledFailedTitle: '기본 제공 Skills 새로고침 실패',
      refreshBundledFallback: '기본 제공 Skills를 새로고침하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      installBundledFailedTitle: '기본 제공 Skill 설치 실패',
      installBundledFallback: '기본 제공 Skill을 설치하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      installedBundledTitle: '기본 제공 Skill 설치됨',
      installedDescription: (id: string) => `${id}/SKILL.md가 현재 작업 공간에 추가되었습니다.`,
      importSourceFailedTitle: 'Skill 소스 가져오기 실패',
      importSourceFallback: 'Skill 소스를 가져오지 못했습니다. 잠시 후 다시 시도해 주세요.',
      importedSourceTitle: 'Skill 소스 가져오기 완료',
      installFailedTitle: 'Skill 설치 실패',
      installFallback: 'Skill을 설치하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      installedTitle: 'Skill 설치됨',
      previewFailedTitle: 'Skill 업데이트 미리 보기 실패',
      previewFallback: 'Skill 업데이트를 미리 보지 못했습니다. 잠시 후 다시 시도해 주세요.',
      updateFailedTitle: 'Skill 업데이트 실패',
      updateFallback: 'Skill을 업데이트하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      updatedTitle: 'Skill 업데이트됨',
      forceUpdatedTitle: 'Skill을 업데이트로 덮어씀',
      updatedDescription: (id: string) => `${id}/SKILL.md가 소스 라이브러리 버전으로 업데이트되었습니다.`,
      toggleFailedTitle: 'Skill 상태 변경 실패',
      toggleFallback: 'Skill 상태를 변경하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      enabledTitle: 'Skill 켜짐',
      disabledTitle: 'Skill 꺼짐',
      pinnedTitle: 'Skill이 컨텍스트에 고정됨',
      unpinnedTitle: 'Skill 고정 해제됨',
      runtimeDescription: (name: string) => `현재 프로젝트의 ${name} 런타임 상태가 업데이트되었습니다.`,
      deleteFailedTitle: 'Skill 삭제 실패',
      deleteFallback: 'Skill을 삭제하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      deletedTitle: 'Skill 삭제됨',
      deletedDescription: (id: string) => `${id} 항목이 제거되었습니다.`,
      openFailedTitle: 'Skill 열기 실패',
      openFallback: 'Skill을 열지 못했습니다. 잠시 후 다시 시도해 주세요.',
      openFailures: {
        invalid_id: '허용되지 않는 Skill 이름입니다.',
        missing: '해당하는 SKILL.md를 찾을 수 없습니다.',
        blocked_path: 'Skill 경로가 작업 공간의 skills 폴더 밖에 있어 열기가 차단되었습니다.',
        not_file: '대상이 열 수 있는 SKILL.md 파일이 아닙니다.',
        not_directory: '대상이 열 수 있는 폴더가 아닙니다.',
        open_failed: '시스템에서 파일을 열지 못했습니다.',
      },
      sourceFailures: {
        invalid_skill: '올바른 SKILL.md 파일을 선택하세요.',
        already_exists: '소스 라이브러리에 같은 이름의 Skill이 이미 있습니다.',
        blocked_path: '이 파일 경로는 가져올 수 없습니다.',
        write_failed: '소스 라이브러리에 쓰지 못했습니다. 파일 권한을 확인해 주세요.',
        cancelled: '취소되었습니다.',
      },
      installFailures: {
        not_found: '이 Skill 소스를 찾을 수 없습니다.',
        already_exists: '이 작업 공간에 같은 이름의 Skill이 이미 있습니다.',
        blocked_path: '대상 경로에 쓸 수 없습니다.',
        write_failed: '작업 공간에 쓰지 못했습니다. 파일 권한을 확인해 주세요.',
      },
      updateFailures: {
        not_managed: '이 Skill은 관리되는 소스에서 온 것이 아닙니다.',
        source_missing: '소스 라이브러리에서 해당 소스를 찾을 수 없습니다.',
        local_modified:
          '작업 공간의 사본이 수정되었습니다. 업데이트하기 전에 로컬 파일과 소스 파일을 열어 비교하세요.',
        metadata_error: 'Skill 메타데이터가 올바르지 않아 안전하게 업데이트할 수 없습니다.',
        blocked_path: '대상 경로에 쓸 수 없습니다.',
        write_failed: '작업 공간에 쓰지 못했습니다. 파일 권한을 확인해 주세요.',
      },
      previewFailures: {
        not_managed: '이 Skill은 관리되는 소스에서 온 것이 아닙니다.',
        source_missing: '소스 라이브러리에서 해당 소스를 찾을 수 없습니다.',
        metadata_error: 'Skill 메타데이터가 올바르지 않아 안전하게 미리 볼 수 없습니다.',
        blocked_path: '대상 경로를 읽을 수 없습니다.',
        read_failed: 'Skill 내용을 읽지 못했습니다. 파일 권한을 확인해 주세요.',
      },
      deleteFailures: {
        not_found: '현재 작업 공간에서 이 Skill을 찾을 수 없습니다.',
        blocked_path: '이 Skill 경로는 삭제할 수 없습니다.',
        blocked_scope: '프로젝트 Skills는 저장소에서 관리합니다. 대신 프로젝트에서 삭제해 주세요.',
        delete_failed: 'Skill을 삭제하지 못했습니다. 파일 권한을 확인해 주세요.',
      },
      runtimeFailures: {
        not_found: '현재 작업 공간에서 이 Skill을 찾을 수 없습니다.',
        blocked_path: 'Skill 상태 경로에 쓸 수 없습니다.',
        state_error: '이 작업 공간의 Skill 상태 파일이 올바르지 않아 먼저 고쳐야 합니다.',
        write_failed: '현재 프로젝트의 Skill 상태를 쓰지 못했습니다. 파일 권한을 확인해 주세요.',
      },
    },
    sessionSettingsActions: {
      bypassConfirmTitle: '전체 접근으로 전환할까요?',
      bypassConfirmDescription:
        "로컬 도구가 Maka의 보호 계층을 거치지 않고 파일을 직접 읽고 쓰며 네트워크에 직접 접근합니다. 완전히 신뢰하는 작업이나 실행 환경으로 이미 격리된 작업에만 사용하세요.",
      bypassConfirmLabel: '전체 접근 켜기',
      bypassCancelLabel: '자동 유지',
      permissionFailedTitle: '권한 모드 변경 실패',
      permissionFallback: '권한 모드를 변경하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      attachmentIngestBlocked: {
        item_too_large: '첨부 파일 하나가 크기 제한을 넘습니다.',
        items_invalid: '첨부 파일 목록이 올바르지 않습니다. 파일을 다시 선택해 다시 보내 주세요.',
        count_limit: '메시지 하나에 첨부 파일은 최대 8개까지입니다.',
        duplicate_source: '첨부 파일 원본이 중복되었습니다. 같은 파일을 두 번 추가하지 마세요.',
        total_size_exceeded: '첨부 파일 전체 크기가 제한을 넘습니다.',
        source_expired: '첨부 파일 원본이 만료되었거나 올바르지 않습니다. 파일을 다시 선택해 다시 보내 주세요.',
      },
      modelFailedTitle: '모델 변경 실패',
      modelFallback: '모델을 변경하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      thinkingFailedTitle: '사고 수준 변경 실패',
      thinkingFallback: '사고 수준을 변경하지 못했습니다. 잠시 후 다시 시도해 주세요.',
    },
    goalDialog: {
      title: '목표 설정',
      description: 'Maka는 목표를 달성하거나, 불가능하다고 판단하거나, 아래 예산에 도달할 때까지 턴마다 스스로 이어서 진행합니다. 입력창 위에서 언제든지 멈출 수 있습니다.',
      conditionLabel: '완료 조건',
      conditionDescription: '무엇을 완료로 볼지 한 문장으로 적으세요. Maka가 매 턴이 끝날 때마다 확인합니다.',
      conditionPlaceholder: '예: 모든 테스트가 통과하고 lint 경고가 없음',
      maxIterationsLabel: '최대 턴 수',
      maxIterationsDescription: '비워 두면 기본값을 사용합니다.',
      maxIterationsInvalid: (max) => `1부터 ${max}까지의 정수를 입력하거나 비워 두세요.`,
      tokenBudgetLabel: '토큰 예산',
      tokenBudgetDescription: '비워 두면 토큰 상한이 없습니다.',
      tokenBudgetInvalid: (min) => `${min} 이상의 정수를 입력하거나 비워 두세요.`,
      cancel: '취소',
      close: '닫기',
      submit: '시작',
      failedFallback: '목표를 설정하지 못했습니다. 다시 시도해 주세요.',
      statusLabels: {
        active: '진행 중',
        waiting: '대기 중',
        paused: '일시 중지됨',
        ...
        cleared: '지워짐',
        stalled: '정체됨',
        budget_limited: '토큰 예산 도달',
        max_iterations: '최대 턴 수 도달',
      },
      reconciledMatching: (condition, status) =>
        `다시 연결한 뒤의 현재 목표: “${condition}” (${status}). 요청과 일치하지만, 중단된 요청이 실제로 반영되었다는 뜻은 아닙니다.`,
      reconciledDifferent: (condition, status) =>
        `다시 연결한 뒤의 현재 목표: “${condition}” (${status}). 이 요청과 다릅니다.`,
      reconciledNoGoal: '다시 연결한 뒤의 현재 상태: 목표를 찾을 수 없습니다.',
      reconciliationUnavailable: '연결이 끊겨 현재 목표를 아직 확인할 수 없습니다. 닫았다가 다시 열어 확인해 주세요. 이 대화상자는 두 번 제출되지 않습니다.',
    },
    errorBoundary: {
      copyPending: '복사하는 중…',
      copied: '복사됨',
      copyFailed: '복사 실패',
      copyReport: '진단 정보 복사',
      title: 'Maka 렌더러가 비정상 종료되었습니다',
      description:
        '처리되지 않은 React 오류를 포착했습니다. 다시 시도해 이 오류 화면을 닫거나, 다시 불러와 창 전체를 새로고침해 주세요. 이슈를 전달하기 전에 진단 정보를 복사하세요.',
      retry: '다시 시도',
      reload: '다시 불러오기',
      clipboardFailure: '클립보드를 사용할 수 없거나 접근이 거부되었습니다. 잠시 후 다시 시도해 주세요.',
    },
    commandPalette: {
      label: '명령 팔레트',
      searchLabel: '명령 팔레트 검색',
      placeholder: '명령, 설정, 작업 검색…',
      closeLabel: '명령 팔레트 닫기',
      resultsLabel: '명령 팔레트 결과',
      emptyTitle: '일치하는 명령 없음',
      emptyDescription: '다른 검색어를 입력하거나 Esc를 눌러 닫으세요.',
      selectHint: '선택',
      runHint: '실행',
      closeHint: '닫기',
      current: '현재',
      ...
      staticKeywords: STATIC_COMMAND_KEYWORDS,
      commands: KO_STATIC_COMMANDS,
      settingsSections: KO_SETTINGS_SECTIONS,
      permissionModes: {
        explore: {
          label: '권한 · 읽기 전용',
          hint: '직접 읽고 검색합니다. 쓰기와 네트워크 접근은 확인을 받습니다',
        },
        ask: {
          label: '권한 · 자동',
          hint: "Maka의 보호 계층 안에서 실행합니다. 현재 권한을 넘어서는 동작은 먼저 묻습니다",
        },
        bypass: {
          label: '권한 · 전체 접근',
          hint: "Maka의 보호 계층을 거치지 않고 파일과 네트워크에 직접 접근합니다",
        },
      },
      settingsCommand: (section: string) => `설정 · ${section}`,
      testDefaultConnection: (name: string) => `기본 연결 테스트 · ${name}`,
      setDefaultConnection: (name: string) => `기본값으로 설정 · ${name}`,
      testConnection: (name: string) => `연결 테스트 · ${name}`,
      settingsKeywords: (section: SettingsSection, label: string) => [section, label, 'settings', '설정'],
      permissionKeywords: (mode: PermissionMode) => [mode, 'permission', 'mode', '권한', '모드'],
      connectionKeywords: (action: 'default' | 'test', name: string, providerType: string) => [
        action,
        'connection',
        '연결',
      ...
      sections: [
        {
          ...
          rows: [
            {
              keys: ['⌘', 'K'],
              description: '명령 팔레트 열기(작업, 설정, 테마 등)',
            },
            { keys: ['?'], description: '이 단축키 패널 열기 또는 닫기' },
            { keys: ['⌘', 'N'], description: '새 작업 만들기' },
            { keys: ['⌘', ','], description: '설정 열기' },
            {
              keys: ['⌘', 'Shift', 'D'],
              description: '현재 컨텍스트의 진단 정보 복사',
            },
            { keys: ['Esc'], description: '현재 대화상자 닫기' },
          ],
        },
        {
          heading: '입력창',
          rows: [
            { keys: ['Enter'], description: '메시지 보내기' },
            { keys: ['Shift', 'Enter'], description: '줄 바꿈 넣기' },
            {
              keys: ['Alt', 'Enter'],
              description: '줄 바꿈 넣기(대체 키)',
            },
          ],
        },
        {
          ...
          rows: [
            {
              keys: ['Tab'],
              description: '작업과 내비게이션 사이에서 포커스 이동',
            },
            {
              keys: ['↑', '↓'],
              description: '포커스된 작업 사이 이동',
            },
            {
              keys: ['Home', 'End'],
              description: '목록의 맨 위 또는 맨 아래로 이동',
            },
            { keys: ['Enter'], description: '포커스된 작업 열기' },
            {
              keys: ['Delete'],
              description: '삭제 확인 창 열기(확인 없이 삭제하지 않음)',
            },
            {
              keys: ['F'],
              description: '작업 검색으로 포커스 이동(Esc를 누르면 지워짐)',
            },
          ],
        },
        {
          ...
          rows: [
            {
              keys: ['Tab'],
              description: '도구 활동과 복사 버튼으로 포커스 이동',
            },
            {
              keys: ['Space', 'Enter'],
              description: '도구 호출 펼치기 또는 접기',
            },
          ],
        },
        {
          heading: '패널 크기 조절',
          rows: [
            { keys: ['Tab'], description: '왼쪽 또는 오른쪽 분할선으로 포커스 이동' },
            {
              keys: ['←', '→'],
              description: '작업 목록 너비 조절(±10px)',
            },
            {
              keys: ['Shift', '←', '→'],
              description: '빠르게 조절(±50px)',
            },
            {
              keys: ['Home', 'End'],
              description: '최소 또는 최대 너비로 바로 이동',
            },
          ],
        },
      ],
    ...
    chrome: {
      windowActions: '창 바로 가기',
      searchConversations: '작업 검색',
      expandSidebar: '사이드바 펼치기',
      collapseSidebar: '사이드바 접기',
      newTask: '새 작업',
      expandWorkbar: '작업 워크바 펼치기',
      collapseWorkbar: '작업 워크바 접기',
      workspaceActions: '작업 공간 동작',
    },
    app: {
      returnToWorkHub: 'WorkHub로 돌아가기',
      loadingWorkbarLabel: '작업 워크바 불러오는 중',
      loadingWorkbar: '작업 워크바 불러오는 중…',
      useSkillPrompt: (skillName: string) => `${skillName} Skill 사용: `,
      newConversation: '새 작업',
      compactSuccessTitle: '컨텍스트 압축됨',
      compactSuccessDescription: '이전 컨텍스트를 체크포인트 요약으로 대체했습니다.',
      compactStartedTitle: '컨텍스트 압축하는 중',
      compactStartedDescription: '이전 컨텍스트를 체크포인트로 요약하는 중입니다.',
      compactUnchangedTitle: '압축할 내용 없음',
      compactUnchangedDescription: '작업이 이미 최신 체크포인트를 사용하고 있습니다.',
      compactErrorTitle: '압축 실패',
      compactErrorFallback: '작업을 압축하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      slashCommands: {
        compact: { name: '컨텍스트 압축', description: '현재 작업은 유지하면서 이전 기록을 압축합니다' },
        graph: { name: 'Graph 사용', description: 'Graph를 확인하거나 전환하거나 한 번 실행합니다' },
        side: { name: '사이드 채팅 열기', description: '사이드 패널에서 특정 주제를 시작합니다' },
        swarm: { name: 'Swarm 사용', description: 'Swarm을 확인하거나 전환하거나 한 번 실행합니다' },
      },
      sideChatUnavailableTitle: '사이드 채팅을 아직 사용할 수 없음',
      sideChatUnavailableDescription:
        '/side를 사용하기 전에 메인 작업에서 메시지를 보내세요.',
      sideChatContextPendingTitle: '대기 중인 컨텍스트를 먼저 처리하세요',
      sideChatContextPendingDescription:
        '입력창에 아직 첨부 파일, 인용, 파일 멘션이 있습니다. /side를 사용하기 전에 보내거나 제거하세요.',
      resumeStartedTitle: '이 턴을 이어서 진행하는 중',
      resumeStartedDescription: '마지막으로 완료된 실행 경계부터 이어서 진행합니다',
      resumeFailedTitle: '이어서 진행 실패',
      resumeFailedFallback: '이 턴을 이어서 진행하지 못했습니다. 작업 상태를 확인한 뒤 다시 시도해 주세요.',
      goalClearFailedTitle: '목표 중지 실패',
      goalClearFailedFallback: '목표가 아직 실행 중일 수 있습니다. 지금 다시 시도해 주세요.',
      goalPauseFailedTitle: '목표 일시 중지 실패',
      goalPauseFailedFallback: '목표가 아직 계속 진행 중일 수 있습니다. 지금 다시 시도해 주세요.',
      goalResumeFailedTitle: '목표 재개 실패',
      goalResumeFailedFallback: '목표가 아직 일시 중지된 상태입니다. 다시 시도해 주세요.',
      appearanceLoadErrorTitle: '모양 설정 불러오기 실패',
      appearanceLoadErrorFallback: '모양 설정을 일시적으로 사용할 수 없습니다. 잠시 후 다시 시도해 주세요.',
      memoryRefreshErrorTitle: '로컬 메모리 상태 새로고침 실패',
      memoryLoadErrorTitle: '로컬 메모리 상태 불러오기 실패',
      memoryErrorFallback: '로컬 메모리 상태를 새로고침하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      openModelSettings: '설정 열기 · 모델',
      configureModelsOnHost: (hostName: string) =>
        `작업을 시작하기 전에 ${hostName}에서 모델 연결을 설정하세요.`,
      sidebarCollapsed: '사이드바가 접혀 있음',
      resizeConversationList: '작업 목록 크기 조절',
      skipErrorTitle: '온보딩 건너뛰기 실패',
      tryAgainLater: '잠시 후 다시 시도해 주세요.',
      loading: '불러오는 중',
      goToModels: '모델로 이동',
      boundaryUnreadableTitle: '이 작업의 권한 읽기 실패',
      boundaryUnreadableDetail:
        '권한을 읽을 수 있을 때까지 여기에 입력할 수 없습니다. 다시 시도하거나 다른 작업으로 전환해 주세요.',
      boundaryUnreadableRetry: '다시 시도',
      boundaryUnreadableRetrying: '다시 시도하는 중…',
      permissionModeStreaming:
        '이 작업의 응답을 스트리밍하는 중입니다. 완료될 때까지 기다린 뒤 권한 모드를 변경하세요.',
      permissionModeRunning: '이 작업이 실행 중입니다. 완료될 때까지 기다린 뒤 권한 모드를 변경하세요.',
      permissionModeWaiting: '도구 호출이 확인을 기다리고 있습니다. 먼저 응답한 뒤 권한 모드를 변경하세요.',
      modeChangeLoading: '이 작업을 아직 불러오는 중입니다. 잠시 후 모드를 변경할 수 있습니다.',
      modeChanging: '모드를 변경하는 중입니다. 완료될 때까지 기다린 뒤 계속하세요.',
      modeChangeStreaming: '이 작업의 응답을 스트리밍하는 중입니다. 완료될 때까지 기다린 뒤 모드를 변경하세요.',
      modeChangeRunning: '이 작업이 실행 중입니다. 완료될 때까지 기다린 뒤 모드를 변경하세요.',
      modeChangeWaiting: '도구 호출이 확인을 기다리고 있습니다. 먼저 응답한 뒤 모드를 변경하세요.',
      goalTurnActive:
        '목표는 다음 턴부터 적용됩니다. 이 턴이 끝날 때까지 기다린 뒤 설정하세요.',
      planModeFailedTitle: 'Plan Mode 변경 실패',
      planModeFallback: 'Plan Mode를 변경하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      orchestrationModeFailedTitle: '오케스트레이션 모드 변경 실패',
      orchestrationModeFallback: '오케스트레이션 모드를 변경하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      planModeExitPendingTitle: '현재 계획을 포기할까요?',
      planModeExitPendingDescription: (title: string) =>
        `“${title}” 계획이 아직 승인되지 않았습니다. Plan Mode를 나가면 기록은 보존되고 이 계획은 포기됨으로 표시됩니다.`,
      planModeExitConfirm: '포기하고 나가기',
      planModeExitCancel: '계속 계획하기',
      planModeExecutionActiveTitle: '계획이 아직 실행 중입니다',
      planModeExecutionActiveDescription: '계획을 수정하려면 진행 중인 실행을 중단한 뒤 Plan Mode에 들어가세요.',
      swarmModeEnabledTitle: 'Swarm Mode 켜짐',
      swarmModeDisabledTitle: 'Swarm Mode 꺼짐',
      swarmModeStatusDescription: '/swarm on, /swarm off를 사용하거나, 한 턴에만 쓰려면 /swarm <작업>을 사용하세요.',
      graphModeEnabledTitle: 'Graph Mode 켜짐',
      graphModeDisabledTitle: 'Graph Mode 꺼짐',
      graphModeStatusDescription: '/graph on, /graph off를 사용하거나, 한 턴에만 쓰려면 /graph <작업>을 사용하세요.',
      graphHistoryTitle: 'Graph 기록',
      graphHistoryDescription: 'Agent Graph 패널의 실행 메뉴에서 기록을 확인하세요.',
      resizeWorkbar: '작업 워크바 크기 조절',
    },
  },
