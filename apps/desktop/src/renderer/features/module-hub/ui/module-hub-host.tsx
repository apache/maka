...
import { ComputerHistoryPage } from './computer-history-page.js';

/** Selects and mounts exactly one Module Hub leaf for the Shell selection. */
export function ModuleHubHost(props: { onOpenSettings(): void; isObscured: boolean }) {
  return <ModuleHubHostView model={useModuleHubHostModel()} {...props} />;
}

/** Environment-free view seam for focused tests and Storybook. */
export function ModuleHubHostView({ model, onOpenSettings, isObscured }: {
  model: ModuleHubHostModel;
  onOpenSettings(): void;
  isObscured: boolean;
}) {
  ...
  if (route === 'computer-history') {
    return <ComputerHistoryPage onCreateDraft={model.createHistoryDraft} onOpenSettings={onOpenSettings} isObscured={isObscured} />;
  }
