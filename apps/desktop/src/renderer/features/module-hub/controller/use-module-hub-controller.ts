...
import { useComputerHistoryDraft, type HistoryDraftHostInput } from './use-computer-history-draft.js';
import {
...
export interface ModuleHubHostModel {
  ...
  readonly createHistoryDraft: (text: string) => void;
}
...
export interface UseModuleHubControllerInput {
  ...
  readonly openSession: (sessionId: string) => void;
  readonly historyDraft: HistoryDraftHostInput;
...
export function useModuleHubController(
  ...
  const createHistoryDraft = useComputerHistoryDraft({
    ...input.historyDraft,
    selection: input.selection,
  });
