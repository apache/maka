...
`module-hub` owns the Desktop renderer behavior behind Extensions, Automations,
...
- local Computer History timeline, detail, privacy controls, and draft intents;
- selection and header composition for Skills, MCP, Scheduled Tasks, Daily
  Review, and the standalone Computer History page.
...
Computer History always uses the local Desktop bridge, independently of the
selected/default remote Runtime Host. Its analysis model is read from the local
Host's Daily Review configuration. `status()` is independent of `timeline()` so
corrupt history cannot remove access to collection controls or deletion.
`applications(bundleIds)` forwards batches of at most 32 IDs to the same local
bridge and preserves native display names and nullable PNG icons. Resolution
and caching belong to the backend; the adapter does not infer brands or consult
remote Hosts.
...
- Daily Review paste captures the active Composer before its first await and
  ...
  feedback.
- Computer History exposes `createHistoryDraft(text)` to its page. Its handoff
  controller lives under the Provider and receives only Shell navigation,
  Composer, and local-target selection capabilities through `historyDraft`.
  It returns to an existing local Composer or selects a local new-task
  destination, preserves its draft, and appends after that destination commits.
  Navigation, Session-selection, target, and unmount fences abandon stale
  continuations. This action never sends, creates a Session, or schedules a task.
