...
export interface TaskEntryControllerCommands {
  ...
  selectLocalTarget(): TaskEntryTarget | undefined;
  addProject(): void;
...
export function useTaskEntryController(
  ...
  const selectLocalTarget = useCallback((): TaskEntryTarget | undefined => {
    if (!localHost) return undefined;
    const projectId = resolveProjectSelection(
      localHost,
      projectSelections.get(localHost.profile.id),
    );
    if (projectId === undefined) return undefined;
    if (projectId === null) selectNoProject(localHost);
    else selectProject(localHost, projectId);
    return { profileId: localHost.profile.id, hostId: localHost.hostId, projectId };
  }, [localHost, projectSelections, selectNoProject, selectProject]);
