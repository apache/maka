...
- `selectLocalTarget()` synchronously selects the ready local Host's remembered
  eligible workspace and returns its target, or leaves selection unchanged when
  unavailable. Computer History uses this command to prepare a local draft
  without exposing the Workspace Picker projection to the Shell or opening a
  directory dialog.
