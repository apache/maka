...
function PricingEditorDialog(props: {
  ...
  const catalogInvalid = submitAttempts > 0 && validation.errors.modelKey !== undefined;
  // Typeahead exposes its field ref, but no native input props. Apply Pricing's
  // required/error semantics to the combobox before the dialog focuses errors.
  const bindCatalogField = useCallback((field: HTMLDivElement | null) => {
    const input = field?.querySelector('[role="combobox"]');
    if (!input) return;
    input.setAttribute('aria-required', 'true');
    if (catalogInvalid) input.setAttribute('aria-invalid', 'true');
