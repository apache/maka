...
const AGENT_GRAPH_PANEL_COPY = {
  'zh-CN': {
    ...
    wait: (operator) => waitReason(operator, ZH_CN_WAIT),
  },
  'zh-TW': {
    title: 'Agent Graph',
    ...
    wait: (operator) => waitReason(operator, ZH_TW_WAIT),
  },
  en: {
    title: 'Agent Graph',
    ...
    wait: (operator) => waitReason(operator, EN_WAIT),
  },
...
interface WaitPhrases {
  inputRoute(upstreamOperatorIds: readonly string[]): string;
  activationMissing(operatorId: string): string;
  settle(operatorId: string): string;
  moreWaits(count: number): string;
  omittedWaits(count: number): string;
  omittedReadiness(count: number): string;
}
...
function waitReason(
  ...
  const reason = wait
    ? wait.kind === 'input_route'
      ? phrases.inputRoute(wait.upstreamOperatorIds)
      : wait.kind === 'activation_missing'
        ? phrases.activationMissing(wait.operatorId)
        : phrases.settle(wait.operatorId)
    : undefined;
  const moreWaits = Math.max(0, waits.length - 1);
  const parts = [
    reason,
    ...(moreWaits > 0 ? [phrases.moreWaits(moreWaits)] : []),
    ...(operator.omitted.readinessWaits > 0 ? [phrases.omittedWaits(operator.omitted.readinessWaits)] : []),
    ...(operator.omitted.readiness > 0 ? [phrases.omittedReadiness(operator.omitted.readiness)] : []),
  ].filter(Boolean);
  return parts.length > 0 ? parts.join(' · ') : undefined;
}

const EN_WAIT: WaitPhrases = {
  inputRoute: (ids) => `Waiting for input from ${ids.join(', ')}`,
  activationMissing: (id) => `Waiting for ${id} activation`,
  settle: (id) => `Waiting for ${id} to settle`,
  moreWaits: (count) => `${count} more wait${count === 1 ? '' : 's'}`,
  omittedWaits: (count) => `${count} wait${count === 1 ? '' : 's'} omitted`,
  omittedReadiness: (count) => `${count} readiness check${count === 1 ? '' : 's'} omitted`,
};

const ZH_CN_WAIT: WaitPhrases = {
  inputRoute: (ids) => `等待 ${ids.join('、')} 的输入`,
  activationMissing: (id) => `等待 ${id} activation`,
  settle: (id) => `等待 ${id} 结束`,
  moreWaits: (count) => `另有 ${count} 项等待条件`,
  omittedWaits: (count) => `${count} 项等待条件已省略`,
  omittedReadiness: (count) => `${count} 项 readiness 检查已省略`,
};

const ZH_TW_WAIT: WaitPhrases = {
  inputRoute: (ids) => `等待 ${ids.join('、')} 的輸入`,
  activationMissing: (id) => `等待 ${id} activation`,
  settle: (id) => `等待 ${id} 結束`,
  moreWaits: (count) => `另有 ${count} 項等待條件`,
  omittedWaits: (count) => `${count} 項等待條件已省略`,
  omittedReadiness: (count) => `${count} 項 readiness 檢查已省略`,
};
