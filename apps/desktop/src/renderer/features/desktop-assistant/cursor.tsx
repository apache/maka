...
export function AssistantCursor({ cursor }: { cursor: NonNullable<DesktopAssistantSnapshot['cursor']> }) {
  ...
  useLayoutEffect(() => {
    ...
    const nearestAngle = (angle: number, previous: number) => previous + ((angle - previous + 180) % 360 + 360) % 360 - 180;
    if (matchMedia('(prefers-reduced-motion: reduce)').matches) {
      heading.current = nearestAngle(Math.atan2(dy, dx) * 180 / Math.PI, heading.current);
    ...
    const keyframes = Array.from({ length: 61 }, (_, index) => {
      ...
      const vx = 3 * u ** 2 * (a.x - from.x) + 6 * u * t * (b.x - a.x) + 3 * t ** 2 * (x - b.x);
      const vy = 3 * u ** 2 * (a.y - from.y) + 6 * u * t * (b.y - a.y) + 3 * t ** 2 * (y - b.y);
      ...
      const angle = startHeading + (tangent - startHeading) * turn ** 2 * (3 - 2 * turn);
      const speed = 16 * time ** 2 * (1 - time) ** 2;
      headings.push({ offset: time, transform: `rotate(${angle}deg) scale(${1 + 0.05 * speed}, ${1 - 0.03 * speed})` });
    ...
    const animations = [element.animate(keyframes, timing), arrow.animate(headings, timing)];
    return () => animations.forEach((animation) => animation.cancel());
  }, [x, y, durationMs]);

  return <div ref={ref} className={`desktopAssistantCursor ${cursor.clicking ? 'isClicking' : ''}`} aria-hidden="true">
    <div ref={headingRef} className="desktopAssistantCursorHeading" style={{ transform: 'rotate(-35deg)' }}>
      <svg className="desktopAssistantCursorArrow" viewBox="-34 -20 48 40" fill="none">
        <path d="M-1.5-2.5Q2.5 0-1.5 2.5L-22 13Q-26 15-25 10L-22 1Q-21.5 0-22-1L-25-10Q-26-15-22-13Z" fill="currentColor" stroke="white" strokeOpacity=".95" strokeWidth="2.5" strokeLinejoin="round" />
      </svg>
    </div>
  </div>;
}
