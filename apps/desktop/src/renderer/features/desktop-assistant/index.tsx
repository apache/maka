...
const copy = {
  en: { title: 'Maka assistant', model: 'Assistant model', chooseModel: 'Choose a model', placeholder: 'Ask a question, or get something done in Maka…', hint: 'Try “Switch the interface to English”', stop: 'Stop', close: 'Hide', send: 'Send', undo: 'Undo last change', thinking: 'Thinking…', acting: 'Working in your app…', paused: 'Paused · you have control', idle: 'Ready', completed: 'Done', error: 'Could not finish' },
  'zh-CN': { title: 'Maka 助手', model: '助手模型', chooseModel: '选择模型', placeholder: '问个问题，或让我帮你操作 Maka…', hint: '试试「把界面语言切换为英文」', stop: '停止', close: '收起', send: '发送', undo: '撤销上次修改', thinking: '思考中…', acting: '正在操作界面…', paused: '已暂停 · 由你接管', idle: '准备就绪', completed: '已完成', error: '未能完成' },
  'zh-TW': { title: 'Maka 助手', model: '助手模型', chooseModel: '選擇模型', placeholder: '問個問題，或讓我幫你操作 Maka…', hint: '試試「把介面語言切換為英文」', stop: '停止', close: '收起', send: '傳送', undo: '復原上次修改', thinking: '思考中…', acting: '正在操作介面…', paused: '已暫停 · 由你接管', idle: '準備就緒', completed: '已完成', error: '未能完成' },
...
export function DesktopAssistantRoot() {
  ...
  useEffect(() => {
    let expected: { x?: number; y?: number; key?: string; wheel?: boolean; until: number; moved: boolean } | undefined;
    ...
    const takeover = (event: Event) => {
      ...
      if (event.target instanceof Element && event.target.closest('.desktopAssistant,.desktopAssistantLauncher')) return;
      if (event instanceof WheelEvent && expected?.wheel && performance.now() <= expected.until) { expected = undefined; return; }
  ...
  useEffect(() => {
    if (snapshot.open && snapshot.expanded && !['acting', 'thinking'].includes(snapshot.phase)) input.current?.focus();
  ...
  return <>
    {!snapshot.open && <div className="desktopAssistantLauncher" role="group" aria-label={t.title}>
      {busy && <span role="status" className="desktopAssistantStatus">{t[snapshot.phase]}</span>}
      <IconButton icon={<MessageCircle size={18} />} label={`${t.title} (⌘⇧K / Ctrl+Shift+K)`} variant="secondary" onPointerDown={(event) => event.preventDefault()} onClick={() => call(bridge.open())} />
      {busy && <IconButton icon={<Square size={14} />} label={t.stop} variant="ghost" onPointerDown={(event) => event.preventDefault()} onClick={() => call(bridge.stop())} />}
    </div>}
    {snapshot.open && <section ref={panel} className={`desktopAssistant ${snapshot.expanded ? '' : 'desktopAssistantCompact'}`} aria-label={t.title}>
      <div className="desktopAssistantHeader">
        ...
        {!snapshot.expanded && <IconButton variant="ghost" label={t.title} icon={<MessageCircle size={16} />} onPointerDown={(event) => event.preventDefault()} onClick={() => call(bridge.expand())} />}
        <IconButton variant="ghost" label={t.close} icon={<X size={16} />} onPointerDown={(event) => event.preventDefault()} onClick={() => call(bridge.close())} />
      </div>
      {snapshot.expanded && <>
