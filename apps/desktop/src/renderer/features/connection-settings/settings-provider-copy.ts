...
const zhCapabilitiesCopy = {
  ...
  thinkingEffortHelp: '这些选项会出现在对话的思考强度选择器中。仅勾选服务商支持的强度；不勾选时使用已有模型资料。',
  thinkingUndeclared: '使用模型自带选项',
  thinkingSelectedCount: (count: number) => `可选 ${count} 种强度`,
  visionInput: '发送图片给模型',
  visionInputHelp: '按服务商和模型资料判断是否发送图片；缺少资料时不发送。手动选择“允许发送”前，请确认此服务商支持该模型接收图片。',
  visionDefaultOption: (supported: boolean | undefined) =>
    supported === undefined ? '按模型资料设置' : supported ? '按模型资料：允许发送' : '按模型资料：不发送',
  visionEnabledOption: '允许发送',
  visionDisabledOption: '不发送',
  contextWindow: '上下文窗口',
  contextWindowHelp: '服务商提供的模型容量；留空使用已知资料。',
  compactionThreshold: '自动压缩阈值',
  compactionThresholdHelp: '达到此 token 数时压缩；留空不主动压缩。',
  maxOutputTokens: '最大输出',
  maxOutputTokensHelp: '单次回复的 token 上限；留空使用模型默认值。',
  fastMode: 'Fast 模式',
...
const zhTwCapabilitiesCopy = {
  ...
  thinkingEffort: '可選思考強度',
  thinkingEffortHelp: '這些選項會出現在對話的思考強度選擇器中。僅勾選服務商支援的強度；不勾選時使用既有模型資料。',
  thinkingUndeclared: '使用模型內建選項',
  thinkingSelectedCount: (count: number) => `可選 ${count} 種強度`,
  visionInput: '傳送圖片給模型',
  visionInputHelp: '依服務商和模型資料判斷是否傳送圖片；缺少資料時不傳送。手動選擇「允許傳送」前，請確認此服務商支援該模型接收圖片。',
  visionDefaultOption: (supported: boolean | undefined) =>
    supported === undefined ? '依模型資料設定' : supported ? '依模型資料：允許傳送' : '依模型資料：不傳送',
  visionEnabledOption: '允許傳送',
  visionDisabledOption: '不傳送',
  contextWindow: '上下文視窗',
  contextWindowHelp: '服務商提供的模型容量；留空使用已知資料。',
  compactionThreshold: '自動壓縮門檻',
  compactionThresholdHelp: '達到此 token 數時壓縮；留空不主動壓縮。',
  maxOutputTokens: '最大輸出',
  maxOutputTokensHelp: '單次回覆的 token 上限；留空使用模型預設值。',
  fastMode: 'Fast 模式',
...
const enCapabilitiesCopy = {
  ...
  maxOutputTokensHelp: 'Output token limit per reply. Leave empty to use the model default.',
  fastMode: 'Fast mode',
