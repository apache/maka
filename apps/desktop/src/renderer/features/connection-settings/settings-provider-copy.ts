...
const zhTwCopy = {
  detail: {
    ...
    deleteFailed: '刪除模型連線失敗', credentialReadFailed: '讀取模型憑證狀態失敗', refreshFailed: '重新整理模型連線失敗',
    saveFailed: '儲存模型連線失敗', saveModelsFailed: '儲存啟用模型失敗',
    modelKey: '模型金鑰', pasteModelKey: '貼上模型金鑰', getModelKey: '取得模型金鑰', saving: '儲存中…',
    advancedRequest: '高階請求設定', advancedRequestHelp: '為這個連線的 HTTP 請求新增請求頭和額外 JSON 請求體。請求頭值作為憑證儲存在本機。',
    ...
    oauthRetiredDetail:
      '這條連線使用的登入方式已從 Maka 移除，無法再登入，也無法用於對話。改用 Anthropic API Key 連線即可繼續使用 Claude 模型；刪除這條連線會一併清除本機儲存的登入憑證。',
    credentialLoadingDetail: '正在讀取模型憑證狀態，讀取完成前暫不測試連線或重新整理模型。',
    credentialUnknownDetail: '模型憑證狀態暫時沒重新整理成功，已避免把未知狀態顯示成未登入或未設定。',
    testConnection: '測試連線',
    ...
    endpointCredentialsMasked: '已儲存的地址內嵌憑證，編輯時預設隱藏',
    modelManagement: '模型',
    ...
    visionToken: '視覺', thinkingToken: '思考', contextToken: (value: string) => `${value} 上下文`,
    noModels: '暫無可選模型，請先更新模型目錄。',
    keySet: '已設定', statusLoading: '正在讀取狀態', credentialUnknown: '憑證狀態未知', keyMissing: '尚未設定金鑰',
    keyTroubleshooting: '模型金鑰 / 服務地址 / 代理設定', endpointTroubleshooting: '本地服務 / 服務地址 / 代理設定', oauthTroubleshooting: 'OAuth 登入 / 代理設定',
    unknownDescription: (provider: string) => `該連線使用的 provider「${provider}」在目前版本未註冊。設定和憑證會保留，切回支援它的版本即可繼續使用。`,
    deleteProviderTitle: (name: string) => `刪除供應商 ${name}？`,
    deleteConnectionTitle: (name: string) => `刪除模型連線 ${name}？`,
    deleteUnknownDescription: '刪除後，支援該 provider 的其他版本也無法恢復這條連線及其憑證。',
    deleteDescription: (isDefault: boolean, disconnectsAccount: boolean) => [
      disconnectsAccount
        ? '這會退出本機帳號、刪除 OAuth 憑證和模型連線；重新整理後不會自動重新建立。'
        : '這會刪除模型連線及其本機憑證；如需再次使用，需要重新新增。',
      isDefault
    ...
    authTroubleshooting: (value: string) => `驗證失敗，請確認 ${value} 後重試。`, recheckTroubleshooting: (value: string) => `檢查 ${value} 後重試。`,
  ...
  shared: {
    ...
    timeout: '請求逾時，請檢查網路或代理後重試。', unavailable: '模型服務暫時不可用，請稍後重試。',
    ...
    lastTest: {
      auth: '驗證失敗', timeout: '請求逾時', provider_unavailable: '模型服務回傳錯誤', network: '網路錯誤', invalid_response: '模型服務回傳錯誤', unknown: '連線測試失敗',
    },
  },
  panel: {
  ...
  add: {
    slugIssues: { required: '請填寫連線識別碼', format: '連線識別碼只能包含小寫字母、數字和連字號', too_long: '連線識別碼不能超過 64 個字元' }, duplicateSlug: '連線識別碼已存在', cloudflareAccount: '請填寫 Cloudflare Account ID', endpointRequired: '這個供應商需要填寫服務地址',
    ...
    requestHeaders: '自訂請求頭', headerName: '請求頭名稱', headerValue: '請求頭值', retainedHeaderValue: '保留已儲存的值', addHeader: '新增請求頭', removeHeader: '移除', noRequestHeaders: '未設定自訂請求頭。',
    extraRequestBody: '額外請求體（JSON）', extraRequestBodyHelp: '僅新增頂層欄位；與 Maka 生成欄位重名時會明確失敗。請求頭值將作為憑證儲存在本機。', requestCustomizationInvalid: '請檢查高階請求設定。',
    accountDetail: '不要在這裡手動新增；請回到模型連線頁的帳號連線完成登入，Maka 會自動建立並重新整理模型連線。',
    slug: '連線識別碼', name: '顯示名稱',
  ...
  oauthFlow: {
    ...
    logoutDescription: '將刪除本機儲存的訂閱憑證，之後需要重新登入才能繼續使用這些 OAuth 模型。', logout: '退出登入', cancel: '取消',
    loggedOut: '已退出登入', credentialsCleared: '本地憑證已清除。', logoutFailed: '退出失敗', logoutFailedRetry: '退出登入失敗，請稍後重試。',
    ...
    resultCodes: {
      ...
      copilot_credential_type_unsupported: '目前的 GitHub 憑證類型不受支援；請使用相容 OAuth 登入或 fine-grained PAT。',
      copilot_local_credential_missing: '找不到可匯入的 GitHub 憑證；請先使用 gh 登入或設定相容憑證。',
      copilot_import_no_credential: 'GitHub Copilot 登入沒有產生可用憑證。',
      copilot_import_superseded: 'GitHub Copilot 帳號在匯入期間發生變化，請重試。',
      copilot_subscription_unavailable: '目前的 GitHub 帳號沒有可用的 Copilot 訂閱權限。',
      copilot_credential_import_rejected: '目前的 GitHub 憑證無法匯入，請檢查憑證後重試。',
      ...
      experimental_disabled: '本機未啟用該帳號登入方式；可改用匯入相容憑證，或由管理員啟用後重試。',
    },
  ...
  oauthSection: {
    ...
    copilotDescription: '匯入相容 GitHub 憑證連線 Copilot 訂閱。', serviceUnavailable: '登入服務暫時不可用，請檢查網路後重試。',
    aria: 'OAuth 登入',
    ...
    copilotSignInDisabledHint: '本機未啟用 GitHub 登入；請改用匯入相容憑證，或由管理員啟用後重試。',
    importCredential: '匯入相容憑證',
    ...
    storageFailed: (name: string) => `${name} 本地憑證讀取失敗，請重新登入。`, providerUnavailable: (name: string) => `${name} 已登入，但目前 provider 狀態不可用。`,
  },
} as const;
