...
const koOverrides = {
    detail: {
      ...
      status: '연결 상태',
      statusHealthy: '정상',
      statusUntested: '테스트되지 않음',
      modelsSummary: (enabled: number, total: number) => `${enabled} / ${total} 활성화됨`,
      filterModels: '모델 검색',
      noModelsMatch: '일치하는 모델이 없습니다',
      enableModelAria: (name: string) => `${name} 모델 활성화`,
      declareCapabilities: '매개변수 구성',
      declareCapabilitiesAria: (name: string) => `${name} 모델 매개변수 구성`,
      modelUndescribed: '이 모델의 매개변수 정보가 없습니다. 직접 구성하세요.',
      visionToken: '비전',
      thinkingToken: '사고',
      contextToken: (value: string) => `${value} 컨텍스트`,
    ...
    shared: {
      ...
      filterMatches: (count: number) =>
        count === 0 ? '일치하는 결과가 없습니다' : `${count}개의 일치 결과`,
      connectionStatuses: {
    ...
    panel: {
      ...
      connections: '연결',
      connectionsHelp: '활성화된 모델은 작업의 모델 선택기에 표시됩니다.',
      retry: '선택하여 다시 시도하세요.',
      ...
      emptyHelp: '아래의 자주 사용하는 공급자에서 시작하거나 전체 공급자를 둘러보세요.',
      recommended: '추천 공급자',
      browseAll: '전체 공급자 보기',
      default: "기본",
      setDefault: "기본값으로 설정",
      ...
      modelCount: (value: number) => `${value}개 모델`,
    ...
    add: {
      ...
      onboardingSelectedCount: (selected: number, total: number) =>
        `${selected} / ${total} 선택됨`,
      onboardingSelectAll: '모두 선택',
      onboardingClearAll: '모두 선택 해제',
      onboardingNoModelsMatch: '일치하는 모델이 없습니다',
      onboardingDefaultModel: '기본 모델',
      onboardingDefaultModelHelp: '새 작업은 이 모델로 시작합니다. 선택한 모델만 지정할 수 있습니다.',
