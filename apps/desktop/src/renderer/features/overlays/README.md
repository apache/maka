...
- `platform/desktop/create-overlays-services.ts` is the only adapter from the
  Desktop bridge and browser environment into this feature (thread search and cancellation,
...
Search requests retain their request IDs through the controller and adapter.
The Search modal cancels the same request when it is superseded, dismissed,
or unmounted, preserving the search lifecycle introduced in #5256.
The legacy overlay layer retains `SettingsOverlay` as the single Escape
owner for both the pending Settings chunk and its resolved content.
