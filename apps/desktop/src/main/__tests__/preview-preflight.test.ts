...
describe('preview preflight surface derivation', () => {
  const verifiedGui = classifyGuiSurface({ ok: true, value: true });
  const absentGui = classifyGuiSurface({ ok: true, value: false });

  it('is verified only when both halves are', () => {
    assert.equal(
      derivePreviewSurface({
        guiSurface: verifiedGui,
        browserReachability: classifyBrowserReachability({
          guiSurface: verifiedGui,
          drivable: { ok: true, value: true },
        }),
      }),
      'verified',
    );
  ...
  it('lets a settled boundary on either half settle the whole', () => {
    assert.equal(
      derivePreviewSurface({
        guiSurface: absentGui,
        browserReachability: classifyBrowserReachability({
          guiSurface: absentGui,
          drivable: { ok: true, value: false },
        }),
  ...
  it('stays unknown when a half is merely unproven', () => {
    assert.equal(
      derivePreviewSurface({
        guiSurface: verifiedGui,
        // Refused right now because the conversation is off screen — transient.
        browserReachability: classifyBrowserReachability({
          guiSurface: verifiedGui,
          drivable: { ok: true, value: false },
        }),
...
describe('preview preflight tool', () => {
  it('claims ready only on a verified surface and an endpoint that answered', async () => {
    ...
    assert.match(result.summary, /A page can be shown now/u);
  });
  ...
  it('keeps an unnamed endpoint from reading as a missing capability', async () => {
    ...
    assert.equal(result.ready, false);
    assert.match(result.summary, /usable preview surface/u);
    assert.match(result.summary, /an absent question, not a negative result/u);
    assert.equal(
  ...
  it('separates a settled surface boundary from an unproven one', async () => {
    ...
    assert.match(headless.summary, /will not change at runtime/u);
  ...
  it('points an unready report at the managed preview endpoint', async () => {
    const result = await run(healthyAuthority());
    assert.equal(result.alternative, ARTIFACT_PREVIEW_HANDOFF);
