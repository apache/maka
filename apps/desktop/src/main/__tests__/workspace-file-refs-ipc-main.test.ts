...
test("Windows drive-letter references are paths, not URI schemes", async () => {
  // On Windows the drive letter is host-absolute and the containment check
  // decides; elsewhere `c:/notes.md` is a plain relative path, so the same
  // carve-out resolves a directory literally named `c:` inside the root.
  if (process.platform === "win32") return;
  const h = await createHarness();
  try {
    await mkdir(join(h.root, "c:"), { recursive: true });
    await writeFile(join(h.root, "c:", "notes.md"), "# drive");
    const read = h.handlers.get("workspace-files:readText")!;
    assert.deepEqual(await read({}, "session-1", "c:/notes.md"), {
      ok: true,
      name: "notes.md",
      text: "# drive",
    });
    // Multi-character schemes stay rejected, and a bare `d:` without a
    // following separator is drive-relative, not a file reference.
    assert.deepEqual(await read({}, "session-1", "mailto:a%20b.md"), {
      ok: false,
      reason: "invalid_reference",
    });
    assert.deepEqual(await read({}, "session-1", "d:"), {
      ok: false,
      reason: "invalid_reference",
    });
  } finally {
...
test("only regular files are openable, verified through the opened handle", async () => {
  const h = await createHarness();
  try {
    // A directory with a Markdown-looking name is not previewable content;
    // the fd-backed verification rejects it before the shell ever sees it.
    await mkdir(join(h.root, "folder.md"), { recursive: true });
    const read = h.handlers.get("workspace-files:readText")!;
    const open = h.handlers.get("workspace-files:openLocally")!;
    const reveal = h.handlers.get("workspace-files:revealInFolder")!;
    assert.deepEqual(await read({}, "session-1", "folder.md"), {
      ok: false,
      reason: "not_found",
    });
    assert.deepEqual(await open({}, "session-1", "folder.md"), {
      ok: false,
      reason: "not_found",
    });
    assert.deepEqual(await reveal({}, "session-1", "folder.md"), {
      ok: false,
      reason: "not_found",
    });
