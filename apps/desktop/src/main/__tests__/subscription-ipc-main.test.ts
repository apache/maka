...
function registerClaudeHandlers(options: {
  experimentalEnabled: boolean;
  serviceCalls: string[];
}): Map<string, Handler> {
  const handlers = new Map<string, Handler>();
  const record = (method: string, result: unknown) => async () => {
    options.serviceCalls.push(method);
    return result;
  };
  const deps = {
    ipcMain: {
    ...
    isClaudeSubscriptionExperimentalEnabled: () => options.experimentalEnabled,
    claudeSubscription: {
...
describe('Claude subscription experimental IPC gate', () => {
  test('fails closed before every sensitive service action when disabled', async () => {
    ...
    const handlers = registerClaudeHandlers({ experimentalEnabled: false, serviceCalls });
  ...
  test('delegates a representative auth action when enabled', async () => {
    const serviceCalls: string[] = [];
    const handlers = registerClaudeHandlers({ experimentalEnabled: true, serviceCalls });
