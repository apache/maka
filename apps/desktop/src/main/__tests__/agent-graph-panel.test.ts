...
describe('AgentGraphPanel dismiss', () => {
  it('mounts the production panel structure used by the layout contract', async () => {
    const graph = snapshot({
      graphId: 'graph-layout',
      status: 'active',
      scheduleRevision: 1,
      operators: [
        {
          operatorId: 'operator-layout',
          childSessionId: 'child-layout',
          provisionId: 'provision-layout',
          agentId: 'implementation',
          provisionedAt: 1,
          status: 'running',
          inboundEdgeIds: [],
          outboundEdgeIds: [],
          scheduledWorkIds: ['work-layout'],
          readiness: [],
          omitted: {
            inboundEdgeIds: 0,
            outboundEdgeIds: 0,
          ...
          currentActivation: {
            activationId: 'activation-layout',
            status: 'running',
            recordCount: 1,
            firstEventTime: 1,
            lastEventTime: 1,
            run: {
              sessionId: 'child-layout',
              agentRunId: 'run-layout',
              turnId: 'turn-layout',
            },
          },
        },
      ...
      work: [
        {
          workId: 'work-layout',
          target: { kind: 'preset', presetId: 'implementation' },
          inputIds: [],
          status: 'requested',
          instructionPreview: 'Keep the panel heading visible.',
    ...
    assert.ok(panel);
    const heading = panel.querySelector('.maka-agent-graph-heading');
    assert.ok(heading);
    const content = panel.querySelector('.maka-agent-graph-content');
    assert.ok(content);
    const controls = heading.querySelector('[aria-controls]');
    assert.equal(controls?.getAttribute('aria-controls'), content.getAttribute('id'));
    assert.equal(panel.querySelectorAll('.maka-agent-graph-operators > li').length, 1);
    assert.match(panel.textContent ?? '', /Keep the panel heading visible/);
