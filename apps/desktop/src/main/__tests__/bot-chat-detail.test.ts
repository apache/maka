...
const expectedCopy = {
  ...
  'zh-TW': {
    help: 'Telegram 使用者 ID 是 64 位元整數；填入後只接收清單裡這些 ID 的來信，其他使用者傳送的訊息會被直接忽略（不會回覆任何提示）。',
    cappedHelp: 'Telegram 使用者 ID 是 64 位元整數；填入後只接收清單裡這些 ID 的來信，其他使用者傳送的訊息會被直接忽略（不會回覆任何提示）（已達到上限）。',
    warnings: [
      [1, '以下輸入不是數字 ID，可能是使用者名稱，因此無法對應到任何使用者：@alice'],
      [3, '以下輸入不是數字 ID，可能是使用者名稱，因此無法對應到任何使用者：@alice、@bob、@carol'],
      [4, '以下輸入不是數字 ID，可能是使用者名稱，因此無法對應到任何使用者：@alice、@bob、@carol 等 4 項'],
      [5, '以下輸入不是數字 ID，可能是使用者名稱，因此無法對應到任何使用者：@alice、@bob、@carol 等 5 項'],
    ],
  },
  en: {
