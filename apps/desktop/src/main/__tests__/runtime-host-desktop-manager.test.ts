...
test('terminates a registered host the tracked update drain did not cover', async () => {
  const current = candidateHarness({ disconnectOnPrepare: true });
  const events: string[] = [];
  const owner = await startRuntimeHostDesktopManager({} as DesktopRuntimeHostCandidateStartInput, {
    startCandidate: async () => ready(current.candidate),
...
test('leaves the registration alone when it names the drained host', async () => {
  const current = candidateHarness({ disconnectOnPrepare: true });
  ...
  const owner = await startRuntimeHostDesktopManager({} as DesktopRuntimeHostCandidateStartInput, {
    ...
    readLocalHostRegistration: async () =>
      ({ pid: 42, lifecycleMode: 'ephemeral' }) as HostRegistration,
    isHostAlive: () => assert.fail('the drained pid must not be liveness-probed'),
    killHost: (pid) => {
      killedPids.push(pid);
    },
  });
...
test('does not terminate service-mode, unlabeled, dead, or unregistered residue', async () => {
  // Each case would be killed if its specific guard were removed: the
  // service and unlabeled registrations report a LIVE pid, so only the
  // explicit-ephemeral polarity protects them; the dead-ephemeral case is
  // protected only by the liveness probe; the unregistered case only by the
  // registration read.
  const cases: {
    name: string;
    ...
    {
      name: 'service registration with a live pid',
      registration: { pid: 77, lifecycleMode: 'service' } as HostRegistration,
      alive: true,
    },
    {
      // A registration written before lifecycleMode existed could belong to
      // a deployment-owned service host; the sweep must stay inert for it.
      name: 'unlabeled registration with a live pid',
      registration: { pid: 77 } as HostRegistration,
      alive: true,
    },
    {
      name: 'ephemeral registration with a dead pid',
      registration: { pid: 77, lifecycleMode: 'ephemeral' } as HostRegistration,
      alive: false,
    },
    { name: 'no registration', registration: undefined, alive: true },
  ];
  for (const { name, registration, alive } of cases) {
    const current = candidateHarness({ disconnectOnPrepare: true });
    ...
    assert.equal((await owner.prepareForUpdate(false)).kind, 'prepared', name);
    await owner.close();
  }
});

test('a residue that survives termination rejects the update preparation', async () => {
  const current = candidateHarness({ disconnectOnPrepare: true });
  const owner = await startRuntimeHostDesktopManager({} as DesktopRuntimeHostCandidateStartInput, {
    startCandidate: async () => ready(current.candidate),
    waitForHostExit: async (pid) => {
      if (pid === 77) throw new Error('Runtime Host did not exit before update');
    },
    readLocalHostRegistration: async () =>
      ({ pid: 77, lifecycleMode: 'ephemeral' }) as HostRegistration,
    isHostAlive: (pid) => pid === 77,
  ...
  await assert.rejects(
    () => owner.prepareForUpdate(false),
    /did not exit before update/,
  );
  await owner.close();
});

test('an unreadable registration does not fail the update preparation', async () => {
  const current = candidateHarness({ disconnectOnPrepare: true });
  const owner = await startRuntimeHostDesktopManager({} as DesktopRuntimeHostCandidateStartInput, {
    startCandidate: async () => ready(current.candidate),
    waitForHostExit: async () => {},
    readLocalHostRegistration: async () => {
      throw new Error('registration torn');
    },
    isHostAlive: () => assert.fail('an unreadable registration names no pid to probe'),
    killHost: () => assert.fail('an unreadable registration names no pid to terminate'),
  });
