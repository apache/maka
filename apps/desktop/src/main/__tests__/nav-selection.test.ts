...
describe('Navigation selection persistence', () => {
  it('round-trips Computer History without changing either hub memory', () => {
    const original = parseNavigationState(JSON.stringify({
      selection: { section: 'extensions', module: 'mcp' },
      moduleMemory: { extensions: 'mcp', automations: 'daily-review' },
    }));
    const history = selectNavigation(original, { section: 'computer-history' });
    assert.deepEqual(parseNavigationState(JSON.stringify(history)), history);
    assert.deepEqual(history.moduleMemory, original.moduleMemory);
    const chat = selectNavigation(history, { section: 'sessions' });
    assert.deepEqual(chat.moduleMemory, original.moduleMemory);
  });
