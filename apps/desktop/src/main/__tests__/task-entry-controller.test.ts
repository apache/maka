...
describe('useTaskEntryController', () => {
  it('selects the existing local workspace through a shell command despite a remote default', async () => {
      ...
      catalog: {
        ...
        getCatalog: async () => {
          ...
          return {
            ...
            hosts: [readyRemoteHost('remote-host'), readyHost()],
          };
        },
      },
    });
    await act(async () => renderController(root, services));
    assert.equal(controller().selectors.target?.profileId, 'remote');
    const expected = { profileId: 'local', hostId: 'host-local', projectId: 'project-a' };
    await act(async () => {
      assert.deepEqual(controller().commands.selectLocalTarget(), expected);
    });
    assert.deepEqual(controller().selectors.target, expected);
    assert.equal(reads, 1);
  });

  it('does not select a local workspace when the catalog has no ready local Host', async () => {
    const { root } = installReactRenderer();
    const services = createFakeTaskEntryServices({
      catalog: {
        ...createFakeTaskEntryServices().catalog,
        getCatalog: async () => ({ defaultProfileId: 'remote', hosts: [readyRemoteHost('remote-host')] }),
      },
    });
    await act(async () => renderController(root, services));
    await act(async () => {
      assert.equal(controller().commands.selectLocalTarget(), undefined);
    });
    assert.equal(controller().selectors.target?.profileId, 'remote');
  });
