...
test('executor selection uses stable commands across Session changes and disconnects on unmount', async () => {
  const writes: unknown[] = [];
  let refreshes = 0;
  const h = await mount({
    refreshCatalog: async () => { refreshes += 1; },
    services: services({
      setExecutorConfiguration: async (...args) => { writes.push(args); return session(args[0]); },
    }),
  });
  const commands = h.current().commands;
  await act(async () => {
    assert.equal(await commands.setSessionExecutor('a', { executorId: 'codex', model: 'model-a', thinkingLevel: 'high' }), true);
  });
  await h.select('b');
  assert.equal(h.current().commands, commands);
  await act(async () => {
    assert.equal(await commands.setSessionExecutor('b', { executorId: 'claude' }), true);
  });
  assert.deepEqual(writes, [
    ['a', { executorId: 'codex', model: 'model-a', thinkingLevel: 'high' }],
    ['b', { executorId: 'claude', thinkingLevel: null }],
  ]);
  assert.equal(refreshes, 2);
  await act(() => h.root.unmount());
  assert.equal(await commands.setSessionExecutor('a', { executorId: 'codex' }), false);
