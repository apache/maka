...
const startupTasks = createStartupTaskRegistry(runtimeHostStartupTaskPlan);
void startupTasks.runTask('resolve-shell-env', () => shellEnvReady).catch((error: unknown) =>
  console.error('[runtime-host] login shell environment resolution failed:', error),
);
...
const runtimeHostPeerConfiguration = await startupTasks.runTask(
...
await startupTasks.runTask('open-runtime-host-peer', async () => {
...
const [runtimeHostClientInstanceId, runtimeHostStartup] = await Promise.all([
  startupTasks.runTask('load-runtime-host-client-instance', () =>
    loadOrCreateRuntimeHostClientInstanceId(join(userDataDir, "runtime-host-client.json")),
  ),
  startupTasks.runTask('resolve-runtime-host-startup', () =>
...
startupTasks.runTaskSync('seed-e2e-fixture', () => {});
startupTasks.runTaskSync('resolve-storage-root', () => {});
...
startupTasks.runTaskSync('register-persistent-client-ipc', registerPersistentClientIpc);
startupTasks.runTaskSync('register-pet-pack-ipc', () => {
  registerPetPackIpc({
    ipcMain,
    workspaceRoot,
    mainWindowController,
...
startupTasks.runTaskSync('register-notifications-ipc', () => {
...
runtimeHostManager = startupTasks.runTaskSync(
  'connect-runtime-host',
  createLocalRuntimeHostManager,
);
...
void startupTasks.runTask('start-enabled-runtime-host-profiles', () => {
  void runtimeHostProfileService.startEnabledProfiles();
}).catch((error: unknown) =>
  console.error('[runtime-host] enabled Runtime Host profiles failed to start:', error),
);
...
export const runtimeHostBootReady = Promise.resolve(
  startupTasks.runTaskSync('initialize-renderer', () => {
...
void runtimeHostBootReady.then(async () => {
  ...
  await startupTasks.runTask('offer-unavailable-default-runtime-host', () => {
...
startupTasks.runTaskSync('start-desktop-background-services', () => {
...
startupTasks.runTaskSync('start-mcp', () => {
  void ensureMcpReady()
...
startupTasks.runTaskSync('resume-mcp-logins', () => {
  void mcpConfigStore
    .get()
    .then((config) => {
...
startupTasks.runTaskSync('refresh-client-settings', () => {
