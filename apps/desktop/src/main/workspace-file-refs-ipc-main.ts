 ...
 *   - Containment and open are one step: resolution returns the already-open
 *     file handle, never a bare path, so a concurrent swap of the target (or a
 *     parent) onto a symlink outside the root cannot slide a different file
 *     between the check and the read. Open/reveal verify through the same
 *     handle before handing the path to the shell wrappers.
 *
...
export function registerWorkspaceFileRefsIpc(
  ...
  handleReconnectableRead(
    ...
    async (_event, sessionId: string, reference: unknown): Promise<WorkspaceFileTextReadResult> => {
      ...
      try {
        // The size check and the read both happen on the opened handle, so
        // they describe and return the same bytes that passed containment.
        const info = await resolved.handle.stat();
        if (info.size > MAX_READ_BYTES) return { ok: false, reason: 'too_large' };
        const text = await resolved.handle.readFile('utf8');
  ...
  deps.ipcMain.handle(
    ...
    async (_event, sessionId: string, reference: unknown): Promise<WorkspaceFileOpenResult> => {
      ...
      const resolved = await resolveReference(deps.client, sessionId, reference);
      if (!resolved.ok) return resolved;
      // The handle verified the target is a regular in-root file; the shell
      // wrappers are path-based, so the fd closes after that verification.
      await resolved.handle.close();
...
type ResolvedReference =
  | { ok: true; handle: FileHandle; path: string; name: string }
  | { ok: false; reason: WorkspaceFileRefFailureReason };

/**
 * Resolve a raw reference against the session's workspace root with full
 * boundary enforcement, and open the contained target. The returned handle —
 * not the path — is the identity callers verify and read through: handing
 * back a path string would let a concurrent symlink swap race the later
 * stat/read/open onto a file that never passed containment. Callers own the
 * handle and must close it. Returns typed failures for every rejection so
 * callers never need to guess why a reference was refused.
 */
async function resolveReference(
  ...
  try {
    // Only regular files are openable references — directories named like
    // Markdown files (`.md` suffix) are not previewable content.
  ...
  return { ok: true, handle, path: canonical, name: basename(canonical) };
}

/**
 * Windows drive-letter paths (`C:\notes.md`, `c:/notes.md`) are
 * indistinguishable from a URI scheme by shape alone — `c:` matches the
 * scheme grammar below — so they are carved out first and left to host-local
 * path semantics: `isAbsolute` recognizes them on Windows, the containment
 * check decides everywhere else. A bare `C:` with no following separator is
 * drive-relative, not a usable file reference, and stays scheme-rejected.
 */
...
function normalizeReference(rawReference: unknown): string | null {
  ...
  if (!WINDOWS_DRIVE_PATH.test(rawReference) && /^[a-z][a-z0-9+.-]*:/i.test(rawReference)) {
    return null;
  }
