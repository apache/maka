...
export function getE2eFixtureState(fixture: E2eFixture | null): E2eFixtureState | null {
  ...
  switch (fixture.scenario) {
    ...
    case 'chat-large-history':
      return { ...state, activeSessionId: LARGE_HISTORY_SESSION_ID, workbarCollapsed: true };
    case 'settings-data':
...
export async function seedE2eFixture(input: {
  ...
  if (scenario === 'chat-large-history') {
    await writeSession(input.workspaceRoot, largeHistorySession(now), largeHistoryMessages(now));
  }
