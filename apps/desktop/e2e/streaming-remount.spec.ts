...
test('remounting a live surface leaves accumulated output settled', async ({
  ...
  // Runtime Host first seeds the active session snapshot before forwarding
  // live deltas. That cold subscription can exceed Playwright's 10s default
  // on the single-worker Linux CI runner near the end of the desktop suite.
