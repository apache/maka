 ...
 */
test('the help modal opens from its entry points, keeps its styled layout, and survives a blank UA-CH platform', async ({ window: page }) => {
  ...
  // environment is pinned by the reload phase below.)
   ...
   * installed before the reload below. Patched Astryx must fall through the
   * blank to navigator.platform and land on ⌘ — on every host OS, which is
   * what makes this phase discriminating in CI (unpatched, the blank decides
   * "not Apple" and the keycap reads Ctrl). Same window: the init script
   * takes effect on the reload, so the healthy-UA-CH assertions above and
   * this broken-environment phase share one launch.
   */
