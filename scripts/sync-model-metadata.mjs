...
export async function main(argv = process.argv) {
  ...
  // Hashing api.json would make the digest move whenever any provider changes,
  // including the ~40 this generator drops, producing header-only diffs that
  // say nothing about the file they sit in. Hashing the extracted payload
  // instead makes the digest describe what actually ships and keeps the
  // generator a pure function of its input: same api.json, same bytes out.
  //
  // Every export derived from the catalog is covered. A digest over a subset
  // would sit in a header claiming to describe the file while upstream changes
  // to the uncovered exports left it unmoved.
  const snapshot = {
    digest: createHash('sha256')
      .update(
        JSON.stringify({
          metadata: generated,
          modelProviderOverrides: generatedModelProviderOverrides,
...
function snapshotHeader(snapshot, handEditLine) {
  return [
    ...
    '// Snapshot: sha256 of the provider and model data extracted from api.json,',
    `// ${snapshot.digest}.`,
    '// models.dev serves a rolling document, so this identifies the extracted',
    '// payload rather than a revision that can be fetched back.',
    handEditLine,
  ];
}
