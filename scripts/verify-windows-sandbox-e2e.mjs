...
export async function verifyWindowsSandboxWorkerE2E(appDirectoryPath) {
  ...
  try {
    ...
    // The sandbox preview verifies relay, not CAS: the script owns every path
    // it writes and has no T0 snapshot to compare, so it opts out of the
    // identity check explicitly (#3484) rather than being mistaken for a
    // "target created while queued" race.
    const execute = (operation) =>
      client.execute({ operation, cwd: workspace, mode: 'ask', expectedIdentity: 'unchecked' });
