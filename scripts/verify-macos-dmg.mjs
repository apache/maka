...
/**
 * macOS draws the 16px and 32px faces of an ICNS from the ARGB slots `ic04`
 * and `ic05`. PNG data packed into the legacy `icp4`/`icp5`/`icp6` slots is
 * not decoded, so it renders as noise — which is what a Finder list row, the
 * Dock's Trash, Activity Monitor and the DMG's own volume icon show. Every
 * larger size keeps working, so nothing else in this verifier notices.
 *
 * electron-builder shipped that exact shape from 26.15.2 through 26.15.3
 * (electron-userland/electron-builder#9940). The icon is generated by a
 * toolset the builder downloads at build time, so the bundle is the only
 * place the result can be seen: assert on it here rather than on a version
 * pin that says nothing about what was produced.
 */
const UNRENDERABLE_ICNS_SLOTS = new Map([
  ['icp4', '16x16'],
  ['icp5', '32x32'],
  ['icp6', '64x64'],
]);
...
function readIcnsSlots(icns) {
  ...
  for (let offset = 8; offset + 8 <= icns.length; ) {
    ...
    if (length < 8 || offset + length > icns.length) {
      throw new Error(`Maka icon entry ${type} declares an unusable length of ${length}.`);
    }
    slots.set(type, icns.subarray(offset + 8, offset + length));
    offset += length;
  }
  return slots;
}
...
export async function verifyPackagedMacApp(
  ...
  await assertRenderableAppIcon(resources);
