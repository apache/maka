...
test('rejects a tiny non-native launcher before it reaches Desktop resources', async () => {
...
test('accepts a native release artifact without managed companions', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'maka-cu-helper-'));
  temporaryDirectories.push(directory);
  await writeFile(join(directory, 'maka-cu-windows.exe'), Buffer.alloc(10 * 1024 * 1024));
  await assert.doesNotReject(inspectWindowsCuArtifact(directory));
});
...
test('direct CLI execution copies the closed artifact and writes its manifest', async () => {
  ...
  assert.equal(manifest.windowsCu.binarySizeBytes, 600 * 1024);
