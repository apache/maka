    ...
    async (page) => {
      ...
      for (let trial = 0; trial < 3; trial++) {
        ...
        await measure('mount', async () => {
          await page.reload();
          await ready(PROMPT_RAIL_PROMPT_COUNT);
        });
        {
          expect(
            await page
      ...
      await report(
        'frontend-geometry-navigation',
        {
          ...
          samples,
          viewport: '1400x900',
          conditions: `One real Desktop + Host, three fresh renderer documents using production layout. Existing ${PROMPT_RAIL_PROMPT_COUNT}-turn fixture; three measurements per action.`,
          limits:
