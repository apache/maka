...
- Electron with the real Host: existing 120-Turn fixture switching and older/latest navigation; hold-open fake backend steering while editing, two background Tasks, Stop; DOM/heap samples after repeated navigation and an idle CPU profile.
...
- Repeated actions use a warm process. First-action samples are labelled separately, not called cold application startup. Storybook reloads each trial. Heap readings include garbage awaiting collection and do not establish a leak by themselves. Idle uses a three-second window and retains the CPU profile. Actual disk-cold startup, RTT injection, physical presentation, wakeups and long-duration leak acceptance remain outside this first harness.
