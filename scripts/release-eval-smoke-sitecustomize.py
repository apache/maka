...
class SingleStepTrial:
    ...
    async def run(self):
        ...
        reader, writer = await asyncio.open_connection(kwargs.relay_host, kwargs.relay_port)
        ...
        if execute.get("token") != kwargs.relay_token or execute.get("kind") != "execute":
        ...
        if decision.get("token") != kwargs.relay_token or decision.get("kind") != "verify":
            ...
            json.dumps(
                {
                    ...
                    "relayAgent": str(Path(sys.modules["relay_agent"].__file__).resolve()),
