...
impl MessageKey {
    fn durable(row: &Value) -> Self {
        Self {
            turn: row["turnId"].as_str().unwrap().into(),
            message: row["id"].as_str().unwrap().into(),
            part: if row["thinking"]["text"].is_string() {
                Part::Thinking
            } else {
                Part::Text
            },
    ...
    fn live(id: &SessionAssistantStreamIdentity) -> Self {
        Self {
            ...
            part: if id.kind == AssistantStreamKind::Thinking {
                Part::Thinking
            } else {
                Part::Text
            },
...
impl Transcript {
    ...
    pub(super) fn needs_fill(&self) -> bool {
    ...
    pub(super) fn discarded(&mut self, row: &Value) {
        let matches = |key: &MessageKey| row["turnId"] == key.turn && row["id"] == key.message;
        if self
            .anchor
            .as_ref()
    ...
    fn upsert(
        ...
        revision: Revision,
        kind: Kind,
        text: impl FnOnce() -> tools::Content,
    ) {
    ...
    pub fn sync(
        ...
        let date_changed = self.time_date.replace(today) != Some(today);
        let trace = self.trace;
        let tools = super::tools::Index::new(rows);
        ...
        for (sequence, row) in rows {
            ...
            if matches!(kind, "tool_call" | "tool_result") {
                match tools.entry(row) {
                    super::tools::Entry::Consumed => continue,
                    super::tools::Entry::Card(card) => {
                        ...
                        self.blocks
                            .get_mut(self.order.last().unwrap())
                            .unwrap()
                            .activity = card.activity();
                        continue;
                    }
                    super::tools::Entry::Raw => {}
                }
            }
            ...
            if matches!(presentation, Kind::User | Kind::Assistant) {
                ...
                if date_changed || block.time.is_none() {
                    let time = time::label(row["ts"].as_u64());
                    if block.time != time {
                        block.time = time;
                        block.layout = None;
                    }
...
mod tests {
    ...
    fn header_clicks_toggle_on_release_drag_selects_and_restored_focus_preserves_failure_color() {
        ...
        let hit = hits
            .iter()
            .find(|hit| hit.action == Action::ToggleMessage(key.clone()))
            .unwrap();
        assert_eq!(
            hit.area.width, 58,
    ...
    fn default_chat_hides_orchestration_but_keeps_real_tools_attention_and_opt_in_trace() {
        ...
        rows.remove(&1);
        rows.remove(&3);
        view.waiting.insert(("turn".into(), "call-0".into()));
        view.sync(&rows, &[], 0, &i18n, false);
    ...
    fn tool_cards_keep_identity_folds_and_results_through_pagination_waiting_and_interleaving() {
        ...
        assert!(draw(&mut view, 80, 40).contains("output-b"));
        view.waiting.insert(("turn".into(), "a".into()));
        view.sync(&rows, &[], 0, &i18n, false);
