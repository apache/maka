...
fn plugin_form_saves_in_place_and_preserves_a_stale_draft_without_model_execution() {
    ...
    // Discard only resets this address; it must not navigate to the first route.
    tui.wait_until(|screen| screen.contains("Plugin fixture 008") && !screen.contains("Draft"));
    tui.close_terminal();
    tui.finish();
    ...
    reopened.wait_for("Plugin fixture 008");
    assert!(
        !reopened
            .screen
            .snapshot()
    ...
    reopened.wait_for("Enabled");
    reopened.wait_for("Pinned");
    assert_eq!(
        runtime.block_on(catalog(&client))["revision"],
        merged["revision"],
        "restoring the exact 008 form must not write preferences"
    );
